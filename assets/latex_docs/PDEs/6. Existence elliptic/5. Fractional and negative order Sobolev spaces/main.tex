...
\begin{enumerate}
    ...
    \item Using these fractional spaces, one can obtain finer regularity results such as in the trace theorem or Sobolev embeddings.
\end{enumerate}
...
\begin{theorem}[Fractional trace theorem]\label{trace theorem}
    ...
    \end{align*}
    Furthermore, given $g \in B^{s-1/p,p}(\partial\Omega)$, there exists $u \in W^{s,p}(\Omega )$ such that $\operatorname{Tr}u=g$.
\end{theorem}
...
\begin{theorem}[Sobolev embedding into higher smoothness]\label{higher smoothness embedding}
    Let $\Omega $ be an extension domain for $W^{s,p}$. Then, given $p_1, p_2 \in [1,\infty)$ and $0<s_1<s_2< 1$, it holds that
    \begin{align*}
        \norm{u}_{W^{s_1,p_1}(\Omega )}\lesssim \norm{u}_{W^{s_2,p_2}(\Omega )}, \quad\forall s_2 - \frac{d}{p_2} = s_1 - \frac{d}{p_1}.
    \end{align*}
\end{theorem}
\begin{exercise}
    Justify via a scaling argument that the condition $s_2 - \frac{d}{p_2} = s_1 - \frac{d}{p_1}$ is necessary for the embedding in Theorem \ref{higher smoothness embedding}.
\end{exercise}
\begin{hint}
...
\begin{theorem}
    Let $\Omega $ be an extension domain for $W^{s,p}$ and consider $p_1,p_2 \in (1,\infty), 0 \leq s_1<s_2 \leq 1$ , and $0<\theta<1$. Then
    \begin{align*}
        \|u\|_{W^{*, p}(\R^d)} \lesssim\|u\|_{W^{s_1, p_1}(\R^d)}^\theta\|u\|_{W^{s_2, p_2}(\R^d)}^{1-\theta}
    \end{align*}

    for all $u \in W^{s_1, p_1}(\R^d) \cap W^{s_2, p_2}(\R^d)$, where $\frac{1}{p}=\frac{\theta}{p_1}+\frac{1-\theta}{p_2}$ and $s=\theta s_1+$ $(1-\theta) s_2.$
\end{theorem}
Higher order embeddings and interpolations can likewise be found in \cite{leoni2023first} Section 11.4.
