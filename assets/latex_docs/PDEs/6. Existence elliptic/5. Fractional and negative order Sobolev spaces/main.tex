...
\begin{exercise}[Fractional Rellish-Kondrachov]\label{fractional rellich}
    Let $\Omega $ be an extension domain for $W^{s,p}$, then, given $s \geq 0, p \in [1, \infty)$ it holds that
    \begin{align*}
...
\begin{hint}
    ...
    \begin{align*}
        \norm{D^\alpha u}_{L^{p_s^*}(\Omega )}\lesssim \norm{u}_{W^{\gamma ,p}(\Omega )} \lesssim \norm{u}_{W^{s,p}(\Omega )}, \quad \forall \abs{\alpha}\leq k.
    \end{align*}
    In particular, $\norm{u}_{W^{k,p_s^*}(\Omega )}\lesssim \norm{u}_{W^{s,p}(\Omega )}$. Then, using the integer case, we conclude
    \begin{align*}
        \norm{u}_{L^{p_s^*}(\Omega )}=\norm{u}_{L^{{(p_\gamma ^*)}_k ^*}(\Omega )}\lesssim \norm{u}_{W^{k,p_s^*}(\Omega )}\lesssim \norm{u}_{W^{s,p}(\Omega )}.
    \end{align*}
    For the second case, by Theorem \ref{subcritical embedding} and ua similar reasoning, we have
    \begin{align*}
...
\end{hint}
\begin{exercise}
    Show that, given $p_1, p_2 \in [1,\infty)$ and $0 \leq s_2<s_1 <\infty$, it holds that
    \begin{align*}
