...
$$
\int_{\mathbb{R}^d}\leftangle{x}\rightangle^{-s}dx<\infty\iff s>d\iff\sum_{k\in\Z^d}\leftangle{k}\rightangle^{-s}<\infty
$$
\end{lemma}
**Prof.** The equivalence of these two statements is a consequence of fact that $\leftangle{x}\rightangle^{-s}$ is monotone decreasing. To  prove the first part of the proposition it is sufficient to apply a change to spherical coordinates. ◻
...
[differentiate under the integral sign](https://nowheredifferentiable.com/2023-01-29-PDE-1/#derivativedistr:~:text=Proposition%202%20(-,Differentiation,-under%20the%20integral) (we recall that sums are
