...
The study of the FOurier transform is a vast subject and cannot be covered in a single blogpost We have chosen to concentrate on the case of functions with domain on euclidean space $\mathbb{R}^d$ or the torus $\mathbb{R}^d$
and studied the $L^2$ theory. However, one may also define the Fourier transform when the domain is a group $G$. In this case the notions of characters (for abelian groups) and representations (for non-abelian grous). In  the case where
...
Since every locally compact Hausdorff group has a [Haar measure](https://en.wikipedia.org/wiki/Haar_measure) $\mu$ (that is a measure on the Borel sets which is translation invariant and finite on compact sets) which is unique up to a multiplicative constant we can speak of the spaces  $L^p(G)$. The Fourier transform is then defined for all $f\in L^1(G)$ as:

<div>
$$\hat{f}(\xi):=\int_G f(x)\xi(x) \mu(dx)$$.
</div>

From here one can proceed as previously to extend teh FOurier transform on $G$ to an isometry (if the Haar measure is properly normalized) of $L^2(G)$. In the particular cases we studied we had that the Haar measure is just the Lebesgue measure and
<div>
$$\widehat{\mathbb{R}^d}=\left\{e^{2\pi ik \cdot}:k\in\Z^d\right\}\quad;\widehat{\mathbb{T}^d}=\left\{e^{2\pi ik \cdot}:k\in\Z^d\right\}$$.
</div>
Which helps shed light on why in the euclidean case we obtain an integral and in the periodic case we obtain a sum. It is also possible to study the $L^p$ theory of the
...
<div>
$$\|\mathcal{F}{f}\|_{L}\|_{L^q(\mathbb{R}^d\to \mathbb{C}^m)}\leq \|{f}\|_{L}\|_{L^p(\mathbb{R}^d\to \mathbb{C}^m)},\quad \forall 1\leq p\leq 2, $$
</div>
...
if and only if $1<p\leq 2$.
Note that in this second case we must restrict ourselves to dimension $1$. The case of dimension $d>1$ is an [open problem](https://terrytao.wordpress.com/2009/04/06/the-fourier-transform/#:~:text=Remark%2046-,It%20is,-a%20famous%20open). For the torus one has similar results where now the exponent of integration is extended
...
There exist counter examples at the boundary cases $p=1,\infty$. As Kolmogorov showed, the Fourier series may even be made to diverge almost everywhere (see [4](https://iopscience.iop.org/article/10.1070/RM1983v038n04ABEH004205/pdf) Theorem 3.1). WIth this we conclude this section.
