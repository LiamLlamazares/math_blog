...
where $1/p+1/q=1$. This extends the Fourier transform to $L^p(\mathbb{R}^d\to \mathbb{C}^m))$ for all $1<p\leq 2$ and to $L^p(\mathbb{T}^d\to \mathbb{C}^m))$ for all $p\geq 1$ (as $\mathbb{T}^d$ has finite measure).
