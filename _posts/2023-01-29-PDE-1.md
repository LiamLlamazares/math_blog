...
#  Three line summary  {#three-line-summary .unnumbered}
...
PDE(link) to these spaces and use their completeness to prove the
...
## Some useful results on integration {#some-useful-results-on-integration .unnumbered}
...
commodity.
 ...
 {#commuting limits with integral .proposition}
...
<div>
 $$f:T\times X\to E$$
  ...
  such that $f(t)$ is
(Bochner(link)) integrable for all ${t}\in{T}$ and such that
...
theorem(link) to the sequence of functions $f\U n:=f(t\U n)$ where
$\{t\U n\}\U {n=1}^\infty$ is some sequence converging to $t$ and the fact
that, in first countable spaces, sequential continuity(link) is
equivalent to continuity. ◻


 {#derivationundertheintsign .proposition}
...
<div>
 $$\mathcal{F}\U 1 f:=\hat{f}, \mathcal{F}\U 1^f$$
</div>

  for the Fourier transform
(on $L^1$) and its inverse. The following proposition holds.

 {#regularitydecayft .proposition}
...
Riemann-Lebesgue lemma(link)). In general, the Fourier transform of an
...
infinitely regular functions with infinite decay:
 ...
 {#Schwartz space definition .definition}
...
for the inner product on $L^2(\mathbb{R}^d\to\mathbb{C}^m)$.
 ...
 {#Planncherellemma .lemma}
**Lemma 1** (Plancherel for Schwartz functions). Given
$f,g\in \mathcal{S}(\mathbb{R}^d\to\mathbb{C}^m)$ (i)
$\left\langle\mathcal{F}\U \mathcal{S}f,g\right\rangle\U {L^2(\mathbb{R}^d\to\mathbb{C}^m)}=\left\langle f,\mathcal{F}\U \mathcal{S}^g\right\rangle\U {L^2(\mathbb{R}^d\to\mathbb{C}^m)};\quad\left\langle\mathcal{F}\U \mathcal{S}^f,g\right\rangle\U {L^2(\mathbb{R}^d\to\mathbb{C}^m)}=\left\langle f,\mathcal{F}\U \mathcal{S}g\right\rangle\U {L^2(\mathbb{R}^d\to\mathbb{C}^m)}$
(ii)
$\mathcal{F}\U \mathcal{S}^\mathcal{F}\U \mathcal{S}f=\mathcal{F}\U \mathcal{S}\mathcal{F}\U \mathcal{S}^f=f$


The first point can be proved by a direct application of Fubini. The
second point is trickier and is the crux of why we call $\mathcal{F}^$
...
That is, the restrictions of $\mathcal{F}\U 1$ and $\mathcal{F}^\U 1$
...
We obtain the following result.
 ...
 {#Plancherel's Theorem .proposition}
**Proposition 4** (Plancherel's theorem). $\mathcal{F}\U \mathcal{S}$
and $\mathcal{F}\U \mathcal{S}^$ may be extended to unitary operators:
...
$L^2(\mathbb{R}^d\to\mathbb{C}^m)$ together with $(ii)$ and Plancherel
(link). As, by a simple limiting argument, it is easy to see that
...
converges uniformly to $\mathcal{F}\U 1f$ by $1$ (point one prop 1 link)
and in $L^2(\mathbb{R}^d\to\mathbb{C}^m)$ to $\mathcal{F}f$ by
**Proposition 2** (link), from where we deduce that as desired
...
$\mathcal{F}\U {C^\infty}$ are unitary linear functions and that hence:
 ...
 {#Plancherelperiodictheorem .proposition}
...
$P: C^\infty({\mathbb T}^d\to\mathbb{C}^m)\to C^\infty({\mathbb T}^d\to\mathbb{R}^n)$.
 ...
 example
Example 1. Set $P=\Delta$, as we have seen previously (link)
...
since, as we saw in (link), the Fourier transform is an automorphism of
${\mathcal S}'(\mathbb{R}^d\to\mathbb{C}^m)$, by using property (link)
we deduce that
...
As a corollary of this, we have the following two results
 ...
 {#characterization smooth functions in Sobolev space .proposition}
