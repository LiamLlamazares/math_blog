...
_Proof_. The proof can be seen by verifying the above property for smooth functions $f \in C\U c^\infty(S\U n)$ and taking limits as $C\U c^\infty(S\U n)$ is dense in $L^2(S\U n)$ and measurability is preserved by limits. ◻
...
_Proof_. The proof is an application of Itô's (product) isometry to move
...
_Proof_. The proof is a bit technical but we sketch the main idea which
