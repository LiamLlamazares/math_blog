...
the inner product above. The reason is that we want the inner
...
Another example is the [Sobolev
...
_Proof._ The idea of the proof is to consider a sequence $f\U n$ that
...
Cauchy sequence. Since $H$ is complete, it converges to some $f$. Then, our assumptions on $F$ come in. Firstly, since $F$ is closed shows
...
_Proof._ Given $h \in  H$, we know that there exists a unique closest
...
are nonzero. Let us take the nonzero terms and relabel them
...
nonzero terms of $I$. The fact that the nonzero $x\U n$ sum to $x$ is
...
_Proof._ The proof is formally identical to the proof that every vector
...
_Proof._ We have that by the orthonormality of $\phi\U \alpha$ and the
...
_Proof._ Consider an orthonormal basis
...
Hilbert spaces provide us a way to guarantee the existence and uniqueness of solutions to
...
_Proof._ For each fixed $u \in H$, we have that
...
Furthermore, it can be verified that the mapping
...
that $\mathcal{L}$ is closed, invertibility follows. To do this, first note that the estimate in
...
As a result, given a Cauchy sequence if $\mathcal{L}u\U n \in  \Im (\mathcal{L})$, we deduce that $u\U n$ is also Cauchy. So, by completeness of $H$,
...
_Proof._ To prove that a solution to
(\ref{lax eq})
...
_Proof._ To prove the first two points are equivalent, we observe that
...
_Proof._ This follows by the last equivalent characterization in
...
_Proof._ The first part again follows from the last equivalent
...
_Proof._ Since $H$ is separable, there exists a countable orthonormal
basis $\left\\{\phi\U n\right\\}\U {n \in \mathbb{N}}$. If we denote $K\U n$ for
...
For the proof of a more general version, see page 334 of [2](https://books.google.co.uk/books?id=wI4fAwAAQBAJ&printsec=frontcover&hl=fr&source=gbs\U ge\U summary\U r&cad=0#v=onepage&q&f=false). It is important to observe the restriction that $U$ is bounded is
...
_Proof._ We have that $T= i \circ T$ so we conclude by
