...
<div>
 $$\begin{aligned}
                  \left.X\right|_{B}:=\left\{\left.f\right|_{B}: f\in X\right\} .
              \end{aligned}$$
</div>
...
-   We write $B_r(x)$ for the ball centered at $x$ with radius $r$ and
    $B_r$ if $x=0$. The space where the ball is contained depending on
    context.
...
<div>
 $$\begin{align}
\label{bc}
    \left.u\right|_{\partial  D}= g.\end{align}$$
</div>
...
<div>
 $$\begin{aligned}
                  \left\lVert \tilde{u} \right\rVert_{H^s(\mathbb{R}^d)}^2= \int_{\mathbb{R}^d}\left\langle\xi \right\rangle^{2s}\widehat{\tilde{u}}(\xi )^2 \,\mathrm{d}\xi < \infty\end{aligned}$$
</div>
...
holds that $\tilde{u}= {1}_{(0,1)}$ with
...
<div>
 $$\begin{aligned}
    H^s(U)= \left.H^s(\mathbb{R}^d)\right|_{U}:= \left\{\left.f\right|_{u}: f \in H^s(\mathbb{R}^d)\right\} .\end{aligned}$$
</div>
...
$\left.C^\infty(\mathbb{R}^d)\right|_{U}= C^\infty(U)$.
...
<div>
 $$\begin{aligned}
    (\varphi, D^\alpha u):= \int_{D} \varphi D^\alpha u  = (-1)^{\left| \alpha \right| }\int_{D}(D^{\alpha} \varphi) u + \text{ boundary effects}  .\end{aligned}$$
</div>
...
$C_c^\infty(U)$ to be the space of smooth functions whose support is
some compact set $K \subset U$.


Another notation for $C^\infty_c(U)$ is $\mathcal{D}(U)$ and it is often
...
<div>
 $$\begin{aligned}
        C_c^\infty(K):= \left\{\varphi \in C_c^\infty(U): \mathbf{supp}(\varphi) \subset K \right\} .
    \end{aligned}$$
</div>

  We endow $C_c^\infty(K)$ with the [topology
...
\label{local smooth norms}
        \left\lVert \varphi \right\rVert_{C^k(K)}:= \sup_{x \in K} \sum_{\left| \alpha \right|\leq k } \left| D^\alpha \varphi \right|, \quad  k \in \mathbb{N}_{0}
    \end{align}$$
</div>
...
<div>
 $$x+\left\{\rho^{-1}(B_\epsilon): \epsilon>0\right\}$$
</div>
...
checking the hints.
 ...
 <a name="TVS ex">
**Exercise 1** </a> . Write $\tau_{\mathcal{P}}$ for the topology generated
by $\mathcal{P}$, Show that $\tau_{\mathcal{P}}$ is the coarsest
...
The fact that $(X,\tau_\mathcal{P})$ is a TVS follows from the
triangle inequality and homogeneity of seminorms. The fact that it is
the coarsest that makes $\rho$ continuous is that
$\rho^{-1}(B_\epsilon)$ must be an open neighborhood of the origin and
...
open.
 ...
 <a name="convex ex">
**Exercise 2** </a> . Show that $(X,\tau_\mathcal{P})$ is locally convex.
...
**Exercise 3** </a> . Show that the topology $(X,\tau_\mathcal{P})$ is
...
-   Given a
    ...
    $x_\bullet\in (X,\tau_\mathcal{P})$ it holds that


<div>
 $$\lim x_\bullet =x \in  X  \iff      \rho(x_\bullet-x)\to 0 \quad\forall \rho \in \mathcal{P}.$$
</div>
...
follows from being able to fit $x_\bullet$ into any basic set
$x+\rho(B_\epsilon)$.
</div>
</div>


**Exercise 4**. Show that $C_c^\infty(K)$ is complete and thus a
...
[3](#convergence TVS)
to show that if $\varphi_n \in C_c^\infty(K)$ is Cauchy then the
sequence of derivatives $D^\alpha \varphi_n$ converge uniformly to
$\varphi^{(\alpha)} \in C_c^0(K)$ for all $\alpha$. It remains to show
that
...
Using $C_c^\infty(K)$ as a stepping stone we can build a topology on
$C_c^\infty(U)$. We use the approach in [Terence Tao's blog post on
...
Let us call a seminorm on $C_c^\infty(U)$ restrictable if it is a
continuous function on $C_c^\infty(K)$ for all $K \subset U$.


**Definition 4**. The topology on $C_c^\infty(U)$ is the one generated
by all the restrictable seminorms on $C_c^\infty(U)$. We call this
...
**Exercise 6**. Show that $C_c^\infty(\Omega)$ with the smooth topology
...
that $\varphi_n \to \varphi \in \mathcal{D}(U)$ if and only if there
exists a compact set $K$ such that for all $n$ the support of $f_n$ and
$f$ are in $K$ and
...
<div class="exercise-text">
Given any sequence
$\mathbf{a}=\left\\{a_j\right\\}_{j\in \mathbb{N}} \in \mathbb{R}_+$ and
an increasing set of compact sets $K_j$ with
$U=\bigcup_{j\in \mathbb{N}} K_j$ show that

<div>
 $$\begin{aligned}
        p_{\mathbf{a}}(\varphi):=\sup _{j \in \mathbb{N}}{a_j} \sum_{|\alpha| \leq j}\left\lVert D^\alpha \varphi(x) \right\rVert_{L^\infty(U\setminus K_j)}.
    \end{aligned}$$
</div>

  Is a restrictable seminorm. Why does this prevent
the support of $\varphi_n$ escaping to infinity? Now knowing all
functions are supported in some $K$ use that
$\left\lVert \cdot  \right\rVert_{C^k(K)}$ is restrictable to conclude
the proof.
</div>
</div>


**Exercise 8** (Completeness). Show that $C_c^\infty(\Omega)$ with the
...
<button class="exercise-button" onclick="toggleExercise(this)">Hint</button>
<div class="exercise-text">
Given a Cauchy net $\varphi_{\bullet} \in C_c^\infty(U)$ show
as in Exercise [7](#local convergence) that the support of $\varphi_{\bullet}$
cannot escape a compact set $K$. Conclude using the completeness on
$C_c^\infty(K)$ and Exercise [3](#convergence TVS).
</div>
</div>


**Exercise 9**. Show that $C_c^\infty(\Omega)$ with the smooth topology
is Hausdorff.
...
The construction of the topology on $C_c^\infty(U)$ is a bit technical a
more intuitive construction would be to define the family of seminorms


<div>
 $$\begin{aligned}
    \left\lVert \varphi \right\rVert_j := \sup_{\left| \alpha \right| \leq j}  \left\lVert D^\alpha \varphi \right\rVert_{L^\infty(U)}.\end{aligned}$$
</div>


And then define the topology on $C_c^\infty(U)$ to be the one generated
by this family of seminorms. This has the following problem.


**Exercise 10**. Show that $C_c^\infty(U)$ with the topology generated
by $\left\lVert \cdot  \right\rVert_j$ is not complete.
...
$\left\lVert \cdot  \right\rVert_j$ whose support escapes to infinity.
</div>
</div>
...
<div>
 $$\begin{aligned}
  ...
  And give it the
[weak-$$](https://en.wikipedia.org/wiki/Weak_topology) topology.
...
<div>
 $$\begin{aligned}
        C_c^\infty(U) \hookrightarrow C_c^\infty(\mathbb{R}^d) \hookrightarrow \mathcal{S}(\mathbb{R}^d)
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
    C^\infty(U)                & :=\left\{\varphi: \left\lVert \varphi \right\rVert_j:= \sup_{\left| \alpha \right| \leq j}  \left\lVert D^\alpha \varphi \right\rVert_{L^\infty(U)}<\infty,\quad \forall k \in \mathbb{N} \right\}                 \\
    C^\infty_{\mathrm{loc}}(U) & :=\left\{\varphi: \left\lVert \varphi \right\rVert_{k,K}:= \sup_{\left| \alpha \right| \leq k}  \left\lVert D^\alpha \varphi \right\rVert_{L^\infty(K)}<\infty,\quad \forall k \in \mathbb{N}, K \subset U \right\}\end{aligned}$$
</div>


Where, we give them respectively the topologies generated by
$\left\lVert \cdot \right\rVert_k$ and
$\left\lVert \cdot \right\rVert_{k,K}$.
...
if and only if for all $\varphi \in C_c^\infty(U)$ the operator
$\varphi w$ defined by
...
$L^p_{\mathrm{loc}}(U)$ for the space of locally $p$ integrable
functions,

<div>
 $$\begin{aligned}
        L^p_{\mathrm{loc}}(U):= \left\{f: \left\lVert f \right\rVert_{L^p(K)}< \infty, \,\text{   for all compact }  K \subset U\right\}.
    \end{aligned}$$
</div>
...
$\bigcup_{n \in \mathbb{N}} K_n =U$.


Note that $L^q_{\mathrm{loc}}(U) \subset L^p_{\mathrm{loc} }(U)$ for all
$q \leq p$.


**Exercise 11**. Show that $L^p_{\mathrm{loc} }(U)$ is a Fréchet
space.
...
[3](#convergence TVS)
if $f_n \in L^p_{\mathrm{loc}}(U)$ is Cauchy then $f_n$ is Cauchy for
all $L^p(K)$ $D^\alpha \varphi_n$ so converges to some $f_K \in L^p(K)$.
Show that $f_K= \left.f\right|_{K}$ where $f \in L^p_{\mathrm{loc}}(U)$
...
Proof. We begin by showing that $T_u \in \mathcal{D}'(U)$. Firstly,
...
<div>
 $$\begin{aligned}
        (f,\varphi u)= \int_{K}\varphi f u \leq \left\lVert u \right\rVert_{L^1(K)} \left\lVert \varphi f \right\rVert_{C^0_c(K)} .
    \end{aligned}$$
</div>

  Which, by our equivalent characterization of
$\mathcal{D}'(U)$ in (\ref{equiv})  shows that $T_u \in  \mathcal{D}'(U)$. We now show
...
the identity $\phi_n$ (see Appendix
[12](#smooth section) )
and set
...
$\varphi_n \subset C_c^\infty(U)$ for $n$ large enough and such that


<div>
 $$\begin{aligned}
        \lim_{n \to \infty}\varphi_n =g \in L^1(\mathbb{R}^d) ; \quad \left\lVert \varphi_n \right\rVert_{L^\infty(\mathbb{R}^d)} \leq \left\lVert g \right\rVert_{L^\infty(\mathbb{R}^d)}=1.
    \end{aligned}$$
</div>

  By the first part of the above, we may take a
subsequence $\varphi_{n_k}$ converging to $g$ almost everywhere, and by
the second we may apply the dominated convergence theorem to obtain that


<div>
 $$\begin{aligned}
        0= \lim_{k \to \infty}\int_{U}\varphi_{n_k} u=\int_{K}\left| u \right|.
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        (\varphi,T_{u} -T_v)\leq \left\lVert \varphi \right\rVert_{C_c^k(K)}\left\lVert u-v \right\rVert_{L^1_{\mathrm{loc}}(U)}.
    \end{aligned}$$
</div>
...
$L^1_{\mathrm{loc}}(U)$ as a subspace of $\mathcal{D}'(U)$. In
particular, any subspaces of $L^1_{\mathrm{loc} }(U)$ such as $L^p(U)$
or $C^\infty(U)$ can also be considered as distributions.


**Exercise 12**. Show that the $L^1_{\mathrm{loc}}(U)$ is not closed in
$\mathcal{D}'(U)$ .
...
Dirac delta $\delta _0 \in \mathcal{D}'(U)$. However
...
amended (consider for example the support of $0=1_{\mathbb{Q}}$). This
...
<div>
 $$\begin{aligned}
        (\varphi, w)=0 , \quad\forall \varphi \in C_c^\infty(U) \text{ with } \mathbf{supp}(\varphi) \subset V  .
    \end{aligned}$$
</div>
...
**Lemma 1** </a> . Let $\left\\{U_\alpha\right\\}_{\alpha \subset I}$ be a
...
extract a finite covering $\left\\{U_i\right\\}_{i=1}^n$ of
$\mathbf{supp}(\varphi)$. Let $\left\\{\rho _i\right\\}_{i=1}^n$ be a
partition of unity subordinate to $U_i$ (see Appendix
[13](#local to global)). Then

<div>
 $$\begin{aligned}
        (\varphi,\omega)=\left(\sum_{i=1}^n \rho _i \varphi,w \right)=\sum_{i=1}^n (\rho _i \varphi,w )=0 .
    \end{aligned}$$
</div>
...
$w$ vanishes. As a result, we can make the following definition.
 ...
 <a name="support def">
**Definition 8** </a> . Let $w \in C_c^\infty(U)$ and let $V$ be the largest
...
Since $L^1_{\mathrm{loc}}(U)$ is naturally included in $\mathcal{D}'(U)$
we obtain in particular the definition of support of a function
$f \in L^1_{\mathrm{loc}}(U)$.


**Exercise 13**. If $f \in L^1_{\mathrm{loc}}(U)$ the support of $f$ is
...
using that $\left\lVert \cdot  \right\rVert_{C^k(U)}$ are restrictable
...
**Exercise 15**. Let $u \in C_{\mathrm{loc}}^1(U)$ have classical
derivatives $u^{(i)}$. Then $u^{(i)}= \partial_i u$.
...
<div>
 $$\begin{aligned}
        T_{u^{(i)}}-T_{\partial _i u}=T_{u^{(i)}-\partial _i u}=0 \in \mathcal{D}'(U)   .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        \left\lVert u \right\rVert_{W^{k,p}(U)}&:=\sum_{\left| \alpha \right|\leq k } \left\lVert D^\alpha u \right\rVert_{L^p(U)}.        \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
    \left\lVert u \right\rVert_{W^{k,p}(U)}\sim \sum_{j=1}^k \left\lVert \nabla^j u \right\rVert_{L^p(U\to \mathbb{R}^{d^j})} .\end{aligned}$$
</div>



The local Sobolev spaces $W^{k,p}_{\mathrm{loc}}(U)$ are defined
similarly, where we now only require that

<div>
 $$\begin{aligned}
    D^\alpha u \in L^p_{\text{loc}}(U) , \quad\forall \left| \alpha \right|\leq k .\end{aligned}$$
</div>
...
$W^{k,p}_{\mathrm{loc}}(U)$ are Banach spaces. If $p \in (1,\infty)$
these spaces are also reflexive.



Proof. Let $\left\\{u_n\right\\}_{n=1}^\infty$ be a Cauchy sequence in
$W^{k,p}(U)$. Then, by definition of the norm on $W^{k,p}(U)$ the
sequence of derivatives $D^\alpha u_n$ is Cauchy in $L^p(U)$ for each
$\left| \alpha \right|\leq k$ and since $L^p(U)$ is complete, converge
to some functions $u^{\alpha}$

<div>
 $$\begin{aligned}
        \lim_{n \to \infty} D^\alpha u_n = u^{(\alpha)} \in L^p(U) , \quad\forall \left| \alpha \right| \leq k  .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        \int_{U} u^{(\alpha)}\varphi =\lim_{n \to \infty}\int_{U} u_n^{(\alpha)}\varphi=\lim_{n \to \infty}(-1)^{\left| \alpha \right| }\int_{U} u_nD^\alpha\varphi=(-1)^{\left| \alpha \right| }\int_{U} uD^\alpha\varphi  .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        T: W^{k,p}(U) & \longrightarrow (L^p(U))_{\left| \alpha \right|\leq k }
        \\
        u             & \longmapsto     (D^{\alpha}(u))_{\left| \alpha \right|\leq k }
        .\end{aligned}$$
</div>

  Is an isometry, so $\mathbf{Im}(T)$ is closed
in the reflexive Banach space $(L^p(U))_{\left| \alpha \right|\leq k }$
and thus reflexive (see [3](https://link.springer.com/book/10.1007/978-0-387-70914-7) page 70). The case of
$W^{k,p}_{\mathrm{loc}}(U)$ is proved identically now working with the
local seminorms. ◻
...
2.  The translation $\tau_y u(x):=u(x-y) \in W^{k,p}(U)$ with
    $D^\alpha \tau_y u = \tau_y D^\alpha u$.
...
<div>
 $$\begin{aligned}
         & (vu,D^\alpha\varphi )  = \int_{\mathbb{R}^d}v(y)\left(\int_{\mathbb{R}^d}D^\alpha \varphi(x) u(x-y) \,\mathrm{d}x \right) \,\mathrm{d}y \\&
        =(-1)^{\left| \alpha \right| } \int_{\mathbb{R}^d}v(y)  \left(\int_{\mathbb{R}^d}\varphi(x) D^\alpha u(x-y) \,\mathrm{d}x \right) \,\mathrm{d}y             =(-1)^{\left| \alpha \right| }(vD^\alpha u, \varphi).
    \end{aligned}$$
</div>
...
-   $u \in L^p_{\mathrm{loc}}(U)$ is almost everywhere differentiable
    with classical derivatives $u^{(i)} \in L^p_{\mathrm{loc}}(U)$ and,
...
$W^{1,p}_{\mathrm{loc}}(U),L^p_{\mathrm{loc}}(U)$ with their none local
...
$\tilde{u}$ and let $\left\\{\phi_n\right\\}_{n=1}^\infty$ be an
...
<div>
 $$\begin{aligned}
        \overline{\left.C_c^\infty(\mathbb{R}^d)\right|_{V}}=\left.W^{k,p}(U)\right|_{V}\quad \text{ and } \quad \overline{\left.C_c^\infty(\mathbb{R}^d)\right|_{U}}=W_{\mathrm{loc}}^{k,p}(U)
    \end{aligned}$$
</div>
...
as a result, the convolution $\varphi_n:= \tilde{u}  \phi_n$ verifies
...
<div>
 $$\begin{aligned}
  ...
  To conclude density of $C_c^\infty(\mathbb{R}^d)$ in
$\left.W^{k,p}(U)\right|_{V}$ consider $K$ such that
...
<div>
 $$\begin{aligned}
  ...
  And a bump function
$\eta_V \in C_c^\infty(\mathbb{R}^d)$ that is equal to $1$ on $V$ and is
supported in $K$ (this is possible by Uryshon's lemma and a
convolution). Then,

<div>
 $$\begin{aligned}
        \varphi_{V,n}:= \varphi_n \eta _V \in C_c^\infty(\mathbb{R}^d); \quad \lim_{n \to \infty}\varphi_{V,n}=u \in W^{k,p}(V).
    \end{aligned}$$
</div>

  The density of $C_c^\infty(\mathbb{R}^d)$ in
$W^{k,p}_{\mathrm{loc}}(U)$ follows by taking $V_n$ converging to $U$
and $u_n:= \varphi_{V_n,n}$ as then, for every compactly included
$W \Subset  U$
...
all of $\mathbb{R}^d$. This is because, the convolution $f \phi_n$ can
only be defined on

<div>
 $$\begin{aligned}
        U_{1/n}:= \left\{x \in U: d(x,\partial U)> \frac{1}{n}\right\} .
    \end{aligned}$$
</div>
...
Proof. Let $\eta   \in C_c^\infty(\mathbb{R}^d)$ be equal to $1$ on
$B_1$ and set $\eta  _n(x):=\eta  (x /n)$. Then, given
$u \in W^{k,p}(\mathbb{R}^n)$ and a smooth approximation to unity
$\phi_n$ we obtain that, by the triangle inequality

<div>
 $$\begin{aligned}
        u= \lim_{n \to \infty}(u\phi_n)\eta  _n \in W^{k,p}(\mathbb{R}^d).
    \end{aligned}$$
</div>
...
$\left\\{V_i\right\\}_{i=0}^\infty$ of $U$ with $V_0 =\emptyset$ and
$V_{i} \Subset V_{i+1}$. By Theorem
[17](#partition) we may
obtain a partition of unity $\rho _i$ subordinate to the "rings"
$U_{i}:= V_{i+1}\setminus \overline{V_{i-1}}$ (the trickery with the
indices is so that the $U_i$ actually cover $U$). Where we relabel so
that
...
<div>
 $$\begin{aligned}
  ...
  We have that $\rho_i u \in C_c^\infty({V_{i+1}})$
with $V_{i+1} \Subset U$ so by the local approximation in Theorem
[3](#local) we can find $n_i$
such that

<div>
 $$\begin{aligned}
        \left\lVert \phi_{n_i}(\rho_i u)-\rho_i u \right\rVert_{W^{k,p}(U)}=\left\lVert \phi_{n_i}(\rho_i u)-\rho_i u \right\rVert_{V_{i+1}}\leq \frac{\epsilon }{2^i} .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        ...
        _{W^{k,p}(U)}\leq\sum_{i=1}^\infty \left\lVert \phi_{n_i}(\rho_i u)-\rho_i u  \right\rVert_{W^{k,p}(U)}\leq \sum_{i=1}^\infty \frac{\epsilon }{2^i}=\epsilon   .
    \end{aligned}$$
</div>
...
functions in $C^\infty_{\mathrm{loc}}(U)$ to $C_c^\infty(\mathbb{R}^n)$.
...
$C^\infty_{\mathrm{loc}}(U)\cap W^{k,p}(U)$ with the
$\left\lVert \cdot  \right\rVert_{W^{k,p}(U)}$ norm.
...
$u_n \in C_c^\infty(\mathbb{R}^d)$. For $u_n$ the equality
(\ref{chain})  holds.
...
<div>
 $$\begin{aligned}
        \overline{\left.C_c^\infty(\mathbb{R}^d)\right|_{\Omega}}= W^{k,p}(\Omega).
    \end{aligned}$$
</div>
...
$\partial \Omega$ is $C^1$, given $x_0 \in \partial  \Omega$ there
...
<div>
 $$\begin{aligned}
        V \cap \Omega = \left\{x \in V :x_d  >\gamma (x_1,\ldots,x_{d-1})\right\}  .
    \end{aligned}$$
</div>

  By considering a translation in the last coordinate
$e_d=(0,\ldots,0,1)$ and its mollification by an approximation of unity
$\phi_n$

<div>
 $$\begin{aligned}
        u_n(x):= u\left(x+ \frac{2}{n}e_d \right); \quad \varphi_n:= u_n \phi_n .
    \end{aligned}$$
</div>

  For $n$ big enough we have that $\varphi$ is well
defined ans smooth on $W_0:=B_{\frac{1}{n}}(x_0)$. Since translation is
...
<div>
 $$\begin{aligned}
        \left\lVert u -\varphi_n \right\rVert_{W^{k,p}(W_0)}\leq \left\lVert u -u_n \right\rVert_{W^{k,p}(W_0)}+\left\lVert u_n -u_n\phi_n \right\rVert_{W^{k,p}(W_0)}\leq \epsilon.
    \end{aligned}$$
</div>

  Now, since $\Omega$ is bounded $\partial \Omega$ is
compact we may extract a finite covering
$\left\\{W_i:=B_{\frac{1}{n_i}}(x_i)\right\\}_{i=0}^n$ of
$\partial \Omega$ and functions $\left\\{\varphi_i\right\\}_{i=1}^n$
smooth on $W_i$ such that

<div>
 $$\begin{aligned}
        \left\lVert u -\varphi_n \right\rVert_{W^{k,p}(W_i)}\leq \frac{\epsilon}{n+2}  .
    \end{aligned}$$
</div>



Now we take an open set $W_{n+1} \Subset \Omega$ such that
$\left\\{W_i\right\\}_{i=0}^{n+1}$ cover $\Omega$. By the local
approximation of Theorem [3](#local) we know we can approximate $u$ on $W_{n+1}$ by some
$\varphi_{n+1} \in C_c^\infty(\mathbb{R}^d)$

<div>
 $$\begin{aligned}
        \left\lVert u-\varphi_{n+1} \right\rVert_{W^{k,p}(W_{n+1})}\leq\frac{\epsilon}{n+2} .
    \end{aligned}$$
</div>

  Finally, we take a smooth partition of unity
$\left\\{\rho _i\right\\}_{i=0}^{n+1}$ subordinate to
$\left\\{W_i\right\\}_{i=0}^{n+1}$ and a bump function
$\eta \in C_c^\infty(\mathbb{R}^d)$ which is equal to $1$ on $\Omega$
and is supported on $\bigcup_{i=0}^{n+1} W_i$ (see example
[5](#bump example2)) and
set
...
<div>
 $$\begin{aligned}
        \left\lVert u-\varphi \right\rVert_{W^{k,p}(\Omega)} \leq \sum_{i=0}^{n+1} \left\lVert \rho _i(u-\varphi_i) \right\rVert_{W^{k,p}(W_i)} \leq  \epsilon  .
    \end{aligned}$$
</div>
...
$k \in \mathbb{N}_+$. Then for all $p \in [1,\infty)$ . Then, given an
...
<div>
 $$\begin{aligned}
        \mathcal{H}^d =\left\{x=(x_1,\ldots,x_d) \in \mathbb{R}^d : x_d \geq 0\right\} .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
            \quad \mathcal{H}^d_{>0} =\mathrm{int}(\mathcal{H}^d)=\left\{x=(x_1,\ldots,x_d) \in \mathbb{R}^d : x_d >0\right\} .
    \end{aligned}$$
</div>
...
$\\{x_d=0\\}\cap \overline{\Omega'}$. That is, as long as $a_j$ verify
...
$k+1$-unknowns $a_j$. Its matrix is the [Vandermonde
...
<div>
 $$\begin{aligned}
        \left\lVert Eu \right\rVert_{W^{k,p}(\Omega')} \leq c\left\lVert u \right\rVert_{W^{k,p}(\Omega)} .
    \end{aligned}$$
</div>

  Where
$c:= (k+1)^{l+1}\max \left\\{1,\left\lVert a \right\rVert_\infty\right\\}$.
Working now in the general case for $\Omega$, we may cover the compact
$\overline{\Omega}$ by a finite amount of bounded open sets
$\Omega_i \subset W$ such that

<div>
 $$\begin{aligned}
        \Phi_i : \Omega_i \xrightarrow{\sim} \Omega_i'\quad \text{and}  \quad \Phi_i : \Omega_i \cap \Omega \xrightarrow{\sim} \Omega_i' \cap \mathcal{H}^d .
    \end{aligned}$$
</div>

  Where $\Omega_i'$ are open in $\mathbb{R}^{d}$ and
$\Phi_i$ are $C^k$ diffeomorphisms. By the previous case, we can extend
$u'_i:=u \circ \Phi_i^{-1} \in C^k (\Omega_i' \cap \mathcal{H}^d)$ to
functions $\widetilde{u'_i} \in C^k(\Omega_i')$ and then transform back
...
<div>
 $$\begin{align}
\label{bound}
        \left\lVert \tilde{u_i} \right\rVert_{W^{k,p}(\Omega_i)}\lesssim  \left\lVert u_i \right\rVert_{W^{k,p}(\Omega)}.
    \end{align}$$
</div>

  The hidden constant depending only on
$c,\left\lVert \Phi_i \right\rVert_{C^k(\Omega_i)}\left\lVert \Phi_i^{-1} \right\rVert_{C^k(\Omega_i')}$.
Next, using a partition of unity subordinate to
$\left\\{\Omega_i\right\\}_{i=1}^n$ we glue the local extensions together
...
<div>
 $$\begin{aligned}
        \left\lVert E u \right\rVert_{W^{k,p}(\mathbb{R}^d)} \lesssim  \left\lVert u \right\rVert_{W^{k,p}(\Omega)}.
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        W^{k,p}(\Omega)= \left.W^{k,p}(\mathbb{R}^d)\right|_{\Omega}; \quad C^{k}(\Omega)= \left.C^{k}(\mathbb{R}^d)\right|_{\Omega}.
    \end{aligned}$$
</div>
...
[7](#extension). By
definition $u= \left.Eu\right|_{\Omega}$. The case
...
**boundary** of type $C^k$ with $k \in \mathbb{N}_+$, we define for all
real $s \in [0,k]$
...
such as $\left.u\right|_{\partial \Omega}=0$. This is well defined if
$u$ is continuous, however, if $u \in  W^{k,p}(U)$, and is thus only
defined **almost everywhere**, then $\left.u\right|_{\partial  U}$ is a
...
<div>
 $$\begin{aligned}
  ...
  Such that $Tu =\left.u\right|_{\partial \Omega}$ for
...
Given $x_0 \in \partial  \Omega$ we take a open set
$U \subset \mathbb{R}^d$ containing $x_0$. Flattening out the boundary
...
<div>
 $$\begin{aligned}
        \int_{U'\cap \partial \mathcal{H}^d}\left| u' \right|^p  \leq\int_{\partial \mathcal{H}^d} \left| \widetilde{u}' \right|^p=\int_{\mathcal{H}^d} \partial_{d} \left| \widetilde{u}' \right|^p\leq\int_{\mathcal{H}^d} p\left| \widetilde{u}' \right|^{p-1}\left| \partial_{d} u \right|   \lesssim \left\lVert \widetilde{u}' \right\rVert_{W^{1,p}(\mathcal{H}^d)}^p  .
    \end{aligned}$$
</div>
...
\label{estimate local}
        \left\lVert u \right\rVert_{L^p(U \cap \partial \mathcal{H}^d)}\lesssim \left\lVert \widetilde{u}' \right\rVert_{W^{1,p}(\mathcal{H}^d)}\lesssim \left\lVert u' \right\rVert_{W^{1,p}(U'\cap \mathcal{H}^d)}\lesssim  \left\lVert u \right\rVert_{W^{1,p}(U\cap \Omega)}
    \end{align}$$
</div>

  We had supposed $u$ smooth, now taking a finite
covering $\left\\{U_i\right\\}_{i=1}^n$ of $\partial  \Omega$ (this is
possible by compactness of $\partial  \Omega$) and taking a subordinate
partition of unity $\rho _i$ we conclude from
(\ref{estimate local})  that

<div>
 $$\begin{aligned}
        \int_{\partial \Omega} \left| u \right|^p  =\sum_{i=1}^n \int_{ U_i \cap \partial \Omega} \left| u \right|^p \lesssim \sum_{i=1}^n \left\lVert u \right\rVert_{W^{1,p}(U_i \cap \Omega)}^p =\left\lVert u \right\rVert_{W^{1,p}(U)}^p .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        \left\lVert T u \right\rVert_{L^p( \partial \Omega)}\lesssim  \left\lVert u \right\rVert_{W^{1,p}(U)} .
    \end{aligned}$$
</div>

  Using Theorem [6](#global) to extend $T$ continuously to
$W^{k,p}(\Omega)= \overline{\left.C_c^\infty(\mathbb{R}^d)\right|_{\Omega}}$
concludes the proof. ◻
...
<div>
 $$\begin{aligned}
        \widehat{Tu}(\xi ')=\int_{\mathbb{R}}\widehat{u}(\xi) \,\mathrm{d}\xi _d .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{align}
\label{f1}
        \left| \widehat{Tu}(\xi ) \right|^2\leq \int_{\mathbb{R}}\left| \widehat{u}(\xi) \right|^2\left\langle\xi \right\rangle^{2s} \,\mathrm{d}\xi _d \int_{\mathbb{R}}\left\langle\xi \right\rangle^{-2s} \,\mathrm{d}\xi _d.
    \end{align}$$
</div>

  The change of variables
$\xi _d \to \left\langle\xi '\right\rangle\xi _d$ shows that


<div>
 $$\begin{align}
\label{f2}
        \int_{\mathbb{R}}\left\langle\xi \right\rangle^{-2s} \,\mathrm{d}\xi _d=\left\langle\xi' \right\rangle^{-2(s-\frac{1}{2})} \int_{\mathbb{R}}\left\langle\xi _d\right\rangle^{-2s}\,\mathrm{d}\xi _d\lesssim  \left\langle\xi' \right\rangle^{-2(s-\frac{1}{2})}
    \end{align}$$
</div>
...
<div>
 $$\begin{aligned}
        \left\lVert u \right\rVert_{H^{s-1 /2}(\mathbb{R}^{d-1})} \lesssim  \left\lVert u \right\rVert_{H^s(\mathbb{R}^d)}.
    \end{aligned}$$
</div>
...
$W^{k,p}(U)$.
 ...
 <a name="trace 0">
**Exercise 20** </a> . Show that for all $u \in W^{k,p}_0(\Omega)$
...
details.\
Being able to approximate functions in $W_0^{k,p}(U)$ by smooth
functions compactly supported **inside of** $U$ gives us many more
tools. For example, a function $u$ in $W^{k,p}_0(U)$ can be extended by
...
<div>
 $$\begin{aligned}
        E: W_0^{k,p}(U) \to W^{k,p}(\mathbb{R}^d); \quad  u \to \widetilde{u}
    \end{aligned}$$
</div>
...
$\left\lVert \widetilde{u} \right\rVert_{W^{k,p}(\mathbb{R}^d)}=\left\lVert \widetilde{u} \right\rVert_{W^{k,p}(U)}$
for $u \in C_c^\infty(U)$. As a result, we can extend $E$ by density to
the closure $C_c^\infty(U)$ in $W^{k,p}(U)$. Which by definition is
$W^{k,p}_0(U)$.
</div>
</div>
...
consider $u \in  W^{1,p}_0(U), v \in W^{1,p'}(U)$ then
...
<div>
 $$\begin{aligned}
        \int_{\Omega} (\partial _iu) v = -\int_{\Omega}u \partial _i v+ \int_{\partial \Omega} u v \textbf{n}_i  \,\mathrm{d}.
    \end{aligned}$$
</div>
...
$f_1, \ldots, f_d \in L^p\left(\mathbb{R}^{d-1}\right)$ for some
...
<div>
 $$\begin{align}
\label{loomis ineq}
        \left\lVert F_d \right\rVert_{L^{p}/({d-1})}\leq\prod_{i=1}^d\|f_i\|_{L^p\left(\mathbb{R}^{d-1}\right)} .
    \end{align}$$
</div>
...
from induction on $d$ . We write $(x_1,\ldots x_{d+1})=(x',x_{d+1})$


<div>
 $$\begin{align}
\label{induc}
        & \left\lVert F_{d+1} \right\rVert_{L^{p /d}(\mathbb{R}^{d+1})}  =\left(\int_{\mathbb{R}}\left(\int_{\mathbb{R}^d} F_d(x)^{\frac{p}{d}} f_{d+1}(x')^{\frac{p}{d}} \,\mathrm{d}x'  \right)  \,\mathrm{d}x_{d+1} \right)^{\frac{d}{p} }          \\
         & \leq \left(\int_{\mathbb{R}}         \left(\int_{\mathbb{R}^d} F_d(x)^{\frac{p}{d-1}} \,\mathrm{d}x'  \right)^{\frac{d-1}{d}}\,\mathrm{d}x_{d+1} \right)^{\frac{d}{p} }\left\lVert f_{d+1} \right\rVert_{L^p(\mathbb{R}^d)}      \notag               \\
         & \leq \left(\int_{\mathbb{R}} \prod_{i=1}^d\|f_i(x_{d+1})\|_{L^p\left(\mathbb{R}^{d}\right)}^{\frac{p}{d}} \,\mathrm{d}x_{d+1}\right)^{\frac{d}{p}}\left\lVert f_{d+1} \right\rVert_{L^p(\mathbb{R}^{d})}    \end{align}$$
</div>
...
<div>
 $$\begin{aligned}
    \left\lVert g_1\cdots g_n  \right\rVert_{L^1} \leq \left\lVert g_1 \right\rVert_{L^{p_1}}\cdots\left\lVert g_1 \right\rVert_{L^{p_n}}; \quad  \frac{1}{p_1}+\cdots \frac{1}{p_n}=1\notag          .\end{aligned}$$
</div>



to
$g_i:= \left\lVert f_i(\cdot ) \right\rVert^{\frac{p}{d}}_{L^p(\mathbb{R}^{d})} \in L^{d}(\mathbb{R})$
gives
...
<div>
 $$\begin{aligned}
        \int_{\mathbb{R}}\prod_{i=1}^{d}g_i \leq \prod_{i=1}^{d} \left\lVert g_i \right\rVert_{L^d(\mathbb{R})}    .
    \end{aligned}$$
</div>
...
$u \in C_c^\infty(\mathbb{R}^d)$. Applying the fundamental theorem of
calculus gives

<div>
 $$\begin{aligned}
        \left| u \right|^m=\int_{-\infty}^\cdot \partial _i \left| u \right|^m \,\mathrm{d}x_i  \leq \int_{\mathbb{R}}\left| u \right|^{m-1}\left| \partial _i u \right| \,\mathrm{d}x_i=: f_i , \quad\forall i=1,\ldots,d.
    \end{aligned}$$
</div>
...
<div>
 $$\begin{align}
\label{hard}
        \left\lVert u \right\rVert_{L^{ md /(d-1)}(\mathbb{R}^d)}^{md}\lesssim  \prod_{i=1}^{d}  \left\lVert u^{m-1}\partial_iu \right\rVert_{L^1(\mathbb{R}^d)} \leq \left\lVert u \right\rVert_{L^{(m-1)p'}(\mathbb{R}^d)}^{(m-1)d}\left\lVert \nabla u \right\rVert_{L^{p}(\mathbb{R}^d \to  \mathbb{R}^d)}^d
    \end{align}$$
</div>
...
<div>
 $$\begin{aligned}
 \left\lVert u \right\rVert_{L^{p^{k}}(\mathbb{R}^d)} \lesssim  \left\lVert \nabla^k u \right\rVert_{L^p(\mathbb{R}^d)}    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        \left\lVert u \right\rVert_{L^{ m d/(d-1)}(\mathbb{R}^d)}^{m}\lesssim  \left\lVert u \right\rVert_{L^{(m-1)d /(d-1)}(\mathbb{R}^d)}^{m-1}\left\lVert \nabla u \right\rVert_{L^{d}(\mathbb{R}^d \to  \mathbb{R}^d)} .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{align}
\label{22}
        \left\lVert u \right\rVert_{L^{ m d/(d-1)}(\mathbb{R}^d)}\lesssim \left\lVert u \right\rVert_{L^{(m-1)d/(d-1)}(\mathbb{R}^d)}+\left\lVert \nabla u \right\rVert_{L^{d}(\mathbb{R}^d \to  \mathbb{R}^d)} , \quad\forall  m \geq 1.
    \end{align}$$
</div>
...
<div>
 $$\begin{aligned}
        \left\lVert u \right\rVert_{L^{d^2 /(d-1)}(\mathbb{R}^d)} \lesssim  \left\lVert u \right\rVert_{W^{1,d}(\mathbb{R}^d)}.
    \end{aligned}$$
</div>

  We also trivially have
$\left\lVert u \right\rVert_{L^d(\mathbb{R}^d)} \leq\left\lVert u \right\rVert_{W^{1,d}(\mathbb{R}^d)}$
so by
...
<div>
 $$\begin{align}
\label{step 1}
        \left\lVert u \right\rVert_{L^{q}(\mathbb{R}^d)} \lesssim  \left\lVert u \right\rVert_{W^{1,d}(\mathbb{R}^d)} , \quad\forall q \in \left[d,\frac{d^2}{d-1}\right].
    \end{align}$$
</div>
...
<div>
 $$\begin{aligned}
        \left\lVert u \right\rVert_{L^{ (d+1)d/(d-1)}(\mathbb{R}^d)}\lesssim \left\lVert u \right\rVert_{L^{d^2/(d-1)}(\mathbb{R}^d)}+\left\lVert \nabla u \right\rVert_{L^{d}(\mathbb{R}^d \to  \mathbb{R}^d)}             \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        \left\lVert u \right\rVert_{L^{q}(\mathbb{R}^d)} \lesssim  \left\lVert u \right\rVert_{W^{1,d}(\mathbb{R}^d)} , \quad\forall q \in \left[d,\frac{(d+1)d}{d-1}\right].
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        \left\lVert u \right\rVert_{L^{q}(\mathbb{R}^d)} \lesssim  \left\lVert u \right\rVert_{W^{1,d}(\mathbb{R}^d)} , \quad\forall q \in \left[d+k,\frac{(d+k)d}{d-1}\right].
    \end{aligned}$$
</div>
...
$\gamma \in \mathbb{R}_+$, we define the Holder space

<div>
 $$\begin{aligned}
        C^{k,\gamma }(\mathbb{R}^d):= \left\{u \in C^k(U): \left\lVert u \right\rVert_{C^{k,\gamma }(U)}<\infty\right\} .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        ...
        \left\lVert u \right\rVert_{C^{k,\gamma }(U)} & :=\left\lVert u \right\rVert_{C^{k}(U)}+\sum_{\left| \alpha \right|= k } \left\lVert D^\alpha u \right\rVert_{C^{0,\gamma }(U)}   .
    \end{aligned}$$
</div>
...
1.  For the first inclusion we will use
    ...
    $L^q(\Omega) \hookrightarrow L^p(\Omega)$. Let $B_1$ be the unit
    ball in $W^{k,p}(\Omega)$. By continuity of the inclusion $B_1$ is
    bounded in $L^q(\Omega)$ and it only remains to show that $B_1$ is
    equicontinuous. We have that

<div>
 $$\begin{aligned}
                \left\lVert \tau_h u-u \right\rVert_{L^p(\Omega)}\leq \left\lVert \nabla u \right\rVert_{L^p(\Omega)}\left| h \right| , \quad\forall u \in W^{1,p}(\Omega) .
            \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
                         \left| D^\alpha u(x)- D^\alpha u(y) \right|&\lesssim  \left\lVert u \right\rVert_{C^{k}(\mathbb{R}^d)}\left| x-y \right| , &&\quad\forall \left| \alpha \right|<k\\
                         \left| D^\alpha u(x)- D^\alpha u(y) \right|&\lesssim  \left\lVert u \right\rVert_{C^{k,\gamma }(\mathbb{R}^d)}\left| x-y \right|^\gamma  , &&\quad\forall \left| \alpha \right|=k.
            \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        A_\alpha:=\left\{D^\alpha u: u \in B_1 \subset C^{k,\gamma }(\overline{\Omega})\right\},\end{aligned}$$
</div>


    is equicontinuous. So by Arzelà--Ascoli we may extract a sequence
    $u_{n}$ such that $D^\alpha u_n$ converges uniformly to some
    $u^{(\alpha)}$. By the fundamental theorem of calculus we conclude
    that $u^{(\alpha)}=D^\alpha u^{(0)}$ and as a result
    $u_n \to u \in C^k(\overline{\Omega})$. That is, the unit ball
    $B_1 \subset C^{k,\gamma }$ is sequentially compact and thus compact
...
sequence $u_n$ whose derivatives are bounded in certain $L^p$ norms we
...
**Theorem 14** (Poincaré inequality). Let $u \in W^{1,p}_0(U)$ where
$U$ is bounded in one direction. Then

<div>
 $$\begin{aligned}
        \left\lVert u \right\rVert_{W^{1,p}(U)}\leq C\left\lVert \nabla u \right\rVert_{W^{1,p}(U \to \mathbb{R}^d)}.
    \end{aligned}$$
</div>
...
Proof. By density (which holds by definition of $W^{k,p}_0(U)$) of it
is sufficient to reason for $u \in C_c^\infty(U)$ and pass to the limit.
By relabeling we may suppose $U$ is bounded along the $x_d$ axis. That
...
<div>
 $$\begin{aligned}
        \left| u(x) \right|= \left| \int_{a}^{x_d} \partial_d u(x',t) \,\mathrm{d}t \right| \leq \left(\int_{\mathbb{R}}\left| \partial _d u(x',t) \right|^p \,\mathrm{d}t\right)^\frac{1}{p}(x_d-a)^{\frac{1}{p'}}  .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        \left\lVert u \right\rVert_{L^p(\mathbb{R}^d)}\leq \left\lVert \partial _du \right\rVert_{L^p(\mathbb{R}^d)}\frac{p'}{p+p'}(b-a)^{\frac{p+p'}{p'}} \lesssim  \left\lVert \nabla u \right\rVert_{L^p(\mathbb{R}^d \to \mathbb{R}^d)} .
    \end{aligned}$$
</div>
...
define the convolution of $f,g$ to be the function $f g$
...
spaces $f,g$ belong to so that $fg$ is a well-defined element (of a
...
1.  If $f \in L^1(\mathbb{R}^d)$ and $g \in L^p(\mathbb{R}^d)$ then
    $fg \in L^p(\mathbb{R}^d)$ with

<div>
 $$\begin{aligned}
                      \left\lVert fg \right\rVert_{L^p(\mathbb{R}^d)}\leq \left\lVert f \right\rVert_{L^1(\mathbb{R}^d)}\left\lVert g \right\rVert_{L^p(\mathbb{R}^d)}.
                  \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
  ...
  Then $fg \in L^r(\mathbb{R}^d)$ with


<div>
 $$\begin{aligned}
                      \left\lVert fg \right\rVert_{L^r(\mathbb{R}^d)}\leq \left\lVert f \right\rVert_{L^p(\mathbb{R}^d)}\left\lVert g \right\rVert_{L^q(\mathbb{R}^d)}.
                  \end{aligned}$$
</div>



In any of the above cases $fg=gf$.
...
<div>
 $$\begin{aligned}
        \left\lVert \int_{\mathbb{R}^d}f(y)g(\cdot -y) \,\mathrm{d}y \right\rVert_{L^p(\mathbb{R}^d)}\leq\int_{\mathbb{R}^d}\left\lVert f(y)g(\cdot -y) \,\mathrm{d}y \right\rVert_{L^p(\mathbb{R}^d)}= \left\lVert f \right\rVert_{L^1(\mathbb{R}^d)} \left\lVert g \right\rVert_{L^p(\mathbb{R}^d)}.
    \end{aligned}$$
</div>

  To see the second point fix $f$ and define the
linear operator $T_fg:= fg$. Then, for $g \in  L^1(\mathbb{R}^d)$ and
$g \in  L^{p'}(\mathbb{R}^d)$ respectively

<div>
 $$\begin{aligned}
        \left\lVert T_f g \right\rVert_{L^p(\mathbb{R}^d)} \leq \left\lVert f \right\rVert_{L^p(\mathbb{R}^d)} \left\lVert g \right\rVert_{L^1(\mathbb{R}^d)}; \quad    \left\lVert T_f g \right\rVert_{L^\infty(\mathbb{R}^d)} \leq \left\lVert f \right\rVert_{L^p(\mathbb{R}^d)} \left\lVert g \right\rVert_{L^{p'}(\mathbb{R}^d)};  .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
    f\mu(x):= fg(x)= \int_{\mathbb{R}^d}f(x-y) g(y)\,\mathrm{d}y = \int_{\mathbb{R}^d} f(x-y)\,\mathrm{d}\mu (y) .\end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        f\mu (x) :=\int_{\mathbb{R}^d} f(x-y)\,\mathrm{d}\mu (y) \in L^p(\mathbb{R}^d).
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
    \left\lVert f \mu  \right\rVert_{L^p(\mathbb{R}^d)} \leq \left\lVert f \right\rVert_{L^p(\mathbb{R}^d)} \left\lVert \mu  \right\rVert_{TV} .\end{aligned}$$
</div>
...
<div>
 $$\begin{align}
\label{push}
    (h, \mu \nu) & := \int_{\mathbb{R}^d} h(x) fg(x) \,\mathrm{d}x = \int_{\mathbb{R}^d}\int_{\mathbb{R}^d}  h(x+y) f(x)  g(y)\,\mathrm{d}x \,\mathrm{d}y\\
...
absolutely continuous with density $fg$.



Proof. The first part is by definition of pushforward. To show that
$\mu \nu$ has density $fg$ it suffices to read the reasoning in
equation (\ref{push})
backward. ◻
...
<div>
 $$\begin{aligned}
        \mathbf{supp}(fg)=\mathbf{supp}(f)+\mathbf{supp}(g); \quad \mathbf{supp}(\mu\nu )=\mathbf{supp}(\mu )+\mathbf{supp}(\nu).
    \end{aligned}$$
</div>
...
with $g \in L_1(U)$ as the integral

<div>
 $$\begin{aligned}
    \int_{\mathbb{R}^d}f(y)g(x-y) \,\mathrm{d}y\end{aligned}$$
</div>
...
$\tilde{f}g$ is well defined and equal to

<div>
 $$\begin{aligned}
    \tilde{f}\phi(x) = \int_{B(x,\epsilon )\cap U} f(y)\phi(x-y) \,\mathrm{d}y+\int_{B(x,\epsilon )\cap U^c} g(y)\phi(x-y) \,\mathrm{d}y .\end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
    U_\epsilon :=\left\{x \in  U : d(x,\partial U)< \epsilon \right\}.\end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
    \tilde{f}\phi(x) = \int_{B(x,\epsilon )} f(y)\phi(x-y) \,\mathrm{d}y , \quad\forall x \in  U_\epsilon   .\end{aligned}$$
</div>
...
$T\varphi \in \mathcal{D}^\star (\mathbb{R}^d)$ by

<div>
 $$\begin{aligned}
    T\varphi(\phi):=T(\widetilde{\varphi}\phi) \quad \text{ where } \widetilde{\varphi}(x):=\varphi(-x) .\end{aligned}$$
</div>
...
$C_c^\infty(\mathbb{R}^d)$ and $\mathcal{D}'(\mathbb{R}^d)$ by
...
1.  Let
    ...
    then
    $\omega  \varphi \in  C^\infty _{\mathrm{loc}}(\mathbb{R}^d)$.
...
<div>
 $$\begin{aligned}
    fg(x):= \int_{G}f(y)g(x-y) \,\mathrm{d}\mu (y).\end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
    fg(x)=\int_{\mathbb{T}^d}f(y)g(x-y) \,\mathrm{d}y; \quad      fg(k):= \sum_{j\in \mathbb{Z}^d} f(j)g(k-j)  .\end{aligned}$$
</div>


The same results are also obtained. In fact, save the commutation
$fg=gf$, the above results hold even if $G$ is not Abelian. In this
...
$\\{\phi_n\\}_{n= 1}^\infty \subset L^1(\mathbb{R}^d)$ is an
**approximation to unity** if

-   Norm 1: $\left\lVert \phi_n \right\rVert_{L^1(\mathbb{R}^d)}=1$.

-   Decreasing support: $\mathbf{supp}(\phi_n) \subset B(0, 1 /n)$.

If $\phi_n \in C_c^\infty(\mathbb{R}^d)$ we say that $\phi_n$ are
smooth.
...
**Example 4**. Let $\varphi \in C_c^\infty(\mathbb{R}^d)$ then


<div>
 $$\begin{aligned}
    \phi_n:= \frac{n^d }{\left\lVert \varphi \right\rVert_{L^1(\mathbb{R}^d)}}\varphi(nx)\end{aligned}$$
</div>
...
<div>
 $$\begin{align}
\label{bump example}
        \varphi(x):=  \exp({\frac{1}{\left| x \right|^2-1}}) 1_{B(0,1)} \in C_c^\infty(\mathbb{R}^d)
    \end{align}$$
</div>
...
<div>
 $$\begin{aligned}
    fe=f , \quad\forall f \in L^1(\mathbb{R}^d).\end{aligned}$$
</div>
...
convolution exists. First, we need the following lemma.
 ...
 <a name="Uryshon">
**Lemma 4** </a> . The space $C_c(\mathbb{R}^d)$ is dense in
$L^p(\mathbb{R}^d)$ for all $p \in [1,\infty)$.



Proof. Consider $f \in  L^p(\mathbb{R}^d)$. If $f =1_A$ for some
...
there exists a continuous function $\varphi \in C_c(U)$ such that
$\varphi \leq 1$ and $\varphi$ is $1$ on $K$. Then,

<div>
 $$\begin{aligned}
        \left\lVert f- \varphi \right\rVert_{L^p(\mathbb{R}^d)} \leq \epsilon .
    \end{aligned}$$
</div>
...
$p \in [1,\infty)$ and consider $g \in C_c(\mathbb{R}^d)$ and an
approximation to unity $\phi_n$. Then it holds that

<div>
 $$\begin{aligned}
        \lim_{n \to \infty}g\phi_n=g \in  C_c(\mathbb{R}^d);\quad \lim_{n \to \infty}f\phi_n=f \in  L^p(\mathbb{R}^d).
    \end{aligned}$$
</div>
...
Proof. Consider $\epsilon >0$. Using that $\phi_n$ has mass $1$ and is
supported on $B(0,1/n)$.

<div>
 $$\begin{aligned}
        g\phi_n(x)-g(x) & = \int_{B(0, \frac{1}{n})}(g(x-y)-g(x)) \phi_n(y) \,\mathrm{d}y
        .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{align}
\label{nm}
        \left\lVert g\phi_n-g \right\rVert_{L^\infty(\mathbb{R}^d)} \leq \int_{B(0, \frac{1}{n})} \left\lVert g(\cdot -y) -g \right\rVert_{L^\infty(\mathbb{R}^d)} \phi_n(y) \,\mathrm{d}y\leq \epsilon   .
    \end{align}$$
</div>

  Where in the last inequality we used that $g$ is
uniformly continuous and $\phi_n$ has mass $1$. Since $\epsilon >0$ was
...
Now, since $K_n:=\mathbf{supp}(g \phi_n) \subset K + B(0, 1 /n)$, whose
...
<div>
 $$\begin{aligned}
        \left\lVert g\phi_n-g \right\rVert_{L^p(\mathbb{R}^d)}^p \leq \int_{K_n} \epsilon^p   \,\mathrm{d}y\leq M\epsilon^p  .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        \left\lVert f\phi_n-f \right\rVert_{L^p(\mathbb{R}^d)} & \leq\left\lVert (f-g)\phi_n \right\rVert_{L^p(\mathbb{R}^d)}+\left\lVert g\phi_n-g \right\rVert_{L^p(\mathbb{R}^d)} \\
...
$f\phi \in C^\infty(\mathbb{R}^d)$ with

<div>
 $$\begin{aligned}
        D^\alpha(f \phi)=fD^\alpha \phi , \quad\forall \alpha \in \mathbb{N}^d.
    \end{aligned}$$
</div>

  Furthermore, if $f$ is compactly supported then
$f\phi \in  C_c^\infty(\mathbb{R}^d)$.



Proof. By induction, it suffices to consider the case
$D^\alpha = \partial _i$ for some $1 \leq i \leq d$. This case can be
...
<div>
 $$\begin{aligned}
        \partial _{x_i} f(y)\phi(x-y) \leq f(y)\left\lVert \phi \right\rVert_{C^\infty(K)} 1_{K+B(0,M)}(y) \in L^1(\mathbb{R}^d)
    \end{aligned}$$
</div>
...
of Definition [19](#Convolution support), $f\phi \in C^\infty(U_\epsilon )$
with an identical proof showing

<div>
 $$\begin{aligned}
        D^\alpha(f \phi)=fD^\alpha \phi \text{ on } U_\epsilon .
    \end{aligned}$$
</div>
...
<div>
 $$\begin{aligned}
        \overline{C_c^\infty(\mathbb{R}^d)}= L^p(\mathbb{R}^d); \quad \overline{C_c^\infty(\mathbb{R}^d)} = C_c(\mathbb{R}^d).
    \end{aligned}$$
</div>
...
it first by a mollifier $\eta_n \in C_c^\infty(U)$ equal to $1$ in
$V_n\subset U$ (see Example [5](#bump example2)). Convolve to get

<div>
 $$\begin{aligned}
    \varphi_n:=(f \eta_n )\rho _n.\end{aligned}$$
</div>

  Show as in Theorem
[15](#density thm) that
$\varphi_n$ converges appropriately.
</div>
</div>
...
approximation to unity $\phi_n$ on $X$. Then
...
$\varphi_n$ be an approximation to unity. Then
...
<div>
 $$\begin{aligned}
  ...
  As a result,
$\overline{C_c^\infty(\mathbb{R}^d)}=\mathcal{D}'(\mathbb{R}^d)$.
...
function**) is a function $\eta \in C_c^\infty(\mathbb{R}^n)$.
...
exists $\eta \in C_c^\infty(\mathbb{R}^d)$ with support in $U$, equal to
...
Now, by [Urysohn's
lemma](https://en.wikipedia.org/wiki/Urysohn%27s_lemma#:~:text=for%20any%20two-,non%2Dempty,-closed%20disjoint%20subsets)
there exists a continuous function $f \in C_c(\mathbb{R}^n)$ such that
...
can obtain the desired function as $\eta=f\phi_{N}$. ◻



**Example 6**. There exists a sequence of
$\eta_n \in C_c^\infty(\mathbb{R}^d)$ such that
...
<div>
 $$\begin{aligned}
  ...
  And
$\left\lVert \eta_n \right\rVert_{C^k(\mathbb{R}^d)} \leq \left\lVert \eta_1 \right\rVert$.



Proof. Let $\eta_1 \in C_c^\infty(\mathbb{R}^d)$ be any (for example
that of (\ref{bump example}) ). Then we can take $\eta_n(x):=\eta(x /n)$. ◻
...
$\\{U_\alpha\\}_{\alpha \in  J}$ of $M$ we say that $\\{\rho_i\\}_{i\in  I}$
is a **partition of unity on $\\{U_\alpha\\}_{\alpha \in  J}$** if:

1.  $\mathbf{supp}(\rho_i)\subset U_\alpha$ for some $\alpha \in J$.

2.  For each $x \in M$, it holds that
    $x \in \mathbf{supp}(\rho_\alpha)$ for only a finite amount of
    $\alpha \in I$.
...
1.  Work in some open subset $\mathbb{R}^n$ (or the upper half space
    $\mathbb{R}^n_+$ if our manifold has boundary) to prove the
    existence of some object $g$ with desired properties.

2.  Cover the manifold $M$ with coordinate charts $U_\alpha$ and
    translate the euclidean result via the identification with
    $U_\alpha$ to obtain locally defined $g_\alpha$.
...
$\left\\{U_\alpha\right\\}_{\alpha \in  J}$ has a partition of unity
$\left\\{\rho _n\right\\}_{n=1}^\infty$.
...
$\left\\{U_i\right\\}_{i=0}^\infty$ of $M$ such that

<div>
 $$\begin{aligned}
    V_{i} \Subset V_{i+1}.\end{aligned}$$
</div>

  And then working with the
compact $\overline{V_{i+1}} \setminus V_{i}$. See [10](https://link.springer.com/chapter/10.1007/978-1-4419-7400-6_3)
...
<div>
 $$\begin{aligned}
    \mathcal{H}^d:=\left\{x=(x_1,\ldots,x_d)\in \mathbb{R}^d: x_d \geq 0\right\} .\end{aligned}$$
</div>
...
$V_\alpha$ and a collection of homeomorphisms with the subspace
topology from $\mathcal{H}^d$

<div>
 $$\begin{align}
\label{chart}
    \Phi_\alpha : V_\alpha \xrightarrow{\sim}\Phi(V_\alpha).\end{align}$$
</div>
...
<div>
 $$\begin{align}
\label{diff}
    \Phi_\beta \circ\Phi_\alpha^{-1}: \Phi_\alpha(V_\alpha\cap V_\beta ) \longmapsto \Phi_\beta (V_\alpha\cap V_\beta ).\end{align}$$
</div>


Are diffeomorphisms on $\mathcal{H}^d$ (again with the subspace
topology). Here $\mathcal{A}=\left\\{(U_\alpha,\Phi_\alpha)\right\\}$ is
...
<div>
 $$\begin{align}
\label{chart2}
        \left.\Phi_\alpha\right|_{\partial M}: V_\alpha \cap M \to \partial \mathcal{H}^{d-1} \simeq \mathbb{R}^{d-1}.
    \end{align}$$
</div>
...
functions $\Phi_\alpha$ to diffeomorphisms on $U_\alpha$ open in
$\mathbb{R}^d$ so that (\ref{chart}) -(\ref{chart2})  now read

<div>
 $$\begin{align}
\label{alt0}
    \Phi_\alpha : U_\alpha \xrightarrow{\sim}\Phi(U_\alpha); \quad    \Phi_\alpha : U_\alpha \cap M \xrightarrow{\sim}\Phi_\alpha(U_\alpha) \cap \mathcal{H}^d .\end{align}$$
</div>


Additionally, by the implicit function theorem there exists for each
coordinate set $U_\alpha$ functions
$\gamma_\alpha \in C^k(\mathbb{R}^{d-1})$ such that, relabeling the
coordinates and decreasing the size of $U_\alpha$ if necessary,


<div>
 $$\begin{align}
\label{alt1}
    \partial M \cap U_\alpha & =\left\{x \in U_\alpha : x_d=\gamma_\alpha(x_1,\ldots,x_{d-1}) \right\}\end{align}$$
</div>
...
<div>
 $$\begin{aligned}
    \Phi_\alpha(x',\gamma _\alpha(x')+\epsilon)=\Phi_\alpha(x)+\frac{\partial \Phi}{\partial x_d}(x)\epsilon +O(\left\lVert \epsilon  \right\rVert^2)  .\end{aligned}$$
</div>


We deduce that, once more reducing the size of $U_\alpha$ if necessary
and depending on the sign of $\partial _d \Phi$ on $U_\alpha$, one and
only one of the following two hold

<div>
 $$\begin{align}
\label{alt3}
   (M \setminus \partial M) \cap U_\alpha & =\left\{x \in U_\alpha : x_d>\gamma_\alpha(x_1,\ldots,x_{d-1}) \right\}                 \\
                 (M \setminus \partial M) \cap U_\alpha & =\left\{x \in U_\alpha : x_d<\gamma_\alpha(x_1,\ldots,x_{d-1}) \right\}\notag.\end{align}$$
</div>
