...
$$
\mathbb{E}\left[X(t)^{2}\right]=\theta^{2}+\frac{\sigma^{2} \theta}{2 \alpha}+e^{-2 \alpha t}\left(\mathbb{E}\left[X_{0}^{2}\right]+\left(2 \theta+\frac{\sigma^{2}}{\alpha}\right)\left(\frac{\theta}{2}-\mathbb{E}\left[X_{0}\right]\right)\right)+e^{-\alpha t}\left(2 \theta+\frac{\sigma^{2}}{\alpha}\right)\left(\mathbb{E}\left[X_{0}\right]-\theta\right) .
$$

[^0] Consequently,

$$
\lim _{t \rightarrow \infty} \mathbb{E}\left[X(t)^{2}\right]=\theta^{2}+\frac{\sigma^{2} \theta}{2 \alpha}
$$

and the long-term variance of the CIR process is

$$
\lim _{t \rightarrow \infty} \operatorname{Var}(X(t))=\lim _{t \rightarrow \infty} \mathbb{E}\left[X(t)^{2}\right]-\lim _{t \rightarrow \infty} \mathbb{E}[X(t)]^{2}=\frac{\sigma^{2} \theta}{2 \alpha} .
$$
...
$$
X_{n+1}=X_{n}+\alpha\left(\theta-X_{n}\right) \Delta+\sigma \sqrt{\left|X_{n}\right|} \Delta W_{n}
$$

here, and from now on, $\Delta>0$ denotes the step size of the scheme and $\Delta W_{n}=W\left(t_{n+1}\right)-W\left(t_{n}\right)$, with $t_{n}=n \Delta$. In [17] the authors determined that identity (4) is verified for the scheme (6) if $\Delta<2 / \alpha$. In addition, given the same restriction on $\Delta$, they show a band where the $\lim _{i n f} \operatorname{lod}_{n \rightarrow \infty} \mathbb{E}\left[X_{n}^{2}\right]$ and lim $\sup _{n \rightarrow \infty} \mathbb{E}\left[X_{n}^{2}\right]$ can be found. To improve the modified Euler scheme, which is a low-order method in the weak and strong senses of convergence, different numerical schemes have been proposed with better convergence rates for the CIR problem. This is particularly the case of the scheme $E(0)$

$$
X_{n+1}=\left(\left(1-\frac{\alpha \Delta}{2}\right) \sqrt{X_{n}}+\frac{\sigma \Delta W_{n}}{2-\alpha \Delta}\right)^{2}+\left(\alpha \theta-\frac{\sigma^{2}}{4}\right) \Delta,
$$
...
$$
\mathbb{E}\left[X_{n}\right]=\left(1-\frac{\alpha \Delta}{2}\right)^{2 n}\left(\mathbb{E}\left[X_{0}\right]-\frac{4 \theta}{4-\alpha \Delta}-\frac{\Delta \sigma^{2}}{(\alpha \Delta-2)^{2}}\right)+\frac{4 \theta}{4-\alpha \Delta}+\frac{\Delta \sigma^{2}}{(\alpha \Delta-2)^{2}},
$$

which, when $\Delta<2 / \alpha$, converges to

$$
\lim _{n \rightarrow \infty} \mathbb{E}\left[X_{n}\right]=\theta+\frac{\alpha \Delta \theta}{4-\alpha \Delta}+\frac{\Delta \sigma^{2}}{(\alpha \Delta-2)^{2}}>\theta,
$$
...
Taking the scheme (7) as a starting point, in this section we propose new methods with similar weak and strong convergence properties to solve numerically the CIR equation (1). From now on, we denote by $\mathcal{F}_{t}$ the $\sigma$-algebra generated by the standard Wiener process $\{W(t)\}_{t \geq 0}$ and, following [8] we use the extension of big $\mathcal{O}$ notation to random variables. For a family of random variables $X=\left\{X_{n}^{\Delta}\right\}$ we will write $X=\mathcal{O}\left(\Delta^{s}\right)$ when for any $p \in \mathbb{N}$ there exist $\delta, M>0$ such that

$$
\sup _{n} \mathbb{E}\left[\left|X_{n}^{\Delta} / \Delta^{s}\right|^{p}\right]<M
$$

when $0<\Delta<\delta$. In the following, the superscript $\Delta$ shall be omitted when its value is clear from context.

To solve numerically Eq. (1) with parameters verifying the restriction (8) we propose the schemes defined by $X_{0}=x_{0}$ and

$$
X_{n+1}:=\left(\left(1-\frac{\alpha}{2} \Delta+K_{\Delta} \Delta^{2}\right) \sqrt{X_{n}}+\frac{\sigma}{2} \Delta W_{n}+S_{\Delta} \Delta \Delta W_{n}\right)^{2}+\left(\alpha \theta-\frac{\sigma^{2}}{4}\right) \Delta
$$

for $n=0,1, \ldots$, where $K_{\Delta}=\mathcal{O}\left(\Delta^{0}\right), S_{\Delta}=\mathcal{O}\left(\Delta^{0}\right)$ are deterministic.

Notice that $X_{n} \geq 0, n \in \mathbb{N}$. Notice also that the scheme (7) is of the form (11) with $K_{\Delta}=0, S_{\Delta}=\alpha \sigma /(4-2 \alpha \Delta)$. We shall show that the proposed schemes (11) have uniformly bounded moments and converge to the exact solution of (1) in the strong and weak senses with the same order as $\mathrm{E}(0)$. The proofs rely on some results proven in [8].

Proposition 2.1. The schemes (11) have uniformly bounded moments.

Proof. It is clear that $\left\{X_{n}\right\}$ is non-negative and adapted to $\left\{\mathcal{F}_{t_{n}}\right\}$, where $t_{n}=n \Delta$. For all $n \in \mathbb{N}$ we have

$$
X_{n+1}<\left(1+\tau \Delta^{2}\right) X_{n}+\rho \sqrt{X_{n}} \Delta W_{n}+\mathcal{O}(\Delta)
$$

where

$$
\tau:=\frac{\alpha^{2}}{4}+\left|K_{\Delta}\left(2-\alpha \Delta+\Delta^{2} K_{\Delta}\right)\right|
$$

is $\mathcal{O}\left(\Delta^{0}\right)$ and positive and

$$
\rho:=\left(1-\frac{\alpha \Delta}{2}+K_{\Delta} \Delta^{2}\right)\left(\sigma+2 \Delta S_{\Delta}\right)
$$

is also $\mathcal{O}\left(\Delta^{0}\right)$ and $\mathcal{F}_{t_{n}}$-adapted. Hence, we conclude by using Lemma 2.6 in [8].
...
$$
\sup _{n} \mathbb{E}\left[\left|X_{n}^{\Delta}-X\left(t_{n}\right)\right|\right] \leq \frac{M}{|\log (\Delta)|} .
$$

Proof. The scheme (11) can be written as

$$
X_{n+1}=X_{n}+\alpha\left(\theta-X_{n}\right) \Delta+\sigma \sqrt{X_{n}} \Delta W_{n}+m_{n+1}-m_{n}+\mathcal{O}\left(\Delta^{2}\right)
$$

where $\left\{m_{n}\right\}$ is the discrete $\mathcal{F}_{t_{n}}$-adapted process, $t_{n}=n \Delta$, defined by $m_{0}:=0$ and

$$
m_{n+1}:=m_{n}+\frac{\sigma^{2}}{4}\left(\Delta W_{n}^{2}-\Delta\right)+\left(2 S_{\Delta}-\frac{\alpha \sigma}{2}\right) \sqrt{X_{n}} \Delta W_{n} \Delta ; \quad n \in \mathbb{N} .
$$

Since $m_{n+1}-m_{n}=\mathcal{O}(\Delta)$ and

$$
\mathbb{E}\left[m_{n+1}-m_{n} \mid \mathcal{F}_{t_{n}}\right]=0 ; \quad n \in \mathbb{N},
$$
...
$$
\mathbb{E}\left[f\left(X_{n}^{\Delta}\right)\right]-\mathbb{E}\left[f\left(X\left(t_{n}\right)\right)\right]=\mathcal{O}(\Delta)
$$
...
$$
\begin{aligned}
\left(X_{n+1}-X_{n}\right)^{2}= & \sigma^{2} X_{n} \Delta W_{n}^{2}+\frac{1}{2} \sigma^{3} \sqrt{X_{n}} \Delta W_{n}^{3} \\
& +\left(2 \alpha \theta \sigma \sqrt{X_{n}}-2 \alpha \sigma X_{n}^{3 / 2}-\frac{1}{2} \sigma^{3} \sqrt{X_{n}}\right) \Delta W_{n} \Delta+\mathcal{O}\left(\Delta^{2}\right) .
\end{aligned}
$$

Then if $t_{n}=n \Delta$,

$$
\mathbb{E}\left[\left(X_{n+1}-X_{n}\right)^{2} \mid \mathcal{F}_{t_{n}}\right]=\sigma^{2} X_{n} \Delta+\mathcal{O}\left(\Delta^{2}\right) .
$$
...
Since $\mathbb{E}\left[\Delta W_{n}\right]=0$ and $\mathbb{E}\left[\Delta W_{n}{ }^{2}\right]=\Delta$, taking expectations in (11) we have

$$
\mathbb{E}\left[X_{n+1}\right]=a(\Delta) \mathbb{E}\left[X_{n}\right]+b(\Delta)
$$

where

$$
a(\Delta):=\left(1-\frac{\alpha}{2} \Delta+K_{\Delta} \Delta^{2}\right)^{2}
$$

and

$$
b(\Delta):=\Delta\left(\alpha \theta+\Delta S_{\Delta} \sigma+\Delta^{2} S_{\Delta}^{2}\right)
$$

Therefore, if $\Delta>0$ is such that $a(\Delta) \neq 1$ then

$$
\mathbb{E}\left[X_{n}\right]=b(\Delta) \sum_{i=0}^{n-1} a(\Delta)^{i}+a(\Delta)^{n} \mathbb{E}\left[X_{0}\right]=b(\Delta) \frac{1-a(\Delta)^{n}}{1-a(\Delta)}+a(\Delta)^{n} \mathbb{E}\left[X_{0}\right] .
$$

Since $a(\Delta) \geq 0$, when $a(\Delta)<1$

$$
\lim _{n \rightarrow \infty} \mathbb{E}\left[X_{n}\right]=\frac{b(\Delta)}{1-a(\Delta)},
$$

and, consequently, $\left\{X_{n}\right\}$ has long term mean $\theta$ if and only if
...
$$
\frac{\alpha}{2 \Delta}-\frac{2}{\Delta^{2}}<K_{\Delta}<\frac{\alpha}{2 \Delta} \text {. }
$$

Then if (19) holds, Eq. (18) becomes the second order equation in terms of $S_{\Delta}$

$$
\Delta S_{\Delta}^{2}+\sigma S_{\Delta}+\frac{1}{4} \theta\left(\alpha^{2}+4 K_{\Delta}\left(2-\alpha \Delta+\Delta^{2} K_{\Delta}\right)\right)=0
$$

with solutions

$$
S_{\Delta, \pm}:=\frac{-\sigma \pm \sqrt{\sigma^{2}-\alpha^{2} \Delta \theta-4 \Delta \theta K_{\Delta}\left(2-\alpha \Delta+\Delta^{2} K_{\Delta}\right)}}{2 \Delta}
$$

when

$$
\sigma^{2}-\alpha^{2} \Delta \theta-4 \Delta \theta K_{\Delta}\left(2-\alpha \Delta+\Delta^{2} K_{\Delta}\right) \geq 0 .
$$

Since $\sigma>0$

$$
S_{\Delta,-}=-\frac{\sigma}{\Delta}+\frac{\theta\left(\alpha^{2}+8 K_{\Delta}\right)}{4 \sigma}+O(\Delta) \neq O\left(\Delta^{0}\right),
$$

whereas

$$
S_{\Delta,+}=\frac{\theta\left(\alpha^{2}+8 K_{\Delta}\right)}{4 \sigma}+O(\Delta)=O\left(\Delta^{0}\right) .
$$

For this reason we choose

$$
S_{\Delta}:=S_{\Delta,+}=\frac{-\sigma+\sqrt{\sigma^{2}-\alpha^{2} \Delta \theta-4 \Delta \theta K_{\Delta}\left(2-\alpha \Delta+\Delta^{2} K_{\Delta}\right)}}{2 \Delta} .
$$

To sum up:

Theorem 3.1. Given $\Delta>0$ and $K_{\Delta}=\mathcal{O}\left(\Delta^{0}\right)$ such that (19) and (22) hold, the scheme

$$
\begin{aligned}
X_{n+1}= & \left(\left(1-\frac{\alpha \Delta}{2}+\Delta^{2} K_{\Delta}\right) \sqrt{X_{n}}+\frac{1}{2} \sqrt{\sigma^{2}-\alpha^{2} \Delta \theta-4 \Delta \theta K_{\Delta}\left(2-\alpha \Delta+\Delta^{2} K_{\Delta}\right)} \Delta W_{n}\right)^{2} \\
& +\Delta\left(\alpha \theta-\frac{\sigma^{2}}{4}\right)
\end{aligned}
$$

is of the form (11) and gives numerical solutions of Eq. (1) that inherit the mean-reverting property of the exact solution, i.e. $\lim _{n \rightarrow \infty} \mathbb{E}\left[X_{n}\right]=\theta$.

The scheme (24) will be denoted $\mathrm{M}\left(K_{\Delta}\right)$.

Corollary 3.2. Consider the scheme (24) for approximating the mean-reverting solution of Eq. (1) when $\sigma^{2} \leq 4 \alpha \theta$.

1. If $\sigma^{2}=4 \alpha \theta$ then for every $\Delta>0$ and $K_{\Delta}=\mathcal{O}\left(\Delta^{0}\right)$ such that (19) holds, the scheme preserves the long-term mean $\theta$.
2. If $\sigma^{2}<4 \alpha \theta$ then for every $0<\Delta \leq \frac{4 \theta}{4 \alpha \theta-\sigma^{2}}$ and $K_{\Delta}=\mathcal{O}\left(\Delta^{0}\right)$ such that

$$
\frac{\alpha}{2 \Delta}-\frac{1}{\Delta^{2}}-\frac{\sqrt{1-\alpha \Delta+\frac{\Delta \sigma^{2}}{4 \theta}}}{\Delta^{2}} \leq K_{\Delta} \leq \frac{\alpha}{2 \Delta}-\frac{1}{\Delta^{2}}+\frac{\sqrt{1-\alpha \Delta+\frac{\Delta \sigma^{2}}{4 \theta}}}{\Delta^{2}},
$$
...
$$
\sigma^{2}-\alpha^{2} \Delta \theta-4 \Delta \theta K_{\Delta}\left(2-\alpha \Delta+\Delta^{2} K_{\Delta}\right)=-4 \Delta^{3} \theta\left(K_{\Delta}-\frac{\alpha}{2 \Delta}\right)\left(K_{\Delta}-\frac{\alpha}{2 \Delta}+\frac{2}{\Delta^{2}}\right) .
$$
...
$$
\begin{aligned}
& \sigma^{2}-\alpha^{2} \Delta \theta-4 \Delta \theta K_{\Delta}\left(2-\alpha \Delta+\Delta^{2} K_{\Delta}\right) \\
& =-4 \Delta^{3} \theta\left(K_{\Delta}-\frac{\alpha}{2 \Delta}+\frac{1}{\Delta^{2}}-\frac{\sqrt{1-\alpha \Delta+\frac{\Delta \sigma^{2}}{4 \theta}}}{\Delta^{2}}\right)\left(K_{\Delta}-\frac{\alpha}{2 \Delta}+\frac{1}{\Delta^{2}}+\frac{\sqrt{1-\alpha \Delta+\frac{\Delta \sigma^{2}}{4 \theta}}}{\Delta^{2}}\right)
\end{aligned}
$$
...
$$
L(\Delta)<K_{\Delta}<U(\Delta)
$$

where

$$
\lim _{\Delta \rightarrow 0} L(\Delta)=-\infty ; \quad \lim _{\Delta \rightarrow 0} U(\Delta)=+\infty .
$$

Since $K_{\Delta}=\mathcal{O}\left(\Delta^{0}\right)$, for any $K_{\Delta}$ there exists $\Delta_{0}>0$ such that scheme $\mathrm{M}\left(K_{\Delta}\right)$ applied with step size $\Delta \in\left(0, \Delta_{0}\right)$ preserves the mean reversion property.
...
Proposition 3.4. The numerical scheme $M_{0}:=M(0)$

$$
X_{n+1}=\left(\left(1-\frac{\alpha \Delta}{2}\right) \sqrt{X_{n}}+\frac{1}{2} \sqrt{\sigma^{2}-\alpha^{2} \Delta \theta} \Delta W_{n}\right)^{2}+\left(\alpha \theta-\frac{\sigma^{2}}{4}\right) \Delta
$$

applied with step size $\Delta<\frac{\sigma^{2}}{\alpha^{2} \theta}$ to solve Eq. (1) preserves the mean-reversion property of the exact solution. Proof. Since $\sigma^{2} \leq 4 \alpha \theta$, the proposed step-size verifies $\Delta<\frac{\sigma^{2}}{\alpha^{2} \theta} \leq 4 / \alpha$. Then the result is a direct application of Theorem 3.1, since when $K_{\Delta}=0$ condition (19) reduces to $\Delta<\frac{4}{\alpha}$ and condition (22) is $\Delta \leq \frac{\sigma^{2}}{\alpha^{2} \theta}$.

Proposition 3.5. The numerical scheme $M_{1}$ given by the expression

$$
X_{n+1}=\left(\sqrt{1-\alpha \Delta} \sqrt{X_{n}}+\frac{\sigma}{2} \Delta W_{n}\right)^{2}+\left(\alpha \theta-\frac{\sigma^{2}}{4}\right) \Delta
$$

applied with step size $0<\Delta<\frac{1}{\alpha}$ to solve Eq. (1) preserves the mean-reversion property of the exact solution.

Proof. The scheme $(27)$ is $\mathrm{M}\left(K_{\Delta}\right)$ with

$$
K_{\Delta}=\frac{\alpha}{2 \Delta}-\frac{1}{\Delta^{2}}+\frac{\sqrt{1-\alpha \Delta}}{\Delta^{2}} .
$$
...
Proposition 3.6. The numerical scheme $M_{2}$ given by the expression

$$
X_{n+1}=\left(1-\frac{4 \alpha \Delta \theta}{\Delta \sigma^{2}+4 \theta}\right)\left(\sqrt{X_{n}}+\frac{\sigma}{2} \Delta W_{n}\right)^{2}+\left(\alpha \theta-\frac{\sigma^{2}}{4}\right) \Delta
$$
...
Proof. The scheme (29) is $\mathrm{M}\left(K_{\Delta}\right)$ with

$$
K_{\Delta}=\frac{\alpha}{2 \Delta}-\frac{1}{\Delta^{2}}+\frac{\sqrt{1-\frac{4 \alpha \Delta \theta}{4 \theta+\Delta \sigma^{2}}}}{\Delta^{2}} .
$$

When $\sigma^{2}=4 \alpha \theta, K_{\Delta}=\frac{\alpha}{2 \Delta}-\frac{1}{\Delta^{2}}+\frac{1}{\Delta^{2} \sqrt{\alpha \Delta+1}}$ and condition (19) obviously holds. On the other hand, when $\sigma^{2}<4 \alpha \theta$ condition (25) reduces to
...
By means of the identity (17) for any scheme $X_{n}$ of the form (11) one can calculate $\mathbb{E}\left[X_{n}\right]$ using the corresponding $a(\Delta)$ and $b(\Delta)$. In the left plot of Fig. 1 we show the evolution of the means $\mathbb{E}\left[X_{n}^{\mathrm{M}_{0}}\right], \mathbb{E}\left[X_{n}^{\mathrm{M}_{1}}\right], \mathbb{E}\left[X_{n}^{\mathrm{M}_{2}}\right], n=t / \Delta$, of the schemes $\mathrm{M}_{0}, \mathrm{M}_{1}, \mathrm{M}_{2}$ when solving Eq. (1) with parameters $\alpha=0.43, \theta=0.06, \sigma=0.15$ in the interval $[0,15]$ with an equidistant partition of step size $\Delta=1 / 8$ and starting point $X_{0}=0.9 \theta$. Additionally, on the right plot of Fig. 1 , the error of each method $M$

$$
\varepsilon_{M}\left(t_{n}\right):=\left|\mathbb{E}\left[X_{n}\right]-\mathbb{E}\left[X\left(t_{n}\right)\right]\right|
$$
...
![](https://cdn.mathpix.com/cropped/2023_09_28_e7fdb60d06f1ce22d705g-07.jpg?height=456&width=1542&top_left_y=170&top_left_x=170)

Fig. 1. Evolution of means and weak errors of schemes (26), (27) and (29).

(24) depends on $K_{\Delta}$, with $K_{\Delta}=\mathcal{O}\left(\Delta^{0}\right)$ fulfilling (19) and (22). From (5) we must study if under these conditions the equation

$$
\lim _{n \rightarrow \infty} \mathbb{E}\left[X_{n}^{2}\right]=\theta^{2}+\frac{\sigma^{2} \theta}{2 \alpha}
$$

can be solved in terms of $K_{\Delta}$. Let us suppose that $\Delta$ and $K_{\Delta}$ are such that (19) and (22) hold. Using that $\mathbb{E}\left[\Delta W_{n}{ }^{4}\right]=3 \Delta^{2}$ we have that

$$
\mathbb{E}\left[X_{n+1}^{2}\right]=c(\Delta) \mathbb{E}\left[X_{n}^{2}\right]+d(\Delta) \mathbb{E}\left[X_{n}\right]+e(\Delta)
$$
...
\begin{aligned}
& c(\Delta):=a(\Delta)^{2}, \\
& d(\Delta):=6 a(\Delta) \Delta\left(\Delta S_{\Delta}+\frac{\sigma}{2}\right)^{2}+2 a(\Delta) \Delta\left(\alpha \theta-\frac{\sigma^{2}}{4}\right), \\
& e(\Delta):=2 \Delta^{2}\left(\Delta S_{\Delta}+\frac{\sigma}{2}\right)^{4}+b(\Delta)^{2},
\end{aligned}
$$
...
$$
\begin{aligned}
\mathbb{E}\left[X_{n+1}^{2}\right]= & c(\Delta) \mathbb{E}\left[X_{n}^{2}\right]+d(\Delta)\left(a(\Delta)^{n}\left(\mathbb{E}\left[X_{0}\right]-\frac{b(\Delta)}{1-a(\Delta)}\right)+\frac{b(\Delta)}{1-a(\Delta)}\right)+e(\Delta) \\
= & c(\Delta)^{n+1} \mathbb{E}\left[X_{0}^{2}\right]+d(\Delta) a(\Delta)^{n} \frac{1-a(\Delta)^{n+1}}{1-a(\Delta)}\left(\mathbb{E}\left[X_{0}\right]-\frac{b(\Delta)}{1-a(\Delta)}\right) \\
...
$$
\lim _{n \rightarrow \infty} \mathbb{E}\left[X_{n}^{2}\right]=\frac{\frac{b(\Delta) d(\Delta)}{1-a(\Delta)}+e(\Delta)}{1-c(\Delta)}=\frac{\theta d(\Delta)+e(\Delta)}{1-c(\Delta)} .
$$

where we have used (18). Inserting this value in (30), the approximation $\left\{X_{n}\right\}$ will have the desired long-term second moment if and only if
...
$$
\begin{aligned}
\left(4 \alpha \theta-\sigma^{2}\right) & \left(-2 \Delta^{8} \theta K_{\Delta}^{4}+4 \Delta^{6} \theta(\alpha \Delta-2) K_{\Delta}^{3}-3 \Delta^{4} \theta(\alpha \Delta-2)^{2} K_{\Delta}^{2}\right. \\
& \left.+\Delta^{2} \theta(\alpha \Delta-2)^{3} K_{\Delta}-\frac{\alpha}{8} \Delta^{2}\left(\alpha \theta(\alpha \Delta(\alpha \Delta-8)+8)+4 \sigma^{2}\right)\right)=0 .
\end{aligned}
$$

This equality is trivially satisfied for all $K_{\Delta}$ when $\sigma^{2}=4 \alpha \theta$. Then, using Corollary 3.2, if $\sigma^{2}=4 \alpha \theta$ for every $\Delta>0$ and $K_{\Delta}=\mathcal{O}\left(\Delta^{0}\right)$ such that (19) holds, the scheme preserves the long-term first and second moments. On the other hand, when $\sigma^{2}<4 \alpha \theta$ Eq. (33) reduces to a quartic equation which only has real solutions

$$
K_{\Delta, \pm}:=\frac{\alpha}{2 \Delta}-\frac{1}{\Delta^{2}} \pm \frac{\sqrt[4]{\theta\left(4 \theta(\alpha \Delta-1)^{2}-\alpha \Delta^{2} \sigma^{2}\right)}}{\sqrt{2} \Delta^{2} \sqrt{\theta}},
$$
...
$$
K_{\Delta,-}=-\frac{2}{\Delta^{2}}+\frac{\alpha}{\Delta}+\frac{\alpha\left(2 \alpha \theta+\sigma^{2}\right)}{16 \theta}+\mathcal{O}(\Delta) ; \quad K_{\Delta,+}=-\frac{\alpha\left(2 \alpha \theta+\sigma^{2}\right)}{16 \theta}+\mathcal{O}(\Delta) ;
$$
...
$$
K_{\Delta,+}=\frac{\alpha}{2 \Delta}-\frac{1}{\Delta^{2}}+\frac{\sqrt[4]{\theta\left(4 \theta(\alpha \Delta-1)^{2}-\alpha \Delta^{2} \sigma^{2}\right)}}{\sqrt{2} \Delta^{2} \sqrt{\theta}} .
$$

We prove now that the numerical method of the class (24) with $K_{\Delta}=K_{\Delta,+}$ preserves the two first moments if it is applied with a sufficiently small step-size.

Proposition 4.1. The numerical scheme MS

$$
\begin{aligned}
X_{n+1}= & \frac{1}{4}\left(\sqrt[4]{4\left(4(1-\alpha \Delta)^{2}-\frac{\alpha \Delta^{2} \sigma^{2}}{\theta}\right) X_{n}}+\sqrt{\frac{4 \theta(1-\alpha \Delta)+\Delta \sigma^{2}-2 \sqrt{\theta\left(4 \theta(1-\alpha \Delta)^{2}-\alpha \Delta^{2} \sigma^{2}\right)}}{\Delta} \Delta W_{n}}\right)^{2} \\
...
we have that $1 /(2 \alpha) \leq \Delta^{-}$and from here $p(\Delta) \geq 0$ if $\Delta \leq 1 /(2 \alpha)$. Then we conclude that $K_{\Delta,+}$ fulfills (34) if $\Delta \leq 1 /(2 \alpha)$.

To prove the statement we show that $K_{\Delta,+}$ fulfills the conditions of Corollary 3.2. When $\sigma^{2}=4 \alpha \theta$ condition (19) for $K_{\Delta,+}$ reads

$$
-\frac{1}{\Delta^{2}}<\frac{\sqrt[4]{1-2 \alpha \Delta}}{\Delta^{2}}<\frac{1}{\Delta^{2}}
$$

![](https://cdn.mathpix.com/cropped/2023_09_28_e7fdb60d06f1ce22d705g-09.jpg?height=424&width=1522&top_left_y=178&top_left_x=176)

Fig. 2. Evolution of the second moment of scheme (36) and its error $\varepsilon_{M S}^{2}$.
...
By means of the identity (32) we calculate $\mathbb{E}\left[X_{n}^{2}\right]$ for the scheme MS. Its evolution when solving Eq. (1) with parameters $\alpha=0.43, \theta=0.06, \sigma=0.15$ in the interval $[0,15]$ with an equidistant partition of step size $\Delta=1 / 8$ and starting point $X_{0}=0.9 \theta$ is shown on the left plot of Fig. 2. Additionally, on the right plot of Fig. 2, the error

$$
\varepsilon_{M}^{2}\left(t_{n}\right):=\left|\mathbb{E}\left[X_{n}^{2}\right]-\mathbb{E}\left[X\left(t_{n}\right)^{2}\right]\right|
$$
...
In this section we carry out two experiments. The first one is devoted to testing the strong order of the proposed methods, whereas the second one is used to confirm the theoretical results regarding the preservation of the first two long-term moments. In addition to $H M, E(0)$, the schemes $M_{0}, M_{1}$ developed in Section 3, and MS proposed in Section 4, for the sake of comparison we shall use the drift-implicit method (DI) proposed by Alfonsi in [8] and the Milstein-like scheme (HH) proposed by Hefter and Herzwurm in [12].
...
![](https://cdn.mathpix.com/cropped/2023_09_28_e7fdb60d06f1ce22d705g-10.jpg?height=834&width=1533&top_left_y=170&top_left_x=178)
...
The results of the analysis with the three sets of parameters are shown in Fig. 3, where the log-log plots of the strong error against $\Delta$ have been plotted. To serve as a reference, dashed straight lines with slopes $\frac{1}{2}$ and 1 have been added. And for a better understanding we show in Table 1 the average slope of every method for each set of parameters. It can be seen that the DI scheme as well as our schemes $\mathrm{M}_{1}$, MS present the lowest errors. Notice that the values for $\sigma^{2} / 4 \alpha \theta$ are $1,3 / 4$ and $1 / 4$ for (37), (38) and (39) respectively. The results show, in agreement with [6], that for increasing values of $\sigma^{2} / 4 \alpha \theta$ the experimental rate of convergence decreases. It can be seen that scheme HM (and HH except for parameters (39)) show experimental rates of strong convergence close to $1 / 2$. On the other hand, schemes $\mathrm{DI}, \mathrm{E}(0), \mathrm{M}_{1}$ and MS present strong orders of convergence greater than $1 / 2$, and approximately equal to 1 for parameters (38) and (39).
![](https://cdn.mathpix.com/cropped/2023_09_28_e7fdb60d06f1ce22d705g-11.jpg?height=834&width=1518&top_left_y=173&top_left_x=180)
...
$$
\bar{X}_{n}:=\frac{1}{N} \sum_{k=1}^{N} X_{n, k} ; \quad \overline{X_{n}^{2}}:=\frac{1}{N} \sum_{k=1}^{N} X_{n, k}^{2}
$$

as approximations of $\mathbb{E}\left[X\left(t_{n}\right)\right]$ and $\mathbb{E}\left[X^{2}\left(t_{n}\right)\right]$ respectively. From here, we calculate the errors in the first two moments of the method $M$ as

$$
\bar{\varepsilon}_{M}\left(t_{n}\right):=\left|\bar{X}_{n}-\mathbb{E}\left[X\left(t_{n}\right)\right]\right| ; \quad \overline{\varepsilon_{M}^{2}}\left(t_{n}\right):=\left|\overline{X_{n}^{2}}-\mathbb{E}\left[X\left(t_{n}\right)^{2}\right]\right| .
$$

We choose the first set of parameters, initial value and step size as

$$
\alpha=0.43, \quad \theta=0.06, \quad \sigma=0.15, \quad x_{0}=0.054, \quad \Delta=\frac{1}{8},
$$

where $\alpha, \theta$ and $\sigma$ are the maximum likelihood estimation parameters obtained in [19] and the starting point $x_{0}$ is chosen to be lesser than the long-term mean $\theta$. With the methods HM, E(0) M $\mathrm{M}_{0}$, MS, DI and HH we solve (1). The values of $\bar{X}_{n}$ and $\overline{X^{2}}{ }_{n}$, extended by interpolation, together with the exact values of $\mathbb{E}[X(t)]$ and $\mathbb{E}\left[X^{2}(t)\right]$ are represented on the left side of Fig. 4. The corresponding errors $\bar{\varepsilon}_{M}(t) \overline{\varepsilon_{M}^{2}}(t)$, in logarithmic scale, are shown in the right pictures of Fig. 4.

We choose the second set of parameters, initial value and step size to be

$$
\alpha=3, \quad \theta=1, \quad \sigma=1, \quad X_{0}=1.1, \quad \Delta=\frac{1}{4}
$$

in this case, the parameters are significantly larger than in the first set and with initial point greater than the long-term mean. With the methods (6), (7), $\mathrm{M}_{2}$, MS, DI and HH we solve (1). The graphical representation of the sample moments and the corresponding errors are shown in Fig. 5. Notice that the two sample moments of $\mathrm{E}(0)$ do not appear in the corresponding graphs, as they are outside the plot range.

For both (42) and (43), one can observe that only the modified Euler and our proposed $M_{0} M_{2}$ and MS methods revert to the long-term mean, in accordance with the theoretical results of Section 3. Furthermore, these three meanreverting schemes present similar errors. In addition, it can be seen that $E(0)$ overestimates the long-term mean, as we claimed in (10). On the other hand, the only scheme whose second moment converges to that of the CIR process is our proposed scheme MS. Notice also that for both experiments the sufficient condition of Remark 4.2 holds, concluding Sample Mean
![](https://cdn.mathpix.com/cropped/2023_09_28_e7fdb60d06f1ce22d705g-12.jpg?height=776&width=742&top_left_y=209&top_left_x=190)

![](https://cdn.mathpix.com/cropped/2023_09_28_e7fdb60d06f1ce22d705g-12.jpg?height=406&width=735&top_left_y=200&top_left_x=960)

Weak Quadratic Error

![](https://cdn.mathpix.com/cropped/2023_09_28_e7fdb60d06f1ce22d705g-12.jpg?height=330&width=705&top_left_y=664&top_left_x=985)
...
which is the choice we recommend when, the long-term variance is not of importance and $\Delta<1 / \alpha$ due to its simpler expression. If the second moment is relevant, we recommend using the scheme MS. And finally, we recommend the scheme $\mathrm{M}_{2}$ if the previous schemes cannot be applied because of step size requirements, as this scheme is less restrictive on the step size condition.
