...
\begin{document}
\title{MC 3: The Malliavin Derivative 1}
\postaliases{/2022-07-02-Malliavin-Calculus-3/}
\postsubtitle{Part 3 of the series on Malliavin calculus}
...
\begin{definition}\label{malnormdef}
...
\begin{corollary}\label{normedspace}
...
\begin{proof}
	...
	\begin{equation*}
		\|DX\|_{L^2(I \times \Omega)}=\sum_{n=1}^{\infty} n n!\|f_{n}\|_{L^2([0,T]^n)}.	\end{equation*}
...
\end{proposition}
\begin{proof}
It only remains to show that $\norm{\cdot }_{\mathbb{D}_{1,2}}$ is complete. Let $\set{X_m}_{m \in \N} \subset  \mathbb{D}_{1,2}$ be Cauchy. Or equivalently, by \Cref{normedspace}, $X_m$ is Cauchy in $L^2(\Omega)$ and $DX_m$ in $L^2(I \times \Omega)$.
...
\begin{proof}
    ...
    \end{equation*}
    The final equality holds due to the identical bound established in \eqref{in2} and the convergence of $X_n,\varphi_n$ to $X,\varphi$ respectively.
\end{proof}
