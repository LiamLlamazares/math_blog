...
\begin{definition}
	...
	\begin{equation*}
		\|X\|_{\mathbb{D}_{1,2}}^2 := \norm{X}_{L^2(\Omega)}  + \sum_{n=1}^{\infty} n n!\|f_n\|_{L^2([0,T]^n)}^2
	\end{equation*}
...
\begin{corollary}\label{normedspace}
	$(\mathbb{D}_{1,2},\|\cdot \|_{\mathbb{D}_{1,2}})$ is a normed space. Furthermore
	\begin{equation*}
		D X=0 \in L^2(I \times \Omega)\iff X\in \R.
	\end{equation*}
\end{corollary}
\begin{proof}
	The triangle inequality and the absolute homogeneity are direct consequences of the isometry of the previous proposition. This shows that $\|\cdot \|_{\mathbb{D}_{1,2}}$ is a norm. The second part follows from the fact that
	\begin{equation*}
...
\begin{proposition}
	The space $\mathbb{D}_{1,2}$ is a Banach space
	\begin{equation*}
