...
\end{itemize}
\section{Notation}
The same as in the other posts of this series. In particular, we recall the notation $\mathbb{L}^2(I\times\Omega)$ for the set of  \postref{Stochastic Calculus/Ito_integral}{prog def}{progressively measurable} square integrable stochastic processes. Furthermore, given a stochastic process $X$ such that  $X(t)\in \mathbb{D}^{1,2}$ for each $t\in I$ we write $D_rX(t)$ for the Malliavin differential at  time $r\in I$ of $X(t)$. That is, if
\begin{equation}\label{ce}
	X(t)=\sum_{n=0}^{\infty}  I_n(f_n(\cdot ,t)),\quad f_n(\cdot ,t)\in L^2(S_n).
\end{equation}
is the  \postref{Stochastic Calculus/Ito_integral}{Malliavin Calculus/1.Chaos_expansion}{chaos expansion} of $X(t)$ for each  $t$ then we have that
...
\end{lemma}
\begin{proof}
	Suppose that $D_tX$ is progressively measurable. Then, using the previously studied \postref{Malliavin Calculus/4.Malliavin_derivative_2}{divergence prop}{divergence property}    and the fact that the Skorokhod integral is an \postref{Malliavin Calculus/2.Skorohod}{itoskor}{extension} of the Itô integral  gives
	\begin{align*}
		D_r \int_{0}^t X(s) dW(s) & =D_r(\delta X1_{[0,t]}) =X(r)1_{[0,t]}(r)+\delta (D_rX1_{[0,t]}) \\&=X(r)+\int_{0}^t D_r(X(s)) dW(s).
	\end{align*}
	We consider the chaos expansion of $X$. Then, as was seen \postref{Malliavin Calculus/2.Skorohod}{zerolemma}{previously}, we have that
	\begin{equation*}
...
\begin{proof}
	...
	\end{align*}
	Where in the first inequality we applied Fubini, Cauchy Schwartz, and the triangle inequality, and in the equality, we used our \postref{Malliavin Calculus/3.Malliavin_derivative_1}{malnormdef}{old calculation}   of the norm of the Malliavin derivative
	\begin{equation*}
...
\end{equation*}
\begin{proposition}\label{maldersde}
	Given our SDE \eqref{SDE} with $\sigma ,b\in C^1_b(\R)$ it holds that there exists a unique solution $X_t$ and for all $r\leq t$ we have
	\begin{align*}
		D_rX(t)=\sigma(X_r)+\int_{r}^tb'(X(s))D_rX(s) ds+\int_{r}^t \sigma'(X(s))D_rX(s) dW(s) .
	\end{align*}
\end{proposition}
...
dg(Z) = g'dZ + \frac{1}{2}g''v^2 ds = e^Z\left[\left(u-\frac{1}{2}v^2+\frac{1}{2}v^2\right)ds + v dW(s)\right] = g(Z)(u ds + v dW(s))
	...
	\end{proof}
\end{theorem}
We end this post by noting that  \Cref{maldersde} has a multidimensional generalization which can also be found in Nualart's book \cite{nualart2018introduction}, on page 119.
