...
\begin{document}
	...
	\begin{proof}
		...
		\end{equation*}
		By taking expectations on both sides, and using that the Skorokhod integral is the adjoint (link) of the Malliavin derivative, and Fubini we obtain
		\begin{align*}
	...
	\end{theorem}
\begin{proof}
By Theorem \ref{dens exists} we know that $X$ has a density $p(x)$. To show that $p$ is smooth, by Sobolev embeddings it is sufficient to show that $p$ is weakly differentiable with order $n$ for all $n \in \N$. That is, for all $\varphi\in C_c^\infty(\R)$, there exists a function $p^{(n)}$ such that
\begin{equation*}
