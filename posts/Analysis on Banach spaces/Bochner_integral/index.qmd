...
[Download PDF version](The-Bochner-Integral.pdf)
:::
...
The dominated convergence theorem is a powerful tool that allows us to pass to the limit under the integral sign. For example, it can be used to show that, under necessary conditions, if $f(t,\omega )$ is continuous (differentiable) in $t$ then so is $\int f(t,\omega )\d\mu(\omega)$ (see [here](<../../PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces/index.qmd>) for a proof). These results, together with the density of simple functions, can be used to prove the [standard approximation theorems](<../../PDEs/3. Sobolev spaces/index.qmd>).
