...
\postsubtitle{Doob's Martingale inequality is cool}
...
\end{theorem}
\begin{proof}
	We have already proved the above inequality for simple processes. By our previous post on the \href{https://nowheredifferentiable.com/2022-05-27-The-Bochner-integral/}{Bochner integral}, we can take $X_n$ converging to $X$ in $L^1(\Omega\to E)$ to obtain that
	\begin{align*}
	...
	\end{equation*}
	Then using the linearity of the integral we obtain that $w(Z_1)=w(Z_2)$ for all linear function $w \in E'$, so $Z_1=Z_2$.
\end{proof}
\section{Martingales}
Okay, so we leveraged some inequalities to prove the existence of a conditional expectation. This done, the following definition mimicking the real case is quite natural. Given measurable spaces $(F,\Ff),(G,\Gg)$ we use the notation $f: \Ff \to \Gg$ to mean $f:(F,\Ff)\to (G,\Gg)$ is measurable 
\begin{definition}
...
\begin{lemma}[Norm: martingale $\to $ submartingale]\label{lem:norm_is_submart}
...
\begin{proof}
	...
	\end{align*}
\end{proof}
Let us recall the following result for real-valued martingales \cite[Theorem 3.2.6]{ccinlar2011probability}.
\begin{lemma}[Doob's maximal Martingale inequality]
	...
	\end{equation*}
\end{lemma}
The idea of the above result is that, since $X_k$ is a submartingale, $X_k\lesssim X_{k+1}\lesssim...\lesssim X_n$. Getting from the continuous to the discrete case is possible by using the continuity of $X$ and approximating it on some finer and finer mesh $t_0,...,t_n$. This said, applying Doob's maximal martingale inequality together with \Cref{lem:norm_is_submart} gives that
\begin{theorem}[Maximal Inequality]\label{thm:doobs_hilbert} Let $p>1$ and let $E$ be a separable Banach space.
...
\begin{definition}
	...
	\begin{equation*}
		\norm{M}_{\mathcal{M}_T^2(E)}:=\qty(\mathbb{E}[\norm{M(T)}^2])^ \frac{1}{2}
	\end{equation*}
\end{definition}
By \Cref{thm:doobs_hilbert} we have that
\begin{equation*}
...
\begin{proposition}
    Let $H$ be a separable Hilbert space and $Q$ be a symmetric, non-negative, trace-class operator on $H$. Let $W(t)$ be an $H$-valued $Q$-Wiener process with respect to a filtration $\mathcal{F}_t$. Then $W(t) \in \mathcal{M}_T^2(H)$.
\end{proposition}
\begin{proof}
    The process $W(t)$ is adapted by definition. Let $0 \leq s < t \leq T$. Given $A \in \mathcal{F}_s$ and $u \in H$, the mapping $x \mapsto \langle x, u \rangle$ is a bounded linear functional. Since $W(t) - W(s)$ is independent of $\mathcal{F}_s$, we apply the properties of the Bochner integral to obtain:
    \begin{align*}
        \br{ \int_A (W(t)-W(s)) d\mathbb{P}, u  }   &= \int_A \langle W(t)-W(s), u \rangle d\mathbb{P} \\
        &= \mathbb{E}[1_A] \mathbb{E}[\langle W(t)-W(s), u \rangle] = 0.
    \end{align*}
    Since this holds for all $u \in H$, the integral is the zero vector in $H$. By linearity,
    \begin{equation*}
        \int_A W(t) d\mathbb{P} = \int_A W(s) d\mathbb{P} \quad \forall A \in \mathcal{F}_s.
    \end{equation*}
    This satisfies the definition of conditional expectation, proving $\mathbb{E}[W(t) \mid \mathcal{F}_s] = W(s)$, so $W(t)$ is a martingale. Finally, by construction $\mathbb{E}[\|W(t)\|^2] = t \operatorname{Tr}(Q)<\infty$ as $Q$ is trace-class. Continuity is given by the definition of the Wiener process. This concludes the proof.
\end{proof}
