...
\begin{CJK*}{UTF8}{gbsn}
	...
	\end{theorem}
	\begin{proof}
		We have already proved in Lemma 1 that $\phi$ is positive definite if and only if it $\check{\phi}$ is multiplicatively positive. Which by the discussion is equivalent to $\check{\phi}$ being positive. As  a result, we obtain by \Cref{positivegives measure} that there exists a tempered measure $\nu $ with
		\begin{equation*}
