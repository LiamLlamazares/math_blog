...
\\
In this series of posts we'll show the relationship between stationary fields and Fourier analysis through Bochner's theorem and the spectral theorem. Furthermore we'll show how differential differential equations (both SDEs and SPDEs) with constant coefficients have as solutions stationary fields. This fact is very useful both for modeling and numerical purposes.Ok enough chit-chat, time for some definitions that we'll use throughout these posts.
...
\end{definition}
The above definition depends on the sigma-algebra $\Nn$ placed on $N$. In our work, $N$ will only ever be a topological space. In this case, we will implicitly assume that $\Nn=\Bb(N)$ is the Borel sigma-algebra on $N$.
\begin{definition}
...
\end{proposition}
\begin{proof}
	We have already seen that every covariance function is positive semi-definite, so it remains to check that a positive semi-definite function $r(u)$ corresponds to the  covariance function of some weakly stationary field. We can choose the process to be Gaussian. This follows by the fact that given any positive semi-definite Hermitian matrix $A$,  we can always find a Gaussian variable with covariance matrix $A$. This allows us to construct the desired family of finite dimensional distributions $\{X_j\}_{j \in  J}$, and then an application of \href{https://en.wikipedia.org/wiki/Kolmogorov_extension_theorem}{Kolmogorov's extension theorem} concludes the proof. See \cite{lindgren2012stationary} page 72 for the details.
\end{proof}
...
\\
In the case of \Cref{Fourier inversion} above it is customary to say that $S$ is the \emph{spectral density} of $X$. We observe that the condition for the existence of the spectral density is quite weak. Essentially, all that is required is that the relationship between the field at points that are far decays to $0$ at a fast enough rate. For completeness, we mention the following equivalent definition which may be found in some references
\begin{lemma}
...
\end{lemma}
\begin{proof}
	Let us denote the above integral by $I(f)$. First, we show that $I(f)$ is well-defined and continuous as a function of $f \in L^1(\R^d\to\C)$. By Bochner's theorem (\Cref{Bochner's theorem}) $r$ takes a maximum at  $0$ so
	\begin{equation*}
...
\end{corollary}
\begin{proof}
	We recall that, by the basic theory of Gaussian random variables, if $Y_n$ is a sequence of Gaussian random variables converging in distribution to $Y$ then $Y$ is also Gaussian. Furthermore, as we showed in \Cref{spectral theorem} (the spectral theorem) for each $f$,
...
\begin{proof}
	...
	\end{equation*}
	Thus, by the $1$-dimensional Bochner's theorem (\Cref{Bochner's theorem}) we can find some distribution function $dF_a$ such that
	...
	\end{equation*}
	Where $dF_{jk}$ is a complex linear combination of $dF_{a^{(jk)}},dF_{b^{(jk)}}, dF_{c^(j)},dF_{c^{(k)}}$ and is thus a \href{https://en.wikipedia.org/wiki/Complex_measure}{\emph{complex} valued measure}.
\end{proof}
...
\end{theorem}
\begin{proof}
	Let us fix $j\neq k\in J$ and consider the $\C^2$ valued field $\{(X_j(u),X_k(u))\}_{u\in\R^d}$. Then by applying \Cref{bochner 3d} (Bochner's multidimensional theorem) we obtain complex measures $\mu_{jk}$ verifying
	\begin{equation*}
...
\end{proof}
The analog of the spectral theorem (\Cref{spectral theorem}) is also immediate
...
\end{theorem}
\begin{proof}
	Since $X_j:=\br{X,e_j}$ are stationary we deduce from the spectral theorem for $\C$ valued stationary fields (\Cref{spectral theorem}) that there exist $dZ_j$ verifying the properties of said theorem for $X_j$. Let us set
	\begin{equation*}
	...
	\end{equation*}
	Where $\nu$ is well-defined since, as was observed at the end of \Cref{bochner 3d}, $\mu_{jj}$ are real measures (as opposed to complex).
\end{proof}
...
\begin{proof}
	...
	\end{multline*}
	Where in the above we used the stochastic Fubini theorem (\Cref{stochastic Fbini}). The second part of the proposition follows from Fourier inversion. We have that $\hat{\check{g}}=g$ so
	\begin{equation*}
...
\begin{theorem}[Differentiation of fields]\label{diff fields theorem}
	...
	\begin{enumerate}
		\item $X$ is $\alpha$ times mean square differentiable.\label{p1}
...
\end{theorem}
\begin{proof}
	To prove the equivalence between the points, it is sufficient to prove it when $\abs{\alpha}=1$ and then apply an iteration. Let us take then\\ $\alpha=(0,\ldots,\overset{(i)}{1},\ldots 0)$. We have that, by the Loeve condition for $L^2$ convergence, the derivative exists if and only if the limit
	\begin{multline*}
	...
	\end{multline*}
	Where the first equality was already shown to be implied by \Cref{p1} and the inequality is due to Fatou's lemma. The fact that $4$ implies $2$, and thus also $1$, is proved similarly by the dominated convergence theorem.  We now prove the equivalence of $4$ and $5$. Suppose that  $4$ holds. Then we have  that, by the dominated convergence theorem
	\begin{equation*}
...
\end{observation}
The just proved \Cref{diff fields theorem} shows that, if the derivative of a stationary field exists, it is indeed given as the output of a linear filter $\Ff_g$ where in this case $g(\omega)=(i\omega)^\alpha$. In particular, we have that by the previous results on linear filters, the derivative of a stationary field is itself stationary. Furthermore, as a simple consequence, we obtain the following corollary,
...
\begin{proof}
	...
	\
	Finally, the formula \eqref{spectrd ODE} for the spectral density of $X$ follows from \Cref{density of derivative}, where we calculated the spectrum of the derivative of a field.\\
\end{proof}
...
\end{equation*}
Thus, we see that in this case, $P$ as in \Cref{spde theorem} is a polynomial. We will call $P$ the polynomial associated to the operator $\mathcal{L}$.
...
By \Cref{spde theorem}, the operator associated is
...
\begin{equation}\label{omk}
	\omega_k:=\frac{2\pi k}{hn},\quad k\in \Kk_0:=\qty[\frac{n}{2},\frac{n}{2}-1]\cap \Z^d
\end{equation}
By \Cref{spde theorem} and Bochner's theorem, we know that, for $i\in \{M,S,T\}$
\begin{equation*}
...
\\
Finally, we wish to simulate from the three fields $X_M,X_S,X_T$. To do so, we will use the spectral process of \Cref{spectral theorem} (\red{Make sure your field has mean zero otherwise see Observation \ref{obs 1}}). Let us write $X$ for any one of these fields we have that, by this theorem
...
\end{equation*}
Where $x_k,y_k$ are iid unit, real-valued normal random variables for $k \in \Kk_+$.
\\
...
\begin{multline}\label{sr+}
	X(u)\simeq\sum_{k\in \Kk_0} e^{i\omega_k\cdot u}Z_k=\sum_{k\in \Kk_0}\sigma _k(cos(\omega_k\cdot u) + i sin(\omega_k\cdot u))(x_k +iy_k)\\
	=\sum_{k\in \Kk_0}\sigma _k(x_{k}cos(\omega_k\cdot u)-y_{k}sin(\omega_k\cdot u))
\end{multline}
Using this together with the independence of $x_k,y_k\sim\mathcal{N}(0,1)$ gives that
\begin{multline}\label{eq1}
	r(u-v)=Cov(X(u),X(v))\\\simeq \sum_{k\in \Kk_0} \sigma_k^2(cos(\omega_k\cdot u)cos(\omega_k\cdot v)+sin(\omega_k\cdot u)sin(\omega_k\cdot v))\\
	=\sum_{k\in \Kk_0} \sigma _k^2 cos(\omega_k\cdot(u-v)).
\end{multline}
...
\begin{multline}\label{eq2}
	...
	\simeq \sum_{k \in \Kk_0}\abs{\frac{2\pi}{hn}}\tilde{S}_h(\omega_k)cos(\omega_k\cdot(u-v)).
\end{multline}
...
\begin{proof}
	...
	\begin{equation*}
		f_n(\omega):= \rm{argmin}_{i\in\{1,...,n\}} \{d(f(\omega),e_i)\}
	\end{equation*}
...
\end{equation*}
We call this the \emph{covariance kernel} associated with $\mu $. By setting $S=V'$ we may apply the previous \Cref{existence reproducing kernel}
...
\end{proposition}
\begin{proof}
	This follows from the fact that in complete normed spaces compactness is equivalent to total boundedness. So if we cover $T_N(A)$ by $B(y_n,\varepsilon/2)$ then for large $N$ and $x\in A$
	\begin{equation*}
		\norm{T(x)-y_n}\leq \norm{T-T_N}\norm{x}-\norm{T_N(x)-y_n}<\varepsilon
	\end{equation*}
\end{proof}
\begin{proposition}
...
\begin{proof}
	...
	As a result we may apply the Hilbert Schmidt \Cref{Hilber-Schmidt} to obtain that
	\begin{equation*}
