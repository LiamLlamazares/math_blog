...
\begin{theorem}\label{dual of integer sobolev}
    ...
    \end{align*}
\end{theorem}
The first equality will be discussed in the next subsection and is most easily proved when $\Omega =\R^d$, in which case one can use the homeomorphism $\Lambda ^s: H^{r,p}(\R^d )\iso H^{r-s,p}(\R^d )$ together with the reflexivity of $L^p(\R^d )$. The second equality is a direct consequence of the integer order equality $W^{k,p}(\Omega )=H^{k,p}(\Omega )$ of Theorem \ref{equivalence fractional spaces}. For fractional order regularities, we have the following result, which can be found in \cite{agranovich2015sobolev} page 228.
\begin{theorem}
...
\end{align*}
where the second equality is by Exercise \ref{dual exercise}. This coincides well with what we observed in \eqref{ismorphism} and justifies an extension of \eqref{ismorphism} for negative $s$.
...
\begin{hint}
    ...
    \end{align*}
    The result follows from Theorem \ref{subcritical embedding},  Theorem \ref{morrey embedding} together with the \href{https://nowheredifferentiable.com/2023-07-12-PDEs-3-Sobolev_spaces/#est32:~:text=Combining%20the%20three%20results}{integer-case} embeddings. For the first case, by Theorem \ref{subcritical embedding}, we have
    \begin{align*}
