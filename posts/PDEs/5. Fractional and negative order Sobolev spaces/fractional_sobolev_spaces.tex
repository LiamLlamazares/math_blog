...
\begin{document}
\title{PDEs 5: Fractional Sobolev spaces}
\postaliases{/2024-02-27-PDEs-5-Fractional_Sobolev_spaces/}
\postsubtitle{Filling in the gaps}
...
In previous posts, we covered the theory of \postref{PDEs/3. Sobolev spaces}{}{Sobolev spaces} $W^{k,p}(\Omega )$ where $k$ is an integer. In the case $k=2$ and when $\Omega =\R^d$ we saw that this space coincided with $H^k(\R^d)$. Furthermore, \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{fracsobdef}{we also saw} how to define $H^s(\R^d)$ when $s$ was any real number. This motivates the following two questions.
\begin{enumerate}
...
We make frequent use of the fact that, as \postref{PDEs/3. Sobolev spaces}{identificationfuncdist}{shown} in a previous post, functions in $L^p(U)$ can be identified as elements of the larger space of \emph{distributions} $\Dd '(U ):= C_c^\infty(U)'$. This is done by identifying a function $f \in L^p(U)$ with the linear functional 
\begin{align*}
    T_f:\phi \mapsto \int_U f\phi \d x.
\end{align*}
This identification between $f$ and $T_f$  allows us to extend by \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{distrsec}{duality} operators that are defined on $C_c^\infty(U)$ to $L^p(U)$. For example, given $u \in L^p(U) \in \Dd'(\Omega )$ we can define its Fourier transform $\Ff u$  and  $\alpha$-th derivative $D^\alpha u$  to be the distributions defined by
\begin{align*}
...
\end{definition}
\begin{observation}
    In the case where $U= \R^d$, by a change of variables, we can equivalently define    \begin{align}\label{norm def}
...
\begin{proposition}[Inclusion ordered by regularity]\label{inclusion ordered by regularity}
    Let  $\Omega$ be an \postref{PDEs/3. Sobolev spaces}{extension domain}{extension domain} for $W^{1,p}$.  Then, for $p \in [1,\infty)$ and $0<s<s'$ it holds that
    \begin{align*}
...
\begin{definition}
    ...
    \end{align*}
\end{definition}
In the definition above, we used the notation $\br{\xi}:=\sqrt{1+|\xi|^2}$. As we saw when we studied Sobolev spaces through the \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{sobolevcondition}{Fourier transform}, using the fact that $\Ff $ is an isometry which transforms differentiation into polynomial multiplication
...
\end{exercise}
\begin{hint}
    Use definition \ref{besov def negative} and that $\Lambda^r$ has inverse $\Lambda^{-r}$.
\end{hint}
...
\begin{theorem}
    Let $\Omega \subset \R^d$ be open with \postref{PDEs/3. Sobolev spaces}{uniformly Lipschitz domain}{uniformly Lipschitz boundary} and consider   $p\in [1,\infty), s\in[1,\infty)$. Then, $\Omega $ is an extension domain for $W^{s,p},H^{s,p},B^{s,p}$.
\end{theorem}
...
\begin{definition}[Slobodeckij space negative $s$]\label{negative s Slobodeckij}
    ...
    \end{align*}
\end{definition}
The equality for $p=2$ in \eqref{p=2} justifies that, for sufficiently regular domains, all three spaces are written $H^s(\Omega )$.   We will prove the left-hand side of this equivalence in \Cref{equivalence of fractional spaces}. For $p\neq 2$, the inclusions are, in general, strict. An example is constructed in \cite{stein1970singular} page 161 exercise 6.8.
\begin{exercise}[Equivalence of fractional spaces]\label{equivalence of fractional spaces}
    Show without using \Cref{equivalence fractional spaces} that
    \begin{align*}
        H^{s,2}(\R^d)=W^{s,2}(\R^d).
    \end{align*}
\end{exercise}
...
\end{hint}
\begin{exercise}
    Use the previous \Cref{equivalence of fractional spaces} to show that if $\Omega $ is an extension domain for $H^{s}$, then
    \begin{align*}
...
\begin{definition}
    ...
    \end{align*}
\end{definition}
\begin{proposition}\label{fractional laplacian}
...
\begin{proof}
    ...
    \end{align*}
    Using \eqref{second order} and proceeding as in \Cref{equivalence fractional spaces} gives
    \begin{align*}
    ...
    \end{align*}
\end{proof}
\section{Dual of Sobolev spaces and correspondence with negative regularity}\label{dual sec}
...
\begin{theorem}\label{dual of integer sobolev}
    ...
    \end{align*}
\end{theorem}
The first equality will be discussed in the next subsection and is most easily proved when $\Omega =\R^d$, in which case one can use the homeomorphism $\Lambda ^s: H^{r,p}(\R^d )\iso H^{r-s,p}(\R^d )$ together with the reflexivity of $L^p(\R^d )$. The second equality is a direct consequence of the integer order equality $W^{k,p}(\Omega )=H^{k,p}(\Omega )$ of \Cref{equivalence fractional spaces}. For fractional order regularities, we have the following result, which can be found in \cite{agranovich2015sobolev} page 228.
\begin{theorem}
...
\begin{observation}
    ...
    \end{align}
    See, for example, \cite{biccari2018local}, \cite{leoni2023first} page 228. The definition in \eqref{alternative negative} is equivalent to Definition \ref{negative s Slobodeckij} when $\Omega = \R^d$ or when $s=k \in \Z$. However, in other cases, the two definitions are not equivalent.
\end{observation}



\subsection{The dual of $H^{s,p}(\R^d)$}
...
\end{align*}
the support being understood \postref{PDEs/3. Sobolev spaces}{support sec}{in the sense of distributions}
...
\end{align}
Applying this to $Y= H^{k,p}_0(\Omega )\hookrightarrow H^{k,p}(\R^d) =X$ we obtain the result of \Cref{dual of integer sobolev}.
\begin{align*}
    H^{k,p}_0(\Omega )' \simeq H^{k,p}(\R^d)'\backslash H^{k,p}(\Omega)^\circ\simeq H^{-k,p'}(\R^d)\backslash H^{-k,p'}_{\Omega^c}(\R^d )
\end{align*}
where the second equality is by \Cref{dual exercise}. This coincides well with what we observed in \eqref{ismorphism} and justifies an extension of \eqref{ismorphism} for negative $s$.
...
\end{exercise}
\begin{hint}
    Use that $\Lambda ^{\gamma }: H^{s,p}(\R^d) \to H^{k ,p}(\R^d)$ is an isomorphism and the just proved \Cref{riesz representation} together with the integer equivalence in \Cref{equivalence fractional spaces} to show that
    \begin{align*}
...
Here, the \postref{PDEs/3. Sobolev spaces}{est12}{analogous} of the exponent $p_k^{*}$ is
\begin{definition}
...
\end{theorem}
\begin{exercise}
    Using \Cref{subcritical embedding} prove \Cref{critical embedding}
\end{exercise}
\begin{hint}
    We are in the subcritical case for $r<d/p=\gamma$. Extend $u$ to $\R^d$ to form $\tl{u}$. Then, by \Cref{inclusion ordered by regularity}, we have
    \begin{align*}
...
The supercritical case $\gamma>d/p$ can be found in \cite{agranovich2015sobolev} page 224 and \cite{leoni2023first} page 275. To simplify the notation we use the convention that $C^r$ stands for $C^{\lfloor r \rfloor, r-\lfloor r \rfloor}$.
...
\end{theorem}
\begin{exercise}
    Justify via a scaling argument that the condition $\gamma_2 - \frac{d}{p_2} = \gamma_1 - \frac{d}{p_1}$ is necessary for the embedding in \Cref{higher smoothness embedding}.
\end{exercise}
\begin{hint}
...
\end{theorem}
The above results can also be formulated in terms of the Sobolev seminorm. For example, \Cref{interpolation embedding} can be formulated as
\begin{align*}
...
\begin{exercise}[Fractional Sobolev Embeddings]\label{fractional_sobolev}
    ...
    \begin{align*}
         ...
         & W^{s, p}(\Omega) \hookrightarrow C^{m, \alpha}(\bar{\Omega}) \hspace{65pt} \text{ if } s>\frac{d}{p} \text{ and } s-\frac{d}{p} \notin \mathbb{N},
    \end{align*}
...
\begin{hint}
    ...
    \end{align*}
    The result follows from applying \Cref{subcritical embedding} and \Cref{morrey embedding} sequentially with the \postref{PDEs/3. Sobolev spaces}{rellich}{integer-case} embeddings. For the first (the subcritical) case, the fractional embedding \Cref{subcritical embedding} yields:
    \begin{align*}
...
\end{exercise}
\begin{hint}
    Set $k_i =\left\lfloor s_i \right\rfloor$ and $\gamma_i =s_i-k_i$.  Apply \Cref{higher smoothness embedding} to the derivatives of order up to $k_2$ to obtain
    \begin{align*}
