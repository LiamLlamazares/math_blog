...
[Download PDF version](fractional_sobolev_spaces.pdf)
:::
...
& W^{s, p}(\Omega) \hookrightarrow C^{s-d/p}(\bar{\Omega})\text { and } s>\frac{d}{p},
\end{aligned}
$$
...
The result follows from @thm-subcritical-embedding,  @thm-morrey-embedding together with the [integer-case](</posts/PDEs/3. Sobolev spaces/index.qmd>) embeddings. For the first case, by @thm-subcritical-embedding, we have
...
![Sobolev critical exponent for $p=2$ and $d=1,2,3$](regularity_coefficient.png) {#fig-fig:regularity-coefficient}
