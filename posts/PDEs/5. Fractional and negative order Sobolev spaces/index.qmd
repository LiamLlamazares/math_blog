...
In previous posts, we covered the theory of [Sobolev spaces](<../../PDEs/3. Sobolev spaces/index.qmd>) $W^{k,p}(\Omega )$ where $k$ is an integer. In the case $k=2$ and when $\Omega =\R^d$ we saw that this space coincided with $H^k(\R^d)$. Furthermore, [we also saw](<../../PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces/index.qmd>) how to define $H^s(\R^d)$ when $s$ was any real number. This motivates the following two questions.
...
We make frequent use of the fact that, as [shown](<../../PDEs/3. Sobolev spaces/index.qmd>) in a previous post, functions in $L^p(U)$ can be identified as elements of the larger space of *distributions* $\Dd '(U ):= C_c^\infty(U)'$. This is done by identifying a function $f \in L^p(U)$ with the linear functional 
...
This identification between $f$ and $T_f$  allows us to extend by [duality](<../../PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces/index.qmd>) operators that are defined on $C_c^\infty(U)$ to $L^p(U)$. For example, given $u \in L^p(U) \in \Dd'(\Omega )$ we can define its Fourier transform $\Ff u$  and  $\alpha$-th derivative $D^\alpha u$  to be the distributions defined by
...
Let  $\Omega$ be an [extension domain](<../../PDEs/3. Sobolev spaces/index.qmd>) for $W^{1,p}$.  Then, for $p \in [1,\infty)$ and $0<s<s'$ it holds that
...
In the definition above, we used the notation $\br{\xi}:=\sqrt{1+|\xi|^2}$. As we saw when we studied Sobolev spaces through the [Fourier transform](<../../PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces/index.qmd>), using the fact that $\Ff$ is an isometry which transforms differentiation into polynomial multiplication
...
Let $\Omega \subset \R^d$ be open with [uniformly Lipschitz boundary](<../../PDEs/3. Sobolev spaces/index.qmd>) and consider   $p\in [1,\infty), s\in[1,\infty)$. Then, $\Omega$ is an extension domain for $W^{s,p},H^{s,p},B^{s,p}$.
...
the support being understood [in the sense of distributions](<../../PDEs/3. Sobolev spaces/index.qmd>)
...
Here, the [analogous](<../../PDEs/3. Sobolev spaces/index.qmd>) of the exponent $p_k^{*}$ is
...
The result follows from @thm-subcritical-embedding,  @thm-morrey-embedding together with the [integer-case](<../../PDEs/3. Sobolev spaces/index.qmd>) embeddings. For the first case, by @thm-subcritical-embedding, we have
