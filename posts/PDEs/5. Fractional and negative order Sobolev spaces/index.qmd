...
$$
\begin{aligned}
\norm{u}_{W^{s,p}(\R^d)}:= \left(\norm{\R^d}_{W^{k,p}(\R^d)}^p+ \sum_{\abs{\alpha}=k }\int_{\R^d}\int_{\R^d}\frac{\abs{D^\alpha u(x+y)-D^\alpha u(y)}^p}{\abs{x-y}^{d+\gamma p}}\d x \d y\right)^\frac{1}{p}.
\end{aligned}
$$ {#eq-norm-def}
...
The result follows from @thm-subcritical-embedding,  @thm-morrey-embedding together with the [integer-case](<https://nowheredifferentiable.com/2023-07-12-PDEs-3-Sobolev_spaces/#est32:~:text=Combining%20the%20three%20results>) embeddings. For the first case, by @thm-subcritical-embedding, we have
...
For the second case, by @thm-subcritical-embedding and reasoning with derivatives up to order $k$, we obtain similarly
...
where by a calculation $k = d / p_\gamma ^*$. So, we conclude once more by using the integer case. Likewise, for the final case, we use @thm-morrey-embedding to directly obtain
...
$$
\begin{aligned}
%         \Ww (\omega ) \in H^{-d/2 -\varepsilon}(\R^d), \quad \text{for all } \varepsilon >0.
%
\end{aligned}
$$
...
$$
\begin{aligned}
%     W(\cdot , \omega ) \in H^{d/2 -\varepsilon}(\R^d), \quad \text{for all } \varepsilon >0 \in H^{1 -\varepsilon}(\R^d).
%
\end{aligned}
$$
...
% verifies that $W(t, \omega ) \in H^{1/2 -\varepsilon}(\R)$ for all $\varepsilon >0$ and almost every $\omega \in \Omega$.
