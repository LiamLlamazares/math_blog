...
aliases:
  - /2024-02-27-PDEs-5-Fractional_Sobolev_spaces/
bibliography: biblio.bib
pdf: "fractional_sobolev_spaces.pdf"
resources:
  - "fractional_sobolev_spaces.pdf"
format:
  html:
    toc: true
...
<div id="quarto-post-pdf-url" data-url="fractional_sobolev_spaces.pdf" style="display:none;"></div>

\postaliases{/2024-02-27-PDEs-5-Fractional_Sobolev_spaces/}
...
In previous posts, we covered the theory of [Sobolev spaces](</posts/PDEs/3. Sobolev spaces/index.qmd>) $W^{k,p}(\Omega )$ where $k$ is an integer. In the case $k=2$ and when $\Omega =\R^d$ we saw that this space coincided with $H^k(\R^d)$. Furthermore, [we also saw](</posts/PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces/index.qmd>) how to define $H^s(\R^d)$ when $s$ was any real number. This motivates the following two questions.
...
We make frequent use of the fact that, as [shown](</posts/PDEs/3. Sobolev spaces/index.qmd>) in a previous post, functions in $L^p(U)$ can be identified as elements of the larger space of *distributions* $\Dd '(U ):= C_c^\infty(U)'$. This is done by identifying a function $f \in L^p(U)$ with the linear functional 
...
This identification between $f$ and $T_f$  allows us to extend by [duality](</posts/PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces/index.qmd>) operators that are defined on $C_c^\infty(U)$ to $L^p(U)$. For example, given $u \in L^p(U) \in \Dd'(\Omega )$ we can define its Fourier transform $\Ff u$  and  $\alpha$-th derivative $D^\alpha u$  to be the distributions defined by
...
$$
\begin{aligned}
\norm{u}_{W^{s,p}(U)}:= \left(\norm{u}_{W^{k,p}(U)}^p+ \sum_{\abs{\alpha}=k }\int_{U}\int_{U}\frac{\abs{D^\alpha u(x)-D^\alpha u(y)}^p}{\abs{x-y}^{d+\gamma p}}\d x \d y\right)^\frac{1}{p}.
\end{aligned}
$$ {#eq-norm-def}
...
In the case where $U= \R^d$, by a change of variables, we can equivalently define    
$$
\begin{aligned}
...
$$
\begin{aligned}
f_u(x,y):=\frac{D^\alpha u(x)-D^\alpha u(y)}{\abs{x-y}^{\frac{d}{p}+\gamma}}.
\end{aligned}
$$
...
Let  $\Omega$ be an [extension domain](</posts/PDEs/3. Sobolev spaces/index.qmd>) for $W^{1,p}$.  Then, for $p \in [1,\infty)$ and $0<s<s'$ it holds that
...
In the definition above, we used the notation $\br{\xi}:=\sqrt{1+|\xi|^2}$. As we saw when we studied Sobolev spaces through the [Fourier transform](</posts/PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces/index.qmd>), using the fact that $\Ff$ is an isometry which transforms differentiation into polynomial multiplication
...
$$
\begin{aligned}
\norm{u}_{B^{s,p}(U)}:= \left(\norm{u}_{W^{k_{-},p}(U)}^p+ \sum_{\abs{\alpha}=k_- }\int_{U}\int_{U}\frac{\abs{D^\alpha u(x)-D^\alpha u(y)}^p}{\abs{x-y}^{d+\gamma p}}\d x \d y\right)^\frac{1}{p}.
\end{aligned}
$$
...
Let $\Omega \subset \R^d$ be open with [uniformly Lipschitz boundary](</posts/PDEs/3. Sobolev spaces/index.qmd>) and consider   $p\in [1,\infty), s\in[1,\infty)$. Then, $\Omega$ is an extension domain for $W^{s,p},H^{s,p},B^{s,p}$.
...
$$
\begin{aligned}
s=s_1(1-\theta)+s_2 \theta, \quad \frac{1}{p}=\frac{1-\theta}{p_1}+\frac{\theta}{p_2}.
\end{aligned}
$$
...
Let $s \geq 0, \varepsilon >0$ and  $\Omega$ an extension domain for $H^{s+\varepsilon,p},B^{s+\varepsilon,p}$. Then,
...
The equality for $p=2$ in ([-@eq-p-2]) justifies that, for sufficiently regular domains, all three spaces are written $H^s(\Omega )$.   We will prove the left-hand side of this equivalence in @exr-equivalence-of-fractional-spaces.  For $p\neq 2$, the inclusions are, in general, strict. An example is constructed in [@stein1970singular] page 161 exercise 6.8.

::: {#exr-equivalence-of-fractional-spaces name="Equivalence of fractional spaces"}

Show without using @thm-equivalence-fractional-spaces that
...
Use the previous @exr-equivalence-of-fractional-spaces to show that if $\Omega$ is an extension domain for $H^{s}$, then
...
Using ([-@eq-second-order]) and proceeding as in @thm-equivalence-fractional-spaces gives

$$
\begin{aligned}
& \Ff \left(\int_{\R^d}\frac{u(x)-u(x+y)}{\abs{y}^{d+2\gamma}}\d y\right)(\xi)= -\frac{1}{2} \left(\int_{\R^d}\frac{e^{2\pi i y \cdot \xi}-2+e^{-2\pi i y \cdot \xi}}{\abs{y}^{d+2\gamma}}\d y\right) \wh{u}(\xi)       \\
& =\left(\int_{\R^d}\frac{1-\cos(2\pi y \cdot \xi)}{\abs{y}^{d+2\gamma}}\d y\right) \wh{u}(\xi) =\int_{\R^d}  \frac{1-\cos(2\pi  y_1) }{\abs{y}^{d+2\gamma	}}\d y\abs{\xi}^{2 \gamma }\wh{u}(\xi) \\& \sim \abs{\xi}^{2 \gamma }\wh{u}(\xi).
\end{aligned}
$$
...
## Dual of Sobolev spaces and correspondence with negative regularity {#sec-dual-sec}
...
The first equality will be discussed in the next subsection and is most easily proved when $\Omega =\R^d$, in which case one can use the homeomorphism $\Lambda ^s: H^{r,p}(\R^d )\iso H^{r-s,p}(\R^d )$ together with the reflexivity of $L^p(\R^d )$. The second equality is a direct consequence of the integer order equality $W^{k,p}(\Omega )=H^{k,p}(\Omega )$ of @thm-equivalence-fractional-spaces.  For fractional order regularities, we have the following result, which can be found in [@agranovich2015sobolev] page 228.
...
See, for example, [@biccari2018local], [@leoni2023first] page 228. The definition in ([-@eq-alternative-negative]) is equivalent to Definition @def-negative-s-slobodeckij when $\Omega = \R^d$ or when $s=k \in \Z$. However, in other cases, the two definitions are not equivalent.
...
the support being understood [in the sense of distributions](</posts/PDEs/3. Sobolev spaces/index.qmd>)
...
Applying this to $Y= H^{k,p}_0(\Omega )\hookrightarrow H^{k,p}(\R^d) =X$ we obtain the result of @thm-dual-of-integer-sobolev. 
...
where the second equality is by @exr-dual-exercise.  This coincides well with what we observed in ([-@eq-ismorphism]) and justifies an extension of ([-@eq-ismorphism]) for negative $s$.
...
Let $\Omega \subset \R^d$ be an extension domain for $W^{k,p}$ where   $k \in \N_0$ and $p \in (1,\infty)$. Then, every element in $W^{-k,p'}(\Omega )=W^{k,p}_0(\Omega )'$ is the unique extension of a distribution  of the form

$$
\begin{aligned}
\sum_{0\leq\abs{\alpha}\leq k} D^\alpha u_\alpha\in \Dd'(\Omega ),\quad \text{where }    u_\alpha \in L^{p'}(\Omega ).
\end{aligned}
$$
...
$$
\begin{aligned}
T: W^{k,p}(\Omega ) & \longrightarrow L^p(\Omega \to \R^M)                \\
u                   & \longmapsto(D^\alpha u)_{0 \leq\abs{\alpha}\leq k},
\end{aligned}
$$

where $M$ is the number of multi-indices bounded by $k$.
...
$$
\begin{aligned}
\ell_1(\mathbf{w})=\int_{\Omega}\mathbf{w}\cdot \mathbf{f}, \quad \forall \mathbf{w} \in L^p(\Omega \to \R^M).
\end{aligned}
$$
...
$$
\begin{aligned}
\ell(v)=\ell_0(Tv)=\int_{\Omega}Tv\cdot \mathbf{f}=\sum_{0\leq\abs{\alpha}\leq k}\int_{\Omega}f_\alpha D^\alpha v .
\end{aligned}
$$
...
$$
\begin{aligned}
\ell(v)=\left(v,\sum_{0\leq\abs{\alpha}\leq k} D^\alpha u_\alpha\right)=: \omega(v)
\end{aligned}
$$ {#eq-representation}
...
Use that $\Lambda ^{\gamma }: H^{s,p}(\R^d) \to H^{k ,p}(\R^d)$ is an isomorphism and the just proved @thm-riesz-representation together with the integer equivalence in @thm-equivalence-fractional-spaces to show that
...
Then, it is necessary to know exactly what boundary data $g$ is admissible. Suppose that $\Ll$ is of order $k$ so we require  $u \in W^{k,p}(\Omega )$. Will ([-@eq-pde]) have a solution? To be able to answer this question, we need to know the image of the trace operator. If $g \notin \operatorname{Tr}(W^{k,p}(\Omega ))$, then there is no hope of finding a solution. The following theorem characterizes the image of the trace operator and can be found in [@agranovich2015sobolev] page 228 and [@leoni2023first, page 350, 492].

::: {#thm-trace-theorem name="Fractional trace theorem"}

Let $\Omega \subset \R^d$ be open with $C^{k,1}$ boundary. Then, for all  $p\in (1,\infty), s\in (1/p,k)$, the trace operator $\operatorname{Tr}$ can be extended from $C(\overline{\Omega } )$  to a bounded operator
...
Here, the [analogous](</posts/PDEs/3. Sobolev spaces/index.qmd>) of the exponent $p_k^{*}$ is
...
Using @thm-subcritical-embedding prove @thm-critical-embedding

:::

::: {.callout-tip collapse="true" title="Hint"}

We are in the subcritical case for $r<d/p=\gamma$. Extend $u$ to $\R^d$ to form $\tl{u}$. Then, by @prp-inclusion-ordered-by-regularity, we have
...
Justify via a scaling argument that the condition $\gamma_2 - \frac{d}{p_2} = \gamma_1 - \frac{d}{p_1}$ is necessary for the embedding in @thm-higher-smoothness-embedding. 
...
The above results can also be formulated in terms of the Sobolev seminorm. For example, @thm-interpolation-embedding can be formulated as
...
::: {#exr-fractional-sobolev name="Fractional Sobolev Embeddings"}

Let $\Omega \subset \mathbb{R}^d$ be an extension domain for $W^{s,p}$. Given $s \ge 0$ and $p \in [1, \infty)$, set $k=\lfloor s \rfloor$ and $\gamma =s-k$. The following continuous embeddings hold:

$$
\begin{aligned}
& W^{s, p}(\Omega) \hookrightarrow L^q(\Omega), \quad \forall q \in [p, p_s^*] \quad \text{ if } s<\frac{d}{p} \\
& W^{s, p}(\Omega) \hookrightarrow L^q(\Omega), \quad \forall q \in [p, \infty) \quad \text{ if } s=\frac{d}{p} \\
& W^{s, p}(\Omega) \hookrightarrow C^{m, \alpha}(\bar{\Omega})  \text{ if } s>\frac{d}{p} \text{ and } s-\frac{d}{p} \notin \mathbb{N},
\end{aligned}
$$
...
The result follows from applying @thm-subcritical-embedding and @thm-morrey-embedding sequentially with the [integer-case](</posts/PDEs/3. Sobolev spaces/index.qmd>) embeddings. For the first (the subcritical) case, the fractional embedding @thm-subcritical-embedding yields:
...
![Sobolev critical exponent for $p=2$ and $d=1,2,3$](regularity_coefficient.svg){#fig-fig:regularity-coefficient fig-align="center" width="80%"}
...
Set $k_i =\left\lfloor s_i \right\rfloor$ and $\gamma_i =s_i-k_i$.  Apply @thm-higher-smoothness-embedding to the derivatives of order up to $k_2$ to obtain
