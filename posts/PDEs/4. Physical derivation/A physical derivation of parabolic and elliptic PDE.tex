...
\title{PDES 4: A physical derivation of parabolic and elliptic PDE}
\postsubtitle{Just have to balance the mass}
...
\begin{example}
	...
	\begin{align}\label{Poisson}
		\Delta u=f.
	\end{align}
	If $u$ solves \eqref{Poisson}, then $u+p$ also solves \eqref{Poisson} where $p$ is any harmonic function (i.e., $\Delta p = 0$), such as a polynomial of degree 0 or 1.
\end{example}
...
\begin{enumerate}[a)]
	\item \emph{Dirichlet boundary condition}: This is the additional imposition that
	      ...
	      \end{align*}
	      where $g$ is some function defined on  $\partial \Omega$ and $u$ is restricted to $\partial\Omega$ through the trace theorem developed in \postref{PDEs/3. Sobolev spaces}{}{the previous post}. In the context of the diffusion of heat, $\Omega$ could be a rod which is kept at a constant temperature at its endpoints.
	...
	\item \emph{Neumann boundary condition:}This is a particular case of the Robin boundary condition where there is no advection flux at the boundary. In this case, \eqref{Robin}  becomes
	      \begin{align*}
