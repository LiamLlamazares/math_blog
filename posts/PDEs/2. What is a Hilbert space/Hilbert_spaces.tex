...
\begin{document}
	\title{PDEs 2: What is a Hilbert space?}
	\postaliases{/2023-05-30-PDE-2-Hilbert/}
\postsubtitle{An appendix for a series on PDEs}
	...
	In particular, this implies that the inner product is continuous jointly, and in each component. An inner product space keeps the key properties of Euclidean space, namely the vector space structure and inner product. The third key property is that of completeness.\begin{definition}
		...
		\end{align*}
	\end{definition}
	Cool! Now we know what is a Hilbert space. Some examples that have come up in previous posts are the space of \postref{Analysis on Banach spaces/Bochner_integral}{}{square-integrable functions} valued in  another Hilbert space $H$ with the inner product
	\begin{align*}
		\br{f,g}_{L^2(\Omega \to H)}:= \int_{\Omega} \br{f,g} d\mu .
	\end{align*}
	Another are the \postref{Analysis on Banach spaces/Bochner_integral}{}{Sobolev spaces} $H^s(\R^d)$ with the inner product
	\begin{align*}
	...
	\begin{corollary}\label{cor}
		...
		\end{align*}
	\end{corollary}
	Note that the imposition that $V$ is closed in \Cref{complement theorem} is necessary. Finite-dimensional subspaces are always closed, but infinite-dimensional subspaces may not be. For example, consider $H=L^2(I)$ for some bounded $I$ and take  $V$ to polynomials on $I$ . By the \href{https://en.wikipedia.org/wiki/Stone%E2%80%93Weierstrass_theorem}{Stone-Weierstrass} theorem $V $ is dense  in $H$. That is $H=\overline{V}$, whereas
	\begin{align*}
	...
	\end{align*}
	In particular, every Hilbert space with a countable basis is isometric to $\ell ^2(\N)$. However, the identification is not ``canonical'' as it depends on the bases chosen. The next example is all pervasive (in fact it has \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{}{even invaded our blog})
	...
	\end{theorem}
	\begin{proof}
		For each fixed $u \in H$, we have that $\ell_u :=B(\cdot ,u) \in  H'$. As a result by Riesz's representation theorem (\Cref{riesz theorem}) there exists a unique $ f_{\ell _u} \in H$ such that
		\begin{align}\label{f}
		...
		\end{align}
		As a result, $\rm{Im}(\Ll)^\perp = \{0\}$. We deduce from the corollary of the orthogonal complement (\Cref{cor}) that $\overline{\rm{Im}(\Ll)}=H$. Thus, if we show that $\Ll$ is closed invertibility follows. The estimate in \eqref{est} together with Cauchy Schwartz show that for all $u \in H$
		\begin{align*}
			\norm{ \Ll u}\geq \norm{u}.
		\end{align*}
...
\end{proposition}
 ...
 \end{proof}
  An important property of compact operators is that they are preserved by continuous ones.
	...
	\end{proposition}
	\begin{proof}
    This follows by the last equivalent characterization in \Cref{equivalent}.
	\end{proof}
  ...
  \end{proposition}
  \begin{proof}
    The first part again follows fromb the last equivalent characterization in \Cref{equivalent}. The second part relies on the fact that in complete spaces compact and \href{https://en.wikipedia.org/wiki/Totally_bounded_space#:~:text=not%20in%20general.-,In,-metric%20spaces%5B}{totally bounded} are equivalent.
  \end{proof}
...
\end{align*}
	...
	\end{corollary}
	\begin{proof}
		We have that $T= i \circ T$ so we conclude by Rellich-Kondrachov's \Cref{Rellich} and the preservation of compact operators by continuous ones (\Cref{comp}).
	\end{proof}
	...
	\begin{theorem}[Fredholm alternative]\label{fredholm} Let $H$ be a Hilbert space and  $K \in \Kk(H)$. Consider $T:= Id-K$ then it holds that
		\begin{align*}
