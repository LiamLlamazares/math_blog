...
:::
	...
	Note that the imposition that $V$ is closed in Theorem @thm-complement-theorem is necessary. Finite-dimensional subspaces are always closed, but infinite-dimensional subspaces may not be. For example, consider $H=L^2(I)$ for some bounded $I$ and take  $V$ to polynomials on $I$ . By the [Stone-Weierstrass](https://en.wikipedia.org/wiki/Stone%E2%80%93Weierstrass_theorem) theorem $V$ is dense  in $H$. That is $H=\overline{V}$, whereas
...
For each fixed $u \in H$, we have that $\ell_u :=B(\cdot ,u) \in  H'$. As a result by Riesz's representation theorem (Theorem @thm-riesz-theorem) there exists a unique $f_{\ell _u} \in H$ such that
...
As a result, we deduce from the corollary of the orthogonal complement theorem @cor-cor that $\overline{\Im(\Ll)}=0$. Thus, if we show that $\Ll$ is closed invertibility follows. The estimate in @eq-est together with Cauchy Schwartz show that for all $u \in H$
...
1. [a)]
2. $K \in \Kk(X,Y)$.
3. $K(B_X)$ is relatively compact where  $B_X=\{ x \in X : \norm{x}<1\}$ is the unit ball in $X$.
4. From every sequence  $Kx_n$ where  $x_n \in B_X$ one can extract a subsequence $K {x_{n_j}}$ converging in $Y$.
...
This follows by the last equivalent characterization in Proposition @prp-equivalent. 
...
The first part again follows fromb the last equivalent characterization in Proposition @prp-equivalent.  The second part relies on the fact that in complete spaces compact and [totally bounded](https://en.wikipedia.org/wiki/Totally_bounded_space#:~:text=not%20in%20general.-,In,-metric%20spaces%5B) are equivalent.
...
We have that $T= i \circ T$ so we conclude by Rellich-Kondrachov's theorem @thm-rellich and the preservation of compact operators by continuous ones (Proposition @prp-comp).
...
1. [a)]
2. $\ker(T)$ is finite dimensional.
3. $T$ is closed.
4. $\Im (T)=\ker(T^*)^\perp$.
5. $\mathrm{dim}(\ker(T))=\mathrm{dim}(\ker(T^*))$
