...
As a result, we deduce from the corollary of the orthogonal complement @cor-cor that $\overline{\Im(\Ll)}=0$. Thus, if we show that $\Ll$ is closed invertibility follows. The estimate in ([-@eq-est]) together with Cauchy Schwartz show that for all $u \in H$
