...
\begin{document}
\title{Parabolic PDE, well-posedness and regularity}
\author{Liam Llamazares-Elias}
\date{\today}
\maketitle
\section{Three point summary}
\begin{itemize}
  \item A parabolic PDE describes how a function evolves over time under the influence of an elliptic operator. Unlike their elliptic counterparts, parabolic PDEs are typically \href{https://nowheredifferentiable.com/2024-02-28-PDEs-6-Elliptic_PDE._Well_posedness_and_regularity/#:~:text=is%20well%2Dposed.-,Definition%202,-(Well%2Dposedness)}{well-posed}. For well-behaved coefficients, a unique solution always exists and depends continuously on the initial data.
...
In the \href{https://nowheredifferentiable.com/2024-02-28-PDEs-6-Elliptic_PDE._Well_posedness_and_regularity/}{previous post of this series}, we studied the well-posedness and regularity of solutions to an elliptic PDE of the form
\begin{align*}
  \begin{cases}
    \Ll u = f & \text{in } U     \\
...
To be able to proceed, we need to define an integral on functions valued in Banach spaces. This integral is called the \href{https://nowheredifferentiable.com/2022-05-27-The-Bochner-integral/}{Bochner integral}. To briefly summarize,
...
\end{align*}
With this identification, the dual of $L^p(I \to X)$ is equal to $L^{p'}(I \to X')$ . We will also use that if $\phi_n \in C^\infty(I)$ is a \href{https://nowheredifferentiable.com/2023-07-12-PDEs-3-Sobolev_spaces/#smooth%20section:~:text=is%20an-,approximation,-to%20unity%20if}{smooth approximation of unity} then, for $p \in [1, \infty)$ 
...
Alternatively, if $U$ is bounded and smooth, $\Delta ^{-1}$ is compact and self-adjoint. Hence, it has a countable orthonormal basis of eigenfunctions which are smooth by the \href{https://nowheredifferentiable.com/2024-02-28-PDEs-6-Elliptic_PDE._Well_posedness_and_regularity/#:~:text=for%20smooth%20coefficients.-,Theorem%2016}{previous post} and induction.  
\end{hint}
...
\begin{proof}
 ...
 B(u_n(t),u_n(t);t) \geq \gamma \norm{u_n(t)}_{H^1_x}^2 - \nu\norm{u_n(t)}_{L^2_x}^2
       \end{align}
 (this is the same as \href{https://nowheredifferentiable.com/2024-02-28-PDEs-6-Elliptic_PDE._Well_posedness_and_regularity/#:~:text=%20.%20We-,recall,-the%20Cauchy%20inequality}{what was proved} as in the elliptic case).
...
\begin{proof}
 ...
 Taking limits above and by density of $\Ss_n$ in $C_t^2H_{0,x}^1$ we obtain that
  ...
  \end{align*}
 where we used that by Lemma \ref{continuity weak} $u_{n_k} \to u \in C_t L_x^2$. In particular,
  \begin{align*}
...
\end{theorem}
We also show a result where the coefficients are allowed to depend on time. The following can be found in Chapter $3$ of \cite{Friedman1983PartialDE} and assumes that the coefficients are \href{https://nowheredifferentiable.com/2023-07-12-PDEs-3-Sobolev_spaces/#:~:text=%2C%20we-,define,-the%20Holder%20space}{Hölder continuous}
