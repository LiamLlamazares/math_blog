...
\end{align*}
With this identification, the dual of $L^p(I \to X)$ is equal to $L^{p'}(I \to X')$ . We will also use that if $\phi_n \in C^\infty(I)$ is a \postref{PDEs/3. Sobolev spaces}{}{smooth approximation of unity} then, for $p \in [1, \infty)$ 
...
Alternatively, if $U$ is bounded and smooth, $\Delta ^{-1}$ is compact and self-adjoint. Hence, it has a countable orthonormal basis of eigenfunctions which are smooth by the \postref{PDEs/6. Existence elliptic}{}{previous post} and induction.  
\end{hint}
...
\begin{proof}
 ...
 B(u_n(t),u_n(t);t) \geq \gamma \norm{u_n(t)}_{H^1_x}^2 - \nu\norm{u_n(t)}_{L^2_x}^2
       \end{align}
 (this is the same as \postref{PDEs/6. Existence elliptic}{}{what was proved} as in the elliptic case).
...
\end{theorem}
We also show a result where the coefficients are allowed to depend on time. The following can be found in Chapter $3$ of \cite{Friedman1983PartialDE} and assumes that the coefficients are \postref{PDEs/3. Sobolev spaces}{}{Hölder continuous}
