...
- A parabolic PDE describes how a function evolves over time under the influence of an elliptic operator. Unlike their elliptic counterparts, parabolic PDEs are typically [well-posed](<../../PDEs/6. Existence elliptic/index.qmd>). For well-behaved coefficients, a unique solution always exists and depends continuously on the initial data.
...
In the [elliptic case](<../../PDEs/6. Existence elliptic/index.qmd>), we studied the well-posedness and regularity of solutions to an elliptic PDE of the form
...
To be able to proceed, we need to define an integral on functions valued in Banach spaces. This integral is called the [Bochner integral](<../../Analysis on Banach spaces/Bochner_integral/index.qmd>). To briefly summarize,
...
With this identification, the dual of $L^p(I \to X)$ is equal to $L^{p'}(I \to X')$ . We will also use that if $\phi_n \in C^\infty(I)$ is a [smooth approximation of unity](<../../PDEs/3. Sobolev spaces/index.qmd>) then, for $p \in [1, \infty)$ 
...
Alternatively, if $U$ is bounded and smooth, $\Delta ^{-1}$ is compact and self-adjoint. Hence, it has a countable orthonormal basis of eigenfunctions which are smooth by the [previous post](<../../PDEs/6. Existence elliptic/index.qmd>) and induction.  
...
b. Continuity in the data: The plan will be to apply Gronwall's inequality. By ([-@eq-galerkin]) and the linearity of the inner product, we obtain
    ...
    (this is the same as [what was proved](<../../PDEs/6. Existence elliptic/index.qmd>) as in the elliptic case).
...
We also show a result where the coefficients are allowed to depend on time. The following can be found in Chapter $3$ of [@Friedman1983PartialDE] and assumes that the coefficients are [Hölder continuous](<../../PDEs/3. Sobolev spaces/index.qmd>)
