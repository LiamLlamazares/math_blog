...
\norm{f}_{L^p(I,X)}  &:= \left( \int_I \norm{f(t)}_X^p \d t \right)^{1/p}, \quad p \in [1, \infty),\\
  \|f\|_{L^{\infty}(I\rightarrow X)}&:=\inf \{r>0: \mu(\|f\|>r)=0\}.
\end{aligned}
$$
...
$$
\begin{aligned}
\Phi: C([0,\epsilon ] \to X) \to C([0,\epsilon ] \to X), \quad u \mapsto \Phi(u)(t) := g + \int_I F(u(s)) \d s.
\end{aligned}
$$
...
We divide the proof into three parts. Existence, continuity, and uniqueness.
 1. [a)]
2. Existence of solutions:  Since the $\phi_i$ are orthonormal and we impose $u_n \in \Ss _n$, solving @eq-galerkin is equivalent to finding $\bm{\lambda}_n\in C(I \to \R^n)$ such that
...
3. Continuity in the data: The plan will be to apply Gronwall's inequality. By @eq-galerkin and the linearity of the inner product, we obtain
...
Using Cauchy's inequality $ab \leq \frac{1}{2} (\epsilon a^2+\epsilon^{-1} b^2)$, the boundedness of the coefficients and the ellipticity of $\bm{A}$ shows that, for some constants $\beta , \nu >0$
...
4. Uniqueness:  To conclude uniqueness, let $u_n^{{1}}, u_n^{{2}}$ be two solutions to the Galerkin problem. Then, $w_n := u_n^{{1}}-u_n^{{2}}$ verifies the homogeneous problem
...
A common technique in PDE is to modify your initial problem $P$ by some quantity $\epsilon$ to obtain a problem $P_ \epsilon$ that is easier to solve. Suppose one can find solutions to $P_ \epsilon$ that are bounded. In that case, a converging subsequence can typically be extracted and, under appropriate conditions, will converge to a solution to the original problem $P$. This is exactly what we show now.
...
Taking limits above and by density of $\Ss_n$ in $C_t^2H_{0,x}^1$ we obtain that
 
$$
\begin{aligned}
\br{v(0),g}_{L_x^2} = \br{v(0),u(0)}_{L_x^2}, \quad \forall v \in C_t^2H_{0,x}^1.
\end{aligned}
$$

where we used that by Lemma @lem-continuity-weak $u_{n_k} \to u \in C_t L_x^2$. In particular,
