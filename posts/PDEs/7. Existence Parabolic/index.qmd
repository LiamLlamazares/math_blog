...
aliases:
  - /2024-10-19-PDEs-7-Parabolic_PDE._Well_posedness_and_regularity/
image: "cover.png"
...
<div id="quarto-post-pdf-url" data-url="Parabolic.pdf" style="display:none;"></div>

\postaliases{/2024-10-19-PDEs-7-Parabolic_PDE._Well_posedness_and_regularity/}

## Three point summary

- A parabolic PDE describes how a function evolves over time under the influence of an elliptic operator. Unlike their elliptic counterparts, parabolic PDEs are typically [well-posed](</posts/PDEs/6. Existence elliptic/index.qmd>). For well-behaved coefficients, a unique solution always exists and depends continuously on the initial data.
...
In the [elliptic case](</posts/PDEs/6. Existence elliptic/index.qmd>), we studied the well-posedness and regularity of solutions to an elliptic PDE of the form
...
To be able to proceed, we need to define an integral on functions valued in Banach spaces. This integral is called the [Bochner integral](</posts/Analysis on Banach spaces/Bochner_integral/index.qmd>). To briefly summarize,
...
With this identification, the dual of $L^p(I \to X)$ is equal to $L^{p'}(I \to X')$ . We will also use that if $\phi_n \in C^\infty(I)$ is a [smooth approximation of unity](</posts/PDEs/3. Sobolev spaces/index.qmd>) then, for $p \in [1, \infty)$ 
...
Alternatively, if $U$ is bounded and smooth, $\Delta ^{-1}$ is compact and self-adjoint. Hence, it has a countable orthonormal basis of eigenfunctions which are smooth by the [previous post](</posts/PDEs/6. Existence elliptic/index.qmd>) and induction.  
...
b. Continuity in the data: The plan will be to apply Gronwall's inequality. By ([-@eq-galerkin]) and the linearity of the inner product, we obtain
    ...
    (this is the same as [what was proved](</posts/PDEs/6. Existence elliptic/index.qmd>) as in the elliptic case).
...
Let $\set{u_n}_{n=1}^\infty$ be the sequence of solutions to the Galerkin problem ([-@eq-galerkin]) guaranteed by Theorem ([-@eq-galerkin]).  By said theorem, $u_n$ and $u'_n$ are bounded sequences in $L_t^2H_{0,x}^1$ and $L_t^2 H_x^{-1}$ respectively. Since these spaces are Hilbert spaces, they are reflexive, and we deduce by the <https://en.wikipedia.org/wiki/Banach
...
![Galerkin solution to the parabolic problem ([-@eq-parabolic-2]) with $n=3$ basis functions.](Galerkin_n_3.svg){fig-align="center" width="80%"}

![Galerkin solution to the parabolic problem ([-@eq-parabolic-2]) with $n=20$ basis functions.](Galerkin_n_20.svg){fig-align="center" width="80%"}
...
![Approximation of $g=1$ in $L^2(U)$ using the basis functions $\phi_j \in H_0^1(U)$.](approx_boundary_l2.svg){fig-align="center" width="80%"}
...
![Exact solution to the parabolic problem ([-@eq-exact]). ](Galerkin_exact.svg){fig-align="center" width="80%"}

![Galerkin solution to the parabolic problem ([-@eq-exact]) with $n=20$ basis functions.](Galerkin_n_20_2.svg){fig-align="center" width="80%"}

## Regularity of the solutions

Having proved the well-posedness of the parabolic problem ([-@eq-parabolic]) and its finite-dimensional Galerkin approximation, we can now move on to study the regularity of the solutions. The proof is similar to the one given in the [elliptic case](</posts/PDEs/6. Existence elliptic/index.qmd>) but more technical, and we will mainly cite some main results without proof. 
...
We also show a result where the coefficients are allowed to depend on time. The following can be found in Chapter $3$ of [@Friedman1983PartialDE] and assumes that the coefficients are [Hölder continuous](</posts/PDEs/3. Sobolev spaces/index.qmd>)
