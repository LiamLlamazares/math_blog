...
Where $D$ is some domain in $\R^d$. In a previous post on the [Fourier transform](https://nowheredifferentiable.com/2023-01-29-PDE-1-Fourier/#:~:text=Sobolev%20spaces-,Sobolev,-spaces%20form%20a) we saw how to define the Sobolev spaces $H^s(D)$ when $D$ is the whole Euclidean space $\R^d$ or the torus $\T^d$. These spaces correspond to $s$-times weakly differentiable functions and we saw how these spaces could help us solve @eq-pde1.  However, in practice $D$ may be an open set in $\R^d$ or even some $d$-dimensional manifold with a boundary condition
...
2. Studying if $\tilde{u} \in  H^s(\R^d)$. That is, as we saw in the [previous post](https://nowheredifferentiable.com/2023-01-29-PDE-1-Fourier/#:~:text=fact%2C%20since%20the-,Fourier,-transform%20is%20a), checking if
...
As we will see later (Corollary @exr-restriction) this is a better approach. However, it is not optimal as it requires some conditions on $U$. For example, if $U$ is not smooth we cannot assert that  $\restr{C^\infty(\R^d)}{U}= C^\infty(U)$.
...
We need a new approach. Motivated as in the previous post by [duality](https://nowheredifferentiable.com/2023-01-29-PDE-1-Fourier/#:~:text=is%20called%20the-,duality,-method%20and%20appears) we should begin by defining what is meant by a weak derivative of a function $u$ in $L^p(U)$. If $u,\varphi$ are smooth functions then we have by integration by parts
...
We endow  $C_c^\infty(K)$ with the [topology generated  by the countable family of seminorms](https://nowheredifferentiable.com/2023-01-29-PDE-1-Fourier/#:~:text=together-,with,-a%20countable%20family)
...
The topology is [metrizable](https://nowheredifferentiable.com/2023-01-29-PDE-1-Fourier/#:~:text=together-,with,-a%20countable%20family) as the family of seminorms is countable
Use Exercise @exr-convergence-tvs  to show that if $\varphi_n \in C_c^\infty(K)$ is Cauchy then the sequence of derivatives $D^\alpha \varphi_n$ converge uniformly to $\varphi^{(\alpha)} \in C_c^0(K)$ for all $\alpha$. It remains to show that
...
Given a Cauchy net $\varphi_{\bullet} \in C_c^\infty(U)$ show as in Exercise @exr-local-convergence that the support of  $\varphi_{\bullet}$ cannot escape a compact set $K$. Conclude using the completeness on  $C_c^\infty(K)$ and Exercise @exr-convergence-tvs. 
...
And endow it with the [topology generated by the family of seminorms](https://nowheredifferentiable.com/2023-01-29-PDE-1-Fourier/#:~:text=together-,with,-a%20countable%20family)
...
Then $u=0$. Consider a compact set $K \subset U$ and write  $g$ for the extension of $\mathrm{sign}(u)$ by zero outside of  $K$. Clearly $g \subset L^1(\R^d)$. Consider an approximation to the identity $\phi_n$ (see Appendix @thm-smooth-section ) and set
...
Let $\varphi$ have support in $U$. Then,     by compactness, we can extract a finite covering $\set{U_i}_{i=1}^n$ of $\supp{\varphi}$. Let $\set{\rho _i}_{i=1}^n$ be a partition of unity  subordinate to $U_i$ (see Appendix @thm-local-to-global). Then
...
By the just proved Lemma @lem-biggest-vanish we see that there is a largest set on which $w$ vanishes. As a result, we can make the following definition.
...
We saw in Theorem @thm-motivation that $f$ is  $0$ almost everywhere on some open set if and only if it integrates to $0$ against any test function on the open set. This shows the first part and the second follows immediately.
...
Where $L^p(U)$ is identified as a subspace of  $\Dd'(U)$ by Theorem @thm-motivation.  We give $W^{k,p}(U)$ the norm
...
In this section, we prove the relevant results. These rely heavily on the analogous density results for functions in $L^p(U)$ (see Appendix @thm-smooth-section). As a result, they will not when $p=\infty$. We start without making any assumptions on $U$ and obtain two local-type results. Throughout this section we will often be switching between different open sets and, if following the proofs, making some drawings is recommended.
...
Taking limits we conclude from Proposition @prp-app-pn that
...
This concludes the proof as by Exercise @exr-convergence-tvs local convergence is equivalent to convergence on every compactly included subset.
...
The proof is an instructive way of using a partition of unity to piece together a local result (in this case Theorem @thm-local) to get a global one. Let $\eps >0$ and consider an open covering $\set{V_i}_{i=0}^\infty$ of $U$ with $V_0 =\emptyset$ and $V_{i} \Subset V_{i+1}$. By Theorem @thm-partition we may obtain a partition of unity $\rho _i$ subordinate to the ``rings'' $U_{i}:= V_{i+1}\setminus \overline{V_{i-1}}$ (the trickery with the indices is so that the $U_i$ actually  cover $U$). Where we relabel so that
...
$$
	...
	We have that $\rho_i u \in C_c^\infty({V_{i+1}})$ with $V_{i+1} \Subset U$ so by the local approximation in Theorem @thm-local we can find $n_i$ such that
...
Note that Theorem @thm-meyers is not strictly stronger than theorem @thm-local as it is not, in general, possible to extend functions in $C^\infty_{\mathrm{loc}}(U)$ to $C_c^\infty(\R^n)$. As a corollary of Meyers-Serrin's theorem, we obtain an equivalent definition of $W^{k,p}(U)$.
...
Before  Theorem @thm-meyers was proved, both our original definition  @def-sobolev-def (distributions with derivatives in $L^p(U)$) and the one in Corollary @exr-eq-def (closure of smooth functions with Sobolev norm) were used as the definition of $W^{k,p}(U)$. But it was unclear which was the ``correct'' Sobolev space. This debate was settled by Meyers and Serrin who proved that, as we just showed, both are equal.

We now show an example of how these kinds of density results can be useful. The following generalizes the second point of Proposition @prp-properties
...
In Exercise @exr-change-of-variables it is important that $\Phi$ is diffeomorphic so that composition with $\Phi$ is continuous.

::: {#exr-12}

Show that the conclusion of Exercise @exr-change-of-variables is false if we only assume that $\Phi \in C^k(\R^d)$ and do not impose invertibility.
...
Now we provide a final global approximation result in the case where the domain is smooth (see Appendix @thm-boundary for a review on manifolds with boundary) and bounded.
...
For $n$ big enough we have that  $\varphi$ is well defined ans smooth on $W_0:=B_{\frac{1}{n}}(x_0)$. Since translation is continuous on $L^p(U)$ for  $p \in [1,\infty)$ and by the behavior of differentiation with convolution  (see Proposition @prp-properties), for $n$ large enough
...
Now we take an open set $W_{n+1} \Subset \Omega$ such that $\set{W_i}_{i=0}^{n+1}$ cover $\Omega$. By the local approximation of Theorem @thm-local we know we can approximate  $u$ on  $W_{n+1}$ by some $\varphi_{n+1} \in C_c^\infty(\R^d)$
...
Finally, we take a smooth partition of unity $\set{\rho _i}_{i=0}^{n+1}$ subordinate to $\set{W_i}_{i=0}^{n+1}$ and a bump function $\eta \in C_c^\infty(\R^d)$ which is equal to $1$ on  $\Omega$ and is supported on $\bigcup_{i=0}^{n+1} W_i$ (see example @exm-bump-example2) and set
...
By Theorem @thm-global we also give ourselves an epsilon of room by supposing that $u \in C^k(\overline{\Omega})$. We define the extension of $u$ to $\Omega'$ as
...
As a result, $E$ is a (bounded) linear operator. So far we had considered $u \in  C^k(\overline{\Omega})$. Now, since by Theorem @thm-global the space  $C^k(\overline{\Omega})$ is dense in $W^{k,p}(\Omega)$ we may extend  $E$ to a linear operator on the whole of $W^{k,p}(\Omega)$. A verification shows that $E$ is also bounded as an operator from  $C^k(\Omega)$ to  $C^k(\R^d)$. This concludes the proof.

:::

::: {#exr-restriction name="Restriction"}

Under the conditions of Theorem @thm-extension it holds that
...
Given $u \in W^{k,p}(\Omega)$ we can extend it to $Eu \in W^{k,p}(\R^d)$ by the just proved extension theorem @thm-extension.  By definition $u= \restr{Eu}{\Omega}$. The case $u \in C^k(\R^d)$ is identical.
...
As in previous results, the trick is to suppose first $u$ is smooth, work locally, and then obtain a global result using a partition of unity and the density in Theorem @thm-global.  \\
...
and using the extension theorem @thm-extension to extend $u':= u\circ \Phi$ to $\widetilde{u}'$ with compact support $K \subset \R^d$  we obtain by the [divergence theorem](https://en.wikipedia.org/wiki/Divergence_theorem#:~:text=space%5Bedit%5D-,We,-are%20going%20to%20prove)
...
Using Theorem @thm-global to extend $T$ continuously to $W^{k,p}(\Omega)= \overline{\restr{C_c^\infty(\R^d)}{\Omega}}$ concludes the proof.
...
Straightening out the boundary and using the extension operator as in the trace theorem @thm-trace, it is sufficient to work in the case where $u$ is smooth and defined on  $\Hh^{d}$.  By Fourier inversion, if we write $\xi=(\xi',\xi _d)$
...
The converse to Exercise @exr-trace-0 holds but is far from trivial.
...
By density (see Theorem @thm-approx-rd), it is enough to take $u \in C_c^\infty(\R^d)$. Applying the fundamental theorem of calculus gives
...
Apply induction on $k$ using Theorem @thm-est1. 
...
Use induction on $k$  with Theorem @thm-estt2. 
...
This holds if and only if $d=1$. Otherwise, we require more integrability. However, this extra integrability can be converted into regularity in the style of [Sobolev spaces](https://nowheredifferentiable.com/2023-01-29-PDE-1-Fourier/#:~:text=.%C2%A0%E2%97%BB-,As%20a%20corollary,-of%20this%2C%20we). First, we recall the following definition
...
The fact that the above inclusions are continuous follows by our three Sobolev inequalities in Exercises @exr-est12,@exr-est22,@exr-est32 together with the extension theorem @thm-extension. 
...
For any of the cases in Rellich-Kondrachov's theorem @thm-rellich, the condition on $q$ holds. The first part of the result then follows as  $C^k(\overline{\Omega}) \hookrightarrow L^q(\Omega)$ and the second part from the fact that  $p< p^{k^*}$.
...
Using the same technique as in Theorem @thm-completeness shows that $u^{(\alpha)}= D^{\alpha}u$. This shows that $u_{n_k} \to u$ in $W^{l,q}(\Omega)$ and, by the equivalence of compactness and sequential compactness, concludes the proof.

:::

This result can be combined directly with Rellich-Kondrachov @thm-rellich or any other embedding. For example, when combined with Corollary @cor-simple-embedding we get
...
The previous definitions all go through word by word in the case where we substitute the domain from Euclidean space  $\R^d$ to a [LCA group with Haar measure](https://nowheredifferentiable.com/2023-01-29-PDE-1-Fourier/#:~:text=Since-,every,-locally%20compact%20Hausdorff) $\mu$. For example
...
By Proposition @prp-young $(L^1(\R^d),*)$ is a [Banach algebra](https://en.wikipedia.org/wiki/Banach_algebra). However, it is a non-unital one. That is there does not exist an element $e$ such that
...
We now prove the second part. By Lemma @lem-uryshon we can choose $g$ such that
...
By induction, it suffices to consider the case $D^\alpha = \partial _i$ for some $1 \leq i \leq d$. This case can be proved by a [differentiation under the integral sign](https://nowheredifferentiable.com/2023-01-29-PDE-1-Fourier/#:~:text=Proposition%202%20) as, given $\abs{x}\leq M$
...
This follows immediately from Proposition @prp-app-pn and Proposition @prp-smooth. 
...
Let $f$ be the non-smooth function to approximate, multiply it first by a mollifier $\eta_n \in C_c^\infty(U)$ equal to $1$ in $V_n\subset U$ (see Example @exm-bump-example2). Convolve to get
...
Show as in Theorem @thm-density-thm that $\varphi_n$ converges appropriately.
...
The first part can be proved by the same method as in Lemma @lem-uryshon (note that Uryshon's lemma [still holds](https://planetmath.org/ApplicationsOfUrysohnsLemmaToLocallyCompactHausdorffSpaces) for LCH spaces). The second part follows by copying the proof of Propositions @prp-app-pn and @prp-smooth. 
...
Note that the second part of Corollary @cor-stone also follows from the [Stone-Weierstrass theorem](https://en.wikipedia.org/wiki/Stone%E2%80%93Weierstrass_theorem).
...
In our case we will always take $M$ to be a subset of  $\R^d$, in this case, different variations of the above definition are possible. For example, by the inverse function theorem and Exercise @exr-ext, we can extend the functions $\Phi_\alpha$ to diffeomorphisms on $U_\alpha$ **open in $\R^d$** so that @eq-chart-@eq-chart2 now read
