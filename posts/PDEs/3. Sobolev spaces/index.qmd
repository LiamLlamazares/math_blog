...
Given a compactly embedded set $V \Subset U$ we can take $n$ large enough so that  $V+  B \qt{0, \frac{1}{n}} \subset U$ and as a result, the convolution  $\varphi_n:= \tilde{u} * \phi_n$  verifies (see Observation @local-smoothing)
...
By induction, it suffices to consider the case $k=1$.We can approximate $u$ on each compact  $K \subset U$ by a sequence of functions $u_n \in C_c^\infty(\R^d)$. For $u_n$ the equality @exr-change-of-variables holds. Furthermore, by a change of variables, @eq-chain is continuous in $u\in W^{1,p}(\Omega)$ so we may pass to the limit and obtain @eq-chain for $u$ on $K$. Since $K$ was any, the equality also holds on the whole of $U$.
...
\begin{observation}
	The above definition is frequently also given letting the index set range over $\eps \in \R_+$ and taking $\supp{\phi_\eps } \subset B(0,\eps )$. This is equivalent and simply leads to taking $\eps \to 0$ instead of $n \to \infty$. \end{observation}
...
\begin{observation}[Local smoothing]
