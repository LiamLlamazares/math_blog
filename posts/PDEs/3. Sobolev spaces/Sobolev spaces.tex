	...
	\item Extending $u$ by zero outside of $U$ to form
	      \begin{align*}
		      \tilde{u}(x):=\begin{cases}
		                    ...
		                    \end{cases}.
	      \end{align*}
	\item Studying if $\tilde{u} \in  H^s(\R^d)$. That is, as we saw in the \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{}{previous post}, checking if
	      \begin{align*}
...
We need a new approach. Motivated as in the previous post by \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{}{duality} we should begin by defining what is meant by a weak derivative of a function $u$ in $L^p(U)$. If $u,\varphi$ are smooth functions then we have by integration by parts
\begin{align*}
...
\begin{definition}
	...
	\end{align*}
	We endow  $C_c^\infty(K)$ with the \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{}{topology generated  by the countable family of seminorms}
...
\end{align*}
This holds if and only if $d=1$. Otherwise, we require more integrability. However, this extra integrability can be converted into regularity in the style of \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{}{Sobolev spaces}. First, we recall the following definition
\begin{definition}
...
\begin{proposition}Convolution of distribution and test function is smooth:
	...
	\end{enumerate}
\end{proposition}
The previous definitions all go through word by word in the case where we substitute the domain from Euclidean space  $\R^d$ to a \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{}{LCA group with Haar measure} $\mu $. For example
\begin{align*}
...
\end{proposition}
\begin{proof}
	By induction, it suffices to consider the case $D^\alpha = \partial _i$ for some $1 \leq i \leq d$. This case can be proved by a \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{}{differentiation under the integral sign} as, given $\abs{x}\leq M$
	\begin{align*}
