...
\begin{itemize}
	...
	\item Given a topological vector space $X$ we write $X'$ for the dual of  $X $ and denote given  $w \in X', \varphi \in X$ the duality pairing as
	      \begin{align*}
		      (\varphi,w):= w(\varphi) .
	      \end{align*}
...
\end{align}
Where $D$ is some domain in $\R^d$. In a previous post on the \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{fracsobdef}{Fourier transform} we saw how to define the Sobolev spaces $H^s(D)$ when $D$ is the whole Euclidean space $\R^d$ or the torus $\T^d$. These spaces correspond to $s$-times weakly differentiable functions and we saw how these spaces could help us solve \eqref{PDE1}. However, in practice $D$ may be an open set in $\R^d$ or even some $d$-dimensional manifold with a boundary condition
	...
	\item Extending $u$ by zero outside of $U$ to form
	      \begin{align*}
		      \tilde{u}(x):=\begin{cases}
		                    ...
		                    \end{cases}.
	      \end{align*}
	\item Studying if $\tilde{u} \in  H^s(\R^d)$. That is, as we saw in the \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{fracsobdef}{previous post}, checking if
	      \begin{align*}
...
\end{align*}
As we will see later (\Cref{restriction}) this is a better approach. However, it is not optimal as it requires some conditions on $U$. For example, if $U$ is not smooth we cannot assert that  $\restr{C^\infty(\R^d)}{U}= C^\infty(U)$.

\section{Test functions and distributions}%
\subsection{Seminorms and their topologies}
We need a new approach. Motivated as in the previous post by \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{distrsec}{duality} we should begin by defining what is meant by a weak derivative of a function $u$ in $L^p(U)$. If $u,\varphi$ are smooth functions then we have by integration by parts
\begin{align*}
...
\begin{definition}
	...
	\end{align*}
	We endow  $C_c^\infty(K)$ with the \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{seminormsgivemetric}{topology generated  by the countable family of seminorms}
...
\begin{definition}
	Let $X$ be a vector space and let $\mathcal{P}$ be a family of seminorms on $X$. Then we define $\tau_\Pp$, the \textbf{topology \href{https://en.wikipedia.org/wiki/Subbase}{generated} by $\mathcal{P}$},
	\begin{align*}
...
\end{exercise}
\begin{hint}
	The topology is \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{seminormsgivemetric}{metrizable} as the family of seminorms is countable
	Use \Cref{convergence TVS}  to show that if $\varphi_n \in C_c^\infty(K)$ is Cauchy then the sequence of derivatives $D^\alpha \varphi_n $ converge uniformly to $\varphi^{(\alpha)} \in C_c^0(K)$ for all $\alpha$. It remains to show that
	\begin{align*}
...
\end{exercise}
\begin{hint}
	Given a Cauchy net $\varphi_{\bullet} \in C_c^\infty(U)$ show as in \Cref{local convergence} that the support of  $\varphi_{\bullet}$ cannot escape a compact set $K$. Conclude using the completeness on  $C_c^\infty(K)$ and \Cref{convergence TVS}.
\end{hint}
\begin{exercise}
...
An equivalent characterization of $\Dd'(U)$ \cite[page 241]{taylor2013partial} is that,  $\omega \in  \Dd'(U)$ if and only if for all $\varphi \in C_c^\infty(U)$ the operator $\varphi w$ defined by \begin{align}\label{equiv}
...
\begin{definition}
	...
	\end{align*}
	And endow it with the \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{seminormsgivemetric}{topology generated by the family of seminorms}
	\begin{align*}
		\rho_{K_n,p}(f):= \norm{f}_{L^p(K_n)}.
	\end{align*}
...
\end{exercise}
\begin{hint}
	The topology is \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{seminormsgivemetric}{metrizable} as the family of seminorms is countable. By \Cref{convergence TVS}  if $f_n \in L^p_{\mathrm{loc}}(U)$ is Cauchy then $f_n$ is Cauchy for all  $L^p(K)$ $D^\alpha \varphi_n $ so converges to some $f_K \in L^p(K)$. Show that $f_K= \restr{f}{K}$ where $f \in L^p_{\mathrm{loc}}(U)$    to conclude the proof.
\end{hint}
...
\end{hint}
\subsection{Support of a distribution}\label{support sec}
...
\end{proof}
By the just proved \Cref{biggest vanish} we see that there is a largest set on which $w$ vanishes. As a result, we can make the following definition.
...
\begin{hint}
	We saw in \Cref{motivation} that $f$ is  $0$ almost everywhere on some open set if and only if it integrates to $0$ against any test function on the open set. This shows the first part and the second follows immediately.
\end{hint}
...
\begin{definition}[Sobolev spaces]\label{sobolev def}
	...
	\end{align*}
	Where $L^p(U)$ is identified as a subspace of  $\Dd'(U)$ by \Cref{motivation}. We give $W^{k,p}(U)$ the norm
	\begin{align*}
...
\begin{proposition}[Properties]\label{properties}Let $u \in W^{k,p}(U)$. Then it holds that
	\begin{enumerate}
		...
		\item Leibniz rule: given $\varphi \in C_c^1(U)$ it holds that
		      \begin{align*}
			      \partial _{i} (u \varphi)= \partial_i u \varphi+ u \partial _i \varphi .
		      \end{align*}

		\item For $U = \R^d$, the translation $\tau_y u(x):=u(x-y) \in W^{k,p}(\R^d)$ with $D^\alpha \tau_y u = \tau_y D^\alpha u$.
...
\end{proposition}
We omit the proof which can be found in \cite{Juha2023} pages $39-43$. The next exercise show that, perhaps somewhat unexpectedly, to have $u \in  W^{1,p}(U)$ it is not sufficient to require that $u$ is differentiable almost everywhere with integrable derivatives.
\begin{example}
...
\section{Smooth approximation of Sobolev functions}\label{approxsec}
...
\begin{proof}
	...
	\end{align*}
	Taking limits we conclude from \Cref{app pn} that
	\begin{align*}
	...
	\end{align*}
	This concludes the proof as by \Cref{convergence TVS} local convergence is equivalent to convergence on every compactly included subset.
\end{proof}
\begin{observation}
...
\begin{theorem}[Meyers-Serrin: local till boundary approximation]\label{Meyers}
	...
	\end{align*}
\end{theorem}
\begin{proof}The proof is an instructive way of using a partition of unity to piece together a local result (in this case \Cref{local}) to get a global one. Let $\eps >0$ and consider an open covering $\set{V_i}_{i=0}^\infty $ of $U$ with $V_0 =\emptyset$ and $V_{i} \Subset V_{i+1}$. By \Cref{partition} we may obtain a partition of unity $\rho _i$ subordinate to the ``rings'' $U_{i}:= V_{i+1}\setminus \overline{V_{i-1}}$ (the trickery with the indices is so that the $U_i$ actually  cover $U$). Where we relabel so that
	\begin{align*}
		\supp{\rho _i}\subset U_i .
	\end{align*}
	We have that $\rho_i u \in C_c^\infty({V_{i+1}})$ with $V_{i+1} \Subset U$ so by the local approximation in \Cref{local} we can find $n_i$ such that
	\begin{align*}
	...
	\end{align*}
\end{proof}
Note that \Cref{Meyers} is not strictly stronger than \Cref{local} as it is not, in general, possible to extend functions in $C^\infty_{\mathrm{loc}}(U)$ to $C_c^\infty(\R^n)$. As a corollary of Meyers-Serrin's theorem, we obtain an equivalent definition of $W^{k,p}(U)$.
...
Before  \Cref{Meyers} was proved, both our original definition  \ref{sobolev def} (distributions with derivatives in $L^p(U)$) and the one in \Cref{eq def} (closure of smooth functions with Sobolev norm) were used as the definition of $W^{k,p}(U)$. But it was unclear which was the ``correct'' Sobolev space. This debate was settled by Meyers and Serrin who proved that, as we just showed, both are equal.

We now show an example of how these kinds of density results can be useful. The following generalizes the second point of \Cref{properties}
...
\end{hint}
In \Cref{change of variables} it is important that $\Phi$ is diffeomorphic so that composition with $\Phi$ is continuous.
\begin{exercise}
	Show that the conclusion of \Cref{change of variables} is false if we only assume that $\Phi \in C^k(\R^d)$ and do not impose invertibility.
\end{exercise}
\begin{hint}
	Divide by zero.
\end{hint}
...
\begin{proof}
	...
	\end{align*}
	For $n$ big enough we have that  $\varphi_n$ is well defined and smooth on $W_0:=B_{\frac{1}{n}}(x_0)$. Since translation is continuous on $L^p(U)$ for  $p \in [1,\infty)$ and by the behavior of differentiation with convolution  (see \Cref{properties}), for $n$ large enough
	\begin{align*}
	...
	Now we take an open set $W_{n+1} \Subset \Omega$ such that $\set{W_i}_{i=0}^{n+1} $ cover $\Omega$. By the local approximation of \Cref{local} we know we can approximate  $u$ on  $W_{n+1}$ by some $\varphi_{n+1} \in C_c^\infty(\R^d)$
	\begin{align*}
		\norm{u-\varphi_{n+1}}_{W^{k,p}(W_{n+1})}\leq\frac{\eps}{n+2} .
	\end{align*}
	Finally, we take a smooth partition of unity $\set{\rho _i}_{i=0}^{n+1}$ subordinate to $\set{W_i}_{i=0}^{n+1} $ and a bump function $\eta \in C_c^\infty(\R^d)$ which is equal to $1$ on  $\Omega$ and is supported on $\bigcup_{i=0}^{n+1} W_i$ (see \Cref{bump example2}) and set
	\begin{align*}
...
\begin{definition}\label{extension domain}
	Let $X(U)$ be a topological space of functions for each open $U \subset \R^d$. We say that $\Omega $ is an \textbf{extension domain} for $X$ if there exists a continuous operator
	\begin{align*}
		E: X(\Omega ) \to X(\R^d),
	\end{align*}
	such that $Eu(x)=u(x)$ for almost all $x \in \Omega $. We call $E$ an \textbf{extension operator}.
\end{definition}

For an extension to be possible, it is crucial that $\Omega$ is regular enough. It will be sufficient to impose that $\Omega$ have uniformly Lipschitz boundary. The following definition and Theorem can be found in \cite[Section 13]{leoni2017first}.

\begin{definition}\label{uniformly Lipschitz domain}
	We say that an open set $\Omega \subset \R^d$ has \textbf{uniformly Lipschitz boundary} if there exists $\eps,L>0, N \in \N$ and an open covering $\set{U_n}_{n \in \N}$ of $\partial \Omega$ such that
	\begin{enumerate}
		\item For all $x \in \partial \Omega$ there exists a $U_n$ such that $B_\eps(x) \subset U_n$.
		\item Every point in $\R^d$ is contained in at most $N$ sets $U_n$.
		\item For each $n$ there exist local coordinates $y=(y', y_n) \in \R^{d-1} \times \R$ and a Lipschitz continuous function $f: \R^{d-1} \to \R$ (both depending on $n$), with Lipschitz constant $\operatorname{Lip} f \leq L$, such that $U_n \cap \Omega=U_n \cap V_n$, where $V_n$ is given in local coordinates by
		      \begin{align*}
			      \set{(x', x_d) \in \R^{d-1} \times \R: x_d>f(x')} .
		      \end{align*}
	\end{enumerate}
	That is, a domain is uniformly Lipschitz if it has a locally finite covering which makes it of type $C^{0,1}$ (see Appendix \ref{boundary}). Below we show two typical examples.
\end{definition}

\begin{exercise}\label{exercise lipschitz}
...
\begin{theorem}[General extension]\label{extension general}
...
\end{theorem}
\begin{proof}
	We prove the result when the domain is bounded with $C^k$ boundary and when $p<\infty$. The general result can be found in \cite[page 424]{leoni2017first}.
	...
	\end{align*}
	By \Cref{global} we also give ourselves an epsilon of room by supposing that $u \in C^k(\overline{\Omega})$. We define the extension of $u$ to $\Omega'$ as
	\begin{align*}
		Eu(x) = \begin{cases}
			        u(x) \quad                                 & x_{d}                                 \geq 0      \\
	...
	\end{align*}
	As a result, $E$ is a (bounded) linear operator. So far we had considered $u \in  C^k(\overline{\Omega})$. Now, since by \Cref{global} the space  $C^k(\overline{\Omega})$ is dense in $W^{k,p}(\Omega)$ we may extend  $E$ to a linear operator on the whole of $W^{k,p}(\Omega)$. A verification shows that $E$ is also bounded as an operator from  $C^k(\Omega)$ to  $C^k(\R^d)$. This concludes the proof.
\end{proof}
\begin{exercise}[Restriction]\label{restriction} Under the conditions of \Cref{extension} it holds that
	\begin{align*}
...
\end{exercise}
\begin{hint}
	Given $u \in W^{k,p}(\Omega)$ we can extend it to $Eu \in W^{k,p}(\R^d)$ by the just proved extension \Cref{extension}. By definition $u= \restr{Eu}{\Omega}$. The case $u \in C^k(\R^d)$ is identical.
\end{hint}
...
\section{Trace theorem}\label{trace sec}
...
\begin{proposition}[Trace]\label{trace}
...
\end{proposition}
\begin{proof}
	We will only prove the case where $\Omega$ is bounded and of class $C^1$. The proof of the general statement can once more be found in \cite[page 592]{leoni2017first}.

	As in previous results, the trick is to suppose first $u$ is smooth, work locally, and then obtain a global result using a partition of unity and the density in \Cref{global}. \\
	...
	\end{align*}
	and using the extension \Cref{extension} to extend $u':= u\circ \Phi$ to $\widetilde{u}'$ with compact support $K \subset \R^d $  we obtain by the \href{https://en.wikipedia.org/wiki/Divergence_theorem#:~:text=space%5Bedit%5D-,We,-are%20going%20to%20prove}{divergence theorem}
	\begin{align*}
	...
	\end{align*}
	Using \Cref{global} to extend $\operatorname{Tr}$ continuously to $W^{k,p}(\Omega)= \overline{\restr{C_c^\infty(\R^d)}{\Omega}}$ concludes the proof.
\end{proof}
...
\begin{definition}
	We define the \textbf{trace} of $u \in W^{k,p}(\Omega)$ as $\operatorname{Tr}(u)$. We also use the notation
	\begin{align*}
		\restr{u}{\partial \Omega}:= \operatorname{Tr}(u).
	\end{align*}
\end{definition}
To get an estimate on the trace we paid $1$-degree of regularity. We can do better and only pay ${1}/{p}$ degrees of regularity. This uses the theory of \href{https://en.wikipedia.org/wiki/Trace_operator#For_p_=_1:~:text=%5Bedit%5D-,A%20more,-concrete%20representation%20of}{Besov spaces} which we will not develop here. In the case $p=2$ we can use the fractional Sobolev spaces $H^s(\Omega)$ to get the improved result.
\begin{theorem}
...
\end{theorem}
\begin{proof}
	Straightening out the boundary and using the extension operator as in the trace \Cref{trace}, it is sufficient to work in the case where $u$ is smooth and defined on  $\Hh^{d}$.  By Fourier inversion, if we write $\xi=(\xi',\xi _d) $
	\begin{align*}
...
\end{hint}
The converse to \Cref{trace 0} holds but is far from trivial.
\begin{proposition}
...
\begin{exercise}[Extension trace 0]\label{extensiontrace}
...
\end{hint}
\begin{observation}\label{traceobs}
...
\begin{exercise}[Integration by parts 2] Let $\Omega $ be a \textbf{bounded} open set of $\R^d$ with $C^1$  \textbf{boundary} and consider $u \in  W^{1,p}(\Omega), v \in W^{1,p'}(\Omega)$ then
	\begin{align*}
		\int_{\Omega} (\partial _iu) v = -\int_{\Omega}u \partial _i v+ \int_{\partial \Omega} u v \textbf{n}_i .
	\end{align*}
...
\begin{proof}
	...
	Where in the first inequality we applied Cauchy-Schwarz with $q= d /(d-1), q' =d$. Now applying the general version of Hölder's inequality\begin{align*}
...
\end{theorem}
\begin{proof}
	By density (see \Cref{approx rd}), it is enough to take $u \in C_c^\infty(\R^d)$. Applying the fundamental theorem of calculus gives
	\begin{align*}
...
\begin{exercise}\label{est12}
	Given $ p<\frac{d}{k}$ define    $p_k^*$ by $\frac{1}{p_k^*}=\frac{1}{p}-\frac{k}{d}$. Then,        \begin{align*}
		\norm{u}_{L^{p_k^*}(\R^d)} \lesssim  \norm{\nabla^k u}_{L^p(\R^d)}    \end{align*}
\end{exercise}
\begin{hint}
	Apply induction on $k$ using \Cref{est1}.
\end{hint}
...
\end{exercise}
\begin{hint}
	Use induction on $k$  with \Cref{estt2}.
\end{hint}
...
\end{align*}
This holds if and only if $d=1$. Otherwise, we require more integrability. However, this extra integrability can be converted into regularity in the converse direction of \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{regularitydecayft}{Sobolev spaces}. First, we recall the following definition
\begin{definition}\label{holder def}
	Let $U \subset \R^d$ be open and $\gamma \in \R_+$, we define the \textbf{Hölder space}
	\begin{align*}
...
\begin{exercise}[Sobolev regularity]\label{est32}
...
In the edge case where $\frac{d}{p} = l$ is an integer, the bootstrapping argument stops at $W^{1,d}(\R^d)$, which embeds into $L^q(\R^d)$ for all $q<\infty$, but \textbf{fails} to embed into $L^\infty(\R^d)$ and $C^{0,\gamma}$. To guarantee continuity, we must sacrifice to a slightly lower integrability $q_\eps = d-\eps < d$ in our final bootstrapping step. This pushes us back into the Morrey regime where $p_{l-1} > d$, yielding $\gamma = 1 - \frac{d}{q_\eps} \in (0,1)$, meaning we achieve $C^{m, \gamma}(\R^d)$ for all $\gamma \in (0,1)$.
\end{hint}
Combining the three results gives
\begin{theorem}[Rellich-Kondrachov]\label{rellich}
	Let $\Omega$ be \textbf{bounded} with $C^k$ boundary, then the following  inclusions are continuous
	\begin{align*}
		 ...
		 & W^{k,p}(\Omega) \hookrightarrow C^{m,\gamma }(\overline{\Omega}) \hookrightarrow C^{m}(\overline{\Omega}) ,  \quad & \text{ and }  p>\frac{d}{k}\end{align*}
	Where $p_k^*$ is defined by the relation $\frac{1}{p_k^*}=\frac{1}{p}-\frac{k}{d}$, $m = k - \lfloor d/p \rfloor - 1$, and $\gamma = \lfloor d/p \rfloor + 1 -\frac{d}{p}$ or $\gamma \in (0,1)$ if $\frac{d}{p} \in \mathbb{N}$. Furthermore, the first for $q<p_k^*$, second, fourth, and third composed with fourth inclusions are \href{https://en.wikipedia.org/wiki/Compact_embedding}{compact}.
\end{theorem}
\begin{exercise}
	Show that if $\Omega$ is unbounded then the inclusions in \Cref{rellich} are not compact.
\end{exercise}
\begin{hint}
...
\end{hint}
\begin{proof}
	The fact that the above inclusions are continuous follows by our three Sobolev inequalities in Exercises \ref{est12},\ref{est22},\ref{est32} together with the extension \Cref{extension}.
...
\end{corollary}
\begin{proof}
	For any of the cases in Rellich-Kondrachov's \Cref{rellich}, the condition on $q$ holds. The first part of the result then follows as  $C^k(\overline{\Omega}) \hookrightarrow L^q(\Omega)$ and the second part from the fact that  $p< p^{k^*}$.
\end{proof}
...
\begin{proof}
	...
	\end{align*}
	Using the same technique as in \Cref{completeness} shows that $u^{(\alpha)}= D^{\alpha}u$. This shows that $u_{n_k} \to u$ in $W^{l,q}(\Omega)$ and, by the equivalence of compactness and sequential compactness, concludes the proof.
\end{proof}
This result can be combined directly with Rellich-Kondrachov \ref{rellich} or any other embedding. For example, when combined with \Cref{simple embedding} we get
\begin{corollary}
	Let $q< p_k^*$, then it holds that following inclusion is compact
	\begin{align*}
		W^{k+l,p}(\Omega) \hookrightarrow W^{l,q}(\Omega).
...
Using \Cref{traceobs} we can also obtain Sobolev embeddings for non-smooth domains if we consider Sobolev spaces with trace $0$.

\begin{theorem}[Rellich for trace 0 and irregular domain] 
	Let $U\subset \R^d$ be a \textbf{bounded} domain. Then, the conclusions of \Cref{rellich}, higher degree embeddings, and its corollary hold if we swap everywhere $W^{k,p}(\Omega)$ for $W^{k,p}_0(U)$ and $W^{k+l,p}(\Omega)$ for $W^{k+l,p}_0(U)$.
\end{theorem}
\begin{proof}
	The proof follows in an identical method as the aforementioned results as we still have density of smooth functions $W^{k,p}_0(U)$ and the extension theorem, which is now given by \Cref{extensiontrace}.
\end{proof}
...
\begin{theorem}[Poincaré inequality]\label{poincare} Let $u \in W^{1,p}_0(U)$ where $U$ is bounded in one direction. Then
	\begin{align*}
...
\begin{proof}
	...
	\end{align*}
	Where the first inequality is point one and the second follows from Cauchy-Schwarz. Now applying \href{ https://en.wikipedia.org/wiki/Riesz%E2%80%93Thorin_theorem#:~:text=the%20sumset%20formulation.-,Riesz%E2%80%93Thorin,-interpolation%20theorem%C2%A0%E2%80%94%C2%A0}{Riesz-Thorin's interpolation theorem} concludes the proof.
\end{proof}
...
\begin{proposition}Convolution of distribution and test function is smooth:
	...
	\end{enumerate}
\end{proposition}
The previous definitions all go through word by word in the case where we substitute the domain from Euclidean space  $\R^d$ to a \href{https://en.wikipedia.org/wiki/Haar_measure}{LCA group with the Haar measure} $\mu $. For example
\begin{align*}
...
By \Cref{Young} $(L^1(\R^d),*)$ is a \href{https://en.wikipedia.org/wiki/Banach_algebra}{Banach algebra}. However, it is a non-unital one. That is there does not exist an element $e$ such that
\begin{align*}
...
\begin{proof}
	...
	We now prove the second part. By \Cref{Uryshon} we can choose $g $ such that
	\begin{align*}
...
\end{proposition}
\begin{proof}
	By induction, it suffices to consider the case $D^\alpha = \partial _i$ for some $1 \leq i \leq d$. This case can be proved by a \postref{PDEs/1. The Fourier transform, tempered distributions and Sobolev spaces}{derivationundertheintsign}{differentiation under the integral sign} as, given $\abs{x}\leq M$
	\begin{align*}
...
\begin{observation}[Local smoothing]\label{local smoothing}
	The smoothing effect also holds when $f$ is only defined on some open set $U$. Then, with the notation of Definition \ref{Convolution support}, $f*\phi \in C^\infty(U_\eps )    $ with an identical proof showing
	\begin{align*}
...
\end{theorem}
\begin{proof}
	This follows immediately from \Cref{app pn} and \Cref{smooth}.
\end{proof}
...
\end{exercise}
\begin{hint}
	Let $f$ be the non-smooth function to approximate, multiply it first by a mollifier $\eta_n \in C_c^\infty(U)$ equal to $1$ in $ V_n\subset U$ (see \Cref{bump example2}). Convolve to get
	\begin{align*}
		\varphi_n:=(f \eta_n )*\rho _n.
	\end{align*}
	Show as in \Cref{density thm} that $\varphi_n$ converges appropriately.
\end{hint}
...
\begin{proof}
	The first part can be proved by the same method as in \Cref{Uryshon} (note that Uryshon's lemma \href{https://planetmath.org/ApplicationsOfUrysohnsLemmaToLocallyCompactHausdorffSpaces}{still holds} for LCH spaces). The second part follows by copying the proof of Propositions \ref{app pn} and \ref{smooth}.
\end{proof}
...
\begin{corollary}\label{Stone}
	...
	\end{align*}
\end{corollary}
Note that the second part of \Cref{Stone} also follows from the \href{https://en.wikipedia.org/wiki/Stone%E2%80%93Weierstrass_theorem}{Stone-Weierstrass theorem}.
...
\begin{definition}\label{bump def}
...
\begin{definition}
	Given a manifold $M$ and an open covering $\{U_\alpha\}_{\alpha \in  J}$ of $M$ we say that  $\{\rho_i\}_{i\in  I}$ is a \textbf{partition of unity on $\{U_\alpha\}_{\alpha \in  J}$} if:
	\begin{enumerate}
...
\begin{enumerate}
	...
	\item Obtain a globally defined object $g$ by using the partition of unity to piece together the local objects
	      ...
	      \end{align*}
\end{enumerate}
In addition to the approximation and extension theorems in Section \ref{extension section}, partitions of unity can be used to show that: every manifold has a \href{https://en.wikipedia.org/wiki/Riemannian_manifold}{Riemannian metric}, show that a function is smooth on some non-open set $S \subset M$ iff it is the restriction of a smooth function defined on a neighborhood of $S$, prove the existence of an outward pointing vector on manifolds with boundary, define integration over an orientable manifold $M$, prove \href{https://en.wikipedia.org/wiki/Generalized_Stokes_theorem}{Stokes' theorem}.
...
\end{align}
are diffeomorphisms on $\Hh^d$ (again with the subspace topology). Here $\Aa=\set{(V_\alpha,\Phi_\alpha)} $ is an \textbf{atlas} of $M$. We say that $\Aa$ is $C^k$ if the diffeomorphisms in \eqref{diff} are $C^k$ (see Definition \ref{smooth non open}).
...
In our case we will always take $M$ to be a subset of  $\R^d$, in this case, different variations of the above definition are possible. For example, by the inverse function theorem and \Cref{ext}, we can extend the functions $\Phi_\alpha$ to diffeomorphisms on $U_\alpha$ \textbf{open in $\R^d$} so that \eqref{chart}-\eqref{chart2} now read
