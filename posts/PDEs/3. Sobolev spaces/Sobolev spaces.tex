...
\begin{proof}
	...
	\end{align*}
	Using \Cref{global} to extend $T$ continuously to $W^{k,p}(\Omega)= \overline{\restr{C_c^\infty(\R^d)}{\Omega}}$ concludes the proof.
\end{proof}
\begin{definition}
...
\end{theorem}
\begin{observation}
	The continuous embeddings above also hold for $\Omega = \R^d$ (by the Sobolev inequalities in Exercises \ref{est12},\ref{est22},\ref{est32}). However, the \textbf{compact} embeddings require $\Omega$ to be bounded. Indeed, on $\R^d$ one can construct bounded sequences in $W^{k,p}(\R^d)$ whose mass escapes to infinity, precluding any convergent subsequence.
\end{observation}
\begin{proof}
