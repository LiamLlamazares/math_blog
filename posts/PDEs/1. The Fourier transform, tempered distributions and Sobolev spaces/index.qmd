...
- The *Fourier transform* defines a linear isometry on the space of
	      ...
	      *inverse Fourier transform*(link). This allows us to write these
...
we will write \(\abs{x}\) to denote its norm and the *Japanese
...
**Proposition 1** (Continuity of the integral). *Let
...
**Proposition 2** ({Differentiation under the integral sign}).
*Let \(X\) be a measure space, \(U\) be a convex open interval of
	\({\mathbb R*\), and \(E\) a Banach space. Consider \[f:U\times X\to E\]
	such that:}

- *\(f(t)\) is measurable for every \(t\in U\) and integrable for
		      some \(t_0\in U\).*
- *For each \(x\in X\) we have that \(f(x)\) is differentiable and
...
*Proof.* First, we show that the integral on the left-hand side of
...
integrable everywhere (see for example [@FF] page 108), however as
we have seen it is sufficient to require the integrability at a point.

\hypertarget{the-fourier-transform-on-mathbb-rd}{%

## \texorpdfstring{The Fourier transform on
\({\mathbb R

^d\)}{The Fourier transform on \{\textbackslash mathbb R\}\^{}d}}}
...
**Definition 1**. *Given
	\(f\in L^1({\mathbb R*^d\to\mathbb{C}^m)\) the *Fourier transform*
	of \(f\) and the *inverse Fourier transform of* \(f\) are defined
	respectively by
...
**Proposition 3** ( **Properties of \(\mathcal{F**_1\)}).
*Given \(f\in L^1({\mathbb R*^d\to\mathbb{C}^m)\)}

1. *The Fourier transform \(\mathcal{F*_1\) is a continuous linear
		      operators
...
3. *If \(f\) is absolutely continuous in \(x_j\) for almost every
	      \(x_1,...x_{j-1*,x_{j+1},...,x_d\) then then
	      \[\widehat{\partial x_j f}(\xi)=2\pi i\xi_j\widehat{ f}(\xi)\qquad\forall \xi\in{\mathbb R}^d.\]}

*Proof.* The proof of the first point is an application of the
...
**Definition 2**. *The \emph{Schwartz space*
...
**Lemma 1** (**Plancherel for Schwartz functions**).
*Given \(f,g\in \mathcal{S*({\mathbb R}^d\to\mathbb{C}^m)\) (i)
...
\(\mathcal{F}^*\) the *inverse* Fourier transform. The proof can be
found in [@Tay] pages 222-226. From \((i)\) and \((ii)\) we deduce
immediately that, given a Schwartz function \(f\), \[
...
**Proposition 4** (**Plancherel's theorem**).
*\(\mathcal{F*_\mathcal{S}\) and \(\mathcal{F}_\mathcal{S}^*\) may
...
## The Fourier transform of periodic
	  functions
...
**Definition 3**. *Given
	\(f\in L^1({\mathbb T*^d\to\mathbb{C}^m)\) we define the *\(k\)-th
...
See for example [@Tay] pages 197-206. We conclude that
...
*\(\mathcal{F*_{C^\infty}\) and \(\mathcal{F}_{C^\infty}^*\) may be
...
**Definition 4**. *The space of \emph{tempered distributions*
...
**Definition 5**. *Given
	...
	\(\alpha\in\mathbb{N}^d\) we define the *(distributional) Fourier
		transform* of \(w\) by \[\mathcal{F}w:= w\circ \mathcal{F}^{-1}\] and
	the *(weak) \(\alpha\)'th* derivative of \(w\) by
	\[D^\alpha w:= w\circ((-1)^\abs{\alpha}D^\alpha).\]}

The method we used above to extend \(\mathcal{F}, D^\alpha\) is called
the *duality method* and appears very frequently. Another way of
...
**Definition 7**. *Given \(k\in\mathbb{N*^+\) we define the
	*Sobolev space* \(H^k({\mathbb R}^d\to\mathbb{C}^m)\) as:
	\[\begin{gathered}
...
*Proof.* The proof is an application of the Cauchy-Schwartz
inequality and
...
\[\begin{gathered}
	...
	\end{gathered}\] In consequence, the sum \[
...
definition of *pseudo-differential operators*. A particular case is
