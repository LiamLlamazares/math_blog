...
- The *Fourier transform* defines a linear isometry on the space of square-integrable complex-valued functions and has as inverse the *inverse Fourier transform*(link). This allows us to write these functions as a superposition of harmonic functions.(link)

- Linear differential operators act as a simple polynomial multiplication on the frequency domain.(link) Smooth functions have Fourier transforms that decay quickly and vice-versa(link). This correspondence also allows us to introduce fractional differential operators and pseudo differential operators such as $\sqrt{-\Delta }$(link).

- The Sobolev spaces $H^s$ and tempered distributions  $\Ss'$ are complete spaces of functions to which we can extend differential operators.(link)
...
Much of the material here contained can be found in my undergraduate thesis(link) on the Navier Stokes equations which in turn are based on Terence Tao's excellent notes(link).
...
In the literature the usual hypothesis is that $\partial _t f$ is integrable everywhere (see for example [@FF] page 108), however as we have seen it is sufficient to require the integrability at a point.
...
The importance of these last two properties is that they give us information on the Fourier transform of a function $f$ based on its regularity and decay. They can be stated informally as decay gives regularity and regularity gives decay (as the Fourier transform of any $f\in L^1(\R^d\to\R^d)$ decays to 0 at infinity by the Riemann-Lebesgue lemma(link)).
...
As then $\Ff_1 f_n$ converges uniformly to $\Ff_1f$ by $1$ (point one prop 1 link) and in $L^2(\R^d\to\C^m)$ to $\Ff f$ by Proposition 2 (link), from where we deduce that as desired $\Ff_1f=\Ff f$. 
...
Where,  we consider $L^2(\R^d\to\C^m)$ as a subspace of $\Ss '(\R^d\to\C^m)$ (see linkto2).
