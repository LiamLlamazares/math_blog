...
\begin{document}
	\title{PDEs 1: The Fourier transform, tempered distributions and Sobolev spaces}
	\postaliases{/2023-01-29-PDE-1-Fourier/}
\postsubtitle{The holy trinity}
	\author{Liam Llamazares--Elias}
	\date{10-14-2022}
	\maketitle
	\section*{ Three line summary }
	\begin{itemize}
		\item The \emph{Fourier transform} defines a linear isometry on the space of square-integrable complex-valued functions and has as inverse the \emph{inverse Fourier transform}. This allows us to write these functions as a superposition of harmonic functions.
		\item Linear differential operators act as a simple polynomial multiplication on the frequency domain. Smooth functions have Fourier transforms that decay quickly and vice-versa. This correspondence also allows us to introduce fractional differential operators and pseudo differential operators such as $\sqrt{-\Delta }$.
		\item The Sobolev spaces $H^s$ and tempered distributions  $\Ss'$ are complete spaces of functions to which we can extend differential operators.
	\end{itemize}
	...
	Much of the material here contained can be found in my \href{https://gredos.usal.es/bitstream/handle/10366/145629/TFG%20LIAM%20LLAMAZARES.pdf;jsessionid=CA7AE0CE6BF33CAA6305E55B21A9A1E0?sequence=1}{undergraduate thesis} on the Navier Stokes equations which in turn are based on Terence Tao's excellent \href{https://terrytao.wordpress.com/2018/09/03/254a-notes-0-physical-derivation-of-the-incompressible-euler-and-navier-stokes-equations/}{notes}.
...
\begin{proposition}[Continuity of the integral]\label{commuting limits with integral}
	...
	\[f:T\times X\to E\]
	such that $f(t)$ is (\postref{Analysis on Banach spaces/Bochner_integral}{}{Bochner}) integrable for all ${t}\in{T}$ and such that
...
\end{proposition}
\begin{proof}
	The proof is an application of the \href{https://en.wikipedia.org/wiki/Dominated_convergence_theorem}{dominated convergence theorem} to the
	sequence of functions $f_n:=f(t_n)$ where $\{t_n\}_{n=1}^\infty$ is some sequence converging to $t$ and the fact that, in first countable spaces, sequential continuity is \href{https://en.wikipedia.org/wiki/Sequential_space#Sequential_continuity}{equivalent} to continuity.
\end{proof}
...
\\&=\int_{X}\lim_{t \to t_0} \frac{f(t)-f(t_0)}{t-t_0} \mu (dx) =\int_{X}\partial _t f(t) \mu(dx).
	\end{align*}
	Where in the first equality we also used the  \postref{Analysis on Banach spaces/Bochner_integral}{strong measurability sec}{linearity of the Bochner integral} and in the last equality we used that by hypothesis $f(x)$ is everywhere differentiable. This concludes the proof.
\end{proof}
In the literature the usual hypothesis is that $\partial _t f$ is integrable everywhere \cite[p. 108]{FF}, however as we have seen it is sufficient to require the integrability at a point.
...
\begin{proposition}[Properties of $\Ff_1$]\label{regularitydecayft}
	Given $f\in L^1(\R^d\to\C^m)$
	\begin{enumerate}
		\item The Fourier transform   $\Ff_1$ is a continuous linear operators $$\Ff_1:L^1(\R^d\to\C^m) \to L^\infty(\R^d\to\C^m)$$\label{p1}.
...
\begin{proof}
	...
	\end{equation*}
	and similarly for the inverse. The second point follows by differentiation under the integral \Cref{derivationundertheintsign}. The third point can be proved by using integration by parts and the observation that every absolutely continuous function must go to zero at infinity.
\end{proof}
...
\bigbreak
The importance of these last two properties is that they give us information on the Fourier transform of a function $f$ based on its regularity and decay. They can be stated informally as decay gives regularity and regularity gives decay (as the Fourier transform of any $f\in L^1(\R^d\to\R^d)$ decays to 0 at infinity by the \href{https://en.wikipedia.org/wiki/Riemann%E2%80%93Lebesgue_lemma}{Riemann-Lebesgue lemma}).\bigbreak
...
\end{definition}
By applying \Cref{regularitydecayft} we deduce that, given $f\in\mathcal{S}(\R^d\to\C^m)$
...
\begin{lemma}[\textbf{Plancherel for Schwartz functions}]\label{Planncherellemma}
	...
	\begin{enumerate}[i)]
		\item $\br{\mathcal{F}_\mathcal{S}f,g}_{L^2(\R^d\to\C^m)}=\br{f,\mathcal{F}_\mathcal{S}^*g}_{L^2(\R^d\to\C^m)};\quad\br{\mathcal{F}_\mathcal{S}^*f,g}_{L^2(\R^d\to\C^m)}=\br{f,\mathcal{F}_\mathcal{S}g}_{L^2(\R^d\to\C^m)}$\label{adjointft}
		\item $\mathcal{F}_\mathcal{S}^*\mathcal{F}_\mathcal{S}f=\mathcal{F}_\mathcal{S}\mathcal{F}_\mathcal{S}^*f=f$\label{inverseft}
	\end{enumerate}
\end{lemma}
...
From \Cref{adjointft} and \Cref{inverseft} we deduce immediately that, given a Schwartz function $f$,
...
As then $\Ff_1 f_n$ converges uniformly to $\Ff_1f$ by \Cref{p1} and in $L^2(\R^d\to\C^m)$ to $\Ff f$ by Plancherel \Cref{Plancherel's Theorem}, from where we deduce that as desired $\Ff_1f=\Ff f$. \bigbreak
...
\begin{definition}
	Given $f\in L^1(\T^d\to\C^m)$ we define the \emph{$k$-th Fourier coefficient} of $f$ as:
...
This is indeed smooth as, by the rapid decay of $a$, we may  differentiate under the integral sign \Cref{derivationundertheintsign} (we recall that sums are just integrals against discrete measures) to commute all derivatives with the above sum to obtain that
...
\section{Distributions and Sobolev Spaces}\label{distrsec}
...
\end{align}
with $T_f$ defined by 
\begin{aligneq}\label{identificationfuncdist}
	T_f(u):=\int_{\R^d} u\cdot \bar{f}\quad\forall{u}\in {\Ss(\R^d\to\C^m)}.
\end{aligneq}
In both cases we have that, by duality, due to the \eqref{rgivesdpft}, given a tempered distribution $w$ and $\alpha\in \N^d$
...
\begin{definition}\label{sobolevdef}
	...
	\begin{align*}
		H^k(\R^d\to\C^m):=
 \lbrace f\in &L^2(\R^d\to\C^m)\\&: D^\alpha f\in L^2(\R^d\to\C^m)\hookrightarrow\Ss '(\R^d\to\C^m)\quad\forall\hspace{2pt}\abs{\alpha}\leq k\rbrace.
	\end{align*}
	Where,  we consider $L^2(\R^d\to\C^m)$ as a subspace of $\Ss '(\R^d\to\C^m)$ using \eqref{identificationfuncdist}.\end{definition} We may interpret the Sobolev space $H^k(\R^d\to\C^m)$ as the space of $k$ times differentiable functions in $L^2(\R^d\to\C^m)$ and we give it the norm:
\begin{aligneq}\label{normdef1}
	\norm{f}_{H^k(\R^d\to\C^m)}:=\sum_{\abs{\alpha}\leq k}\norm{D^\alpha f}_{L^2(\R^d\to\C^m)}\qquad f\in H^k(\R^d\to\C^m).
\end{aligneq}

Note that it is not enough to require $\abs{\alpha}=k$ as, for example, the tempered distribution $1 \in \Ss'(\R^d\to\C)$ has a derivative equal to zero however is not itself in $L^2(\R^d\to\C)$. Now, by Plancherel for $\Ss$ \Cref{Planncherellemma}, the Fourier transform is an automorphism of $\Ss'(\R^d\to\C^m)$, by using \Cref{regularitydecayft}  we deduce that
...
\begin{aligneq}\label{fracsobdef}
...
\begin{aligneq}\label{sobolevdef2}
	H^k(\T^d\to\C^m):=
\lbrace f\in &L^2(\T^d\to\C^m)\\&: D^\alpha f\in L^2(\T^d\to\C^m)\hookrightarrow\Ss '(\T^d\to\C^m)\quad\forall\hspace{2pt}\abs{\alpha}\leq k\rbrace
\end{aligneq}
Using the same method as before, by the correspondence between regularity and decay for the Fourier transform of periodic functions  \Cref{regdecaypft} and \Cref{Plancherelperiodictheorem}, we deduce that
...
\end{equation}
where of course the definitions in \eqref{sobolevdef2} and \eqref{sobolevgeneraldef} coincide for $s\in\N$.
...
\begin{lemma}[\textbf{Sobolev Embedding into Continuous Functions}]\label{Sobolev derivatives embedding periodic}
...
\begin{proof}
	...
	\end{equation}
	converges absolutely (and uniformly). Since by Plancherel's Theorem, the above sum also converges in $L^2(\T^d\to\C^m)$ to $f$ we deduce that \eqref{Fourier sum} converges almost everywhere to $f$ (for example by taking a subsequence of the above sum that converges almost everywhere to $f$). Therefore
...
\begin{proof}
	...
	is absolutely convergent (by \Cref{Sobolev derivatives embedding periodic}), and hence, we may commute the derivatives of $f$ with the sum in its Fourier series to deduce the point-wise equality
...
\begin{equation*}
	p(x,D)f:=\Ff^{-1}\qty(p(x,\cdot  )\hat{f}(\cdot  )).
\end{equation*}
...
\begin{equation*}
{(-\Delta)}^sf:=\Ff^{-1} \left(\qty(4\pi^2 \abs{\xi }^2)^s\hat{f}(\xi )\right).
\end{equation*}
